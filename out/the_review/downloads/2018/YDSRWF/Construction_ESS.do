...
do "Coding_Far_Right_Parties.do"
...
*** Remove countries where there is no first stage (i.e. that the reform did not increase respondent education levels)
quietly foreach x in "PT (1964)" {
	di "******************** Reform = `x' **********************"
	foreach y in plus6 eduyrs_actual {
		rdplot `y' running if reform=="`x'", c(0) p(3) lowerend(-20) upperend(19) numbinl(20) numbinr(20) graph_options(graphregion(fcolor(white) lcolor(white)) ylab(,nogrid) ///
			title("Portugal (1964)") ytitle("6 yrs of schooling or more") xtitle("Cohort relative to reform") legend(off) ylabel(#3))
		graph save Graph "g_`y'_`x'.gph", replace
	}
}

quietly foreach x in "ES (1969)" {
	di "******************** Reform = `x' **********************"
	foreach y in plus8 eduyrs_actual {
		rdplot `y' running if reform=="`x'", c(0) p(3) lowerend(-20) upperend(19) numbinl(20) numbinr(20) graph_options(graphregion(fcolor(white) lcolor(white)) ylab(,nogrid) ///
			title("Spain (1969)") ytitle("8 yrs of schooling or more") xtitle("Cohort relative to reform") legend(off) ylabel(#3))
		graph save Graph "g_`y'_`x'.gph", replace
	}
}

quietly foreach x in "AT (1962)" "DE (varies by region)" "FI (varies by region)" "IE (1967)" "IT (1963)" "GR (1975)" {
	di "******************** Reform = `x' **********************"
	foreach y in plus9 eduyrs_actual {
		rdplot `y' running if reform=="`x'", c(0) p(3) lowerend(-20) upperend(19) numbinl(20) numbinr(20) graph_options(graphregion(fcolor(white) lcolor(white)) ylab(,nogrid) ///
			title("`x'") ytitle("9 yrs of schooling or more") xtitle("Cohort relative to reform") legend(off) ylabel(#3))
		graph save Graph "g_`y'_`x'.gph", replace
	}
}

quietly foreach x in "BE (1983)" {
	di "******************** Reform = `x' **********************"
	foreach y in plus12 eduyrs_actual {
		rdplot `y' running if reform=="`x'", c(0) p(3) lowerend(-20) upperend(19) numbinl(20) numbinr(20) graph_options(graphregion(fcolor(white) lcolor(white)) ylab(,nogrid) ///
			title("Belgium (1983)") ytitle("12 yrs of schooling or more") xtitle("Cohort relative to reform") legend(off) ylabel(#3))
		graph save Graph "g_`y'_`x'.gph", replace
	}
}

gr combine "g_plus9_AT (1962).gph" "g_plus12_BE (1983).gph" "g_plus9_FI (varies by region).gph" "g_plus9_DE (varies by region).gph" "g_plus9_GR (1975).gph" "g_plus9_IE (1967).gph" "g_plus9_IT (1963).gph" "g_plus6_PT (1964).gph" "g_plus8_ES (1969).gph", rows(3) cols(3) ///
	subtitle(, color(black) fcolor(white) lcolor(white)) graphregion(fcolor(white) lcolor(white) ifcolor(white) ilcolor(white))
graph save Graph "no_FS_limit.gph", replace
...
*** Remove countries without strong first stages
drop if cntry=="BE" | cntry=="DE" | cntry=="IT" | cntry=="IE" | cntry=="ES" | cntry=="FI" | cntry=="AT" | cntry=="GR" | cntry=="PT" | regiongb==12
