		...
		*****ll=0 is all countries, 1 only countries that ever had conflict
		...
		forval way = 0 / 1 {
			...
			foreach vv in $models {
				...
				*							
				drop if p_`x'_model22 == .
				drop if  `x'_before  == .
				
				if `way' == 1 {
					if mod(`way',2) == 1 { 	
						*
						replace p_`x'_model`vv' = p_`x'_model22
					}
					if mod(`way',2) != 1 { 	
						*
						replace p_`x'_model`vv' = p_`x'_model22w
					}
				}
...
forval c = 3(-1)2  {
	...
	use "output/nofe_predict_one_`on_or_in'_int`c'_to15_alpha3_beta001_all_both", clear
	...
	*drop if country == ""
	...
	postfile buffer way cut model before conftype tpn fpn tpd fpd using output/for_roc_onlyintersection_nofe_`on_or_in'`runs', replace
	prepare_roc 0 `cheat'
	postclose buffer
...
forval r = 1 / `runs' {
	...
	clear
	use output/for_roc_onlyintersection_nofe_`on_or_in'`r'
	...
	*gen area = tpr * fpr_dif
	...
	save output/for_roc_onlyintersection_nofe_`on_or_in'`r', replace
