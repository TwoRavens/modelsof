...
gen italian=(bpl==434|fbpl==434|mbpl==434)
gen irish=(bpl==414|fbpl==414|mbpl==414) 
gen belgian=(bpl==420|fbpl==420|mbpl==420) 
gen french=(bpl==421|fbpl==421|mbpl==421) 
gen swiss=(bpl==426|fbpl==426|mbpl==426) 
gen portuguese=(bpl==436|fbpl==436|mbpl==436) 
gen english=(bpl==410|fbpl==410|mbpl==410) 
gen scottish=(bpl==411|fbpl==411|mbpl==411) 
gen welsh=(bpl==412|fbpl==412|mbpl==412) 
gen danish=(bpl==400|fbpl==400|mbpl==400) 
gen norwegian=(bpl==404|fbpl==404|mbpl==404) 
gen swedish=(bpl==405|fbpl==405|mbpl==405) 
gen finnish=(bpl==401|fbpl==401|mbpl==401) 
gen austrian=(bpl==450|fbpl==450|mbpl==450) 
gen russian=(bpl==465|fbpl==465|mbpl==465)
...
foreach nat in german italian irish belgian french swiss portuguese english scottish welsh danish norwegian swedish finnish austrian russian {
	...
	forval by=1850/1930 {
		...
		gen ethnic=`b'
		local coh=`by'-20
		collapse (count) pernum (first) firstname sex if birthyear<`by'&birthyear>=`coh', by(ethnic nameid)
...
save FNIdataset_coh20, replace* start from complete count 1930 census 
