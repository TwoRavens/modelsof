...
use panel_commune_2006_2008.dta, clear
...
* REGRESSION
	...
	* National city
	gen hanoi=(tinh==101)
	gen haiphong=(tinh==103)
	gen danang=(tinh==501)
	gen cantho=(tinh==815)
	gen hcm=(tinh==701)
	gen city=(tinh==101 | tinh==103 | tinh==501 | tinh==815 | tinh==701)
	
	global inde time treatment time_treatment lnarea lnpopden city i.reg8 
				
	set more off
	reg transport treatment
	outreg2 using HDND_regression2006.xls, bdec(3) tdec(3) e(rmse)  replace
	
	* REGRESSION
	xi: ivreg2 goodroadv $inde, cluster(tinh huyen)
	
	outreg2 using HDND_regression2006.xls,  bdec(3) tdec(3) e(rmse) 
	...
	reg transport treatment
	outreg2 using HDND_regression2006.xls, bdec(3) tdec(3) e(rmse)  excel
...
mean 	goodroadv transport pro3 tapwater roadv
		...
		broadcast post vpost
		rm2c7a	rm2c7f market nonfarm vmarket1 vmarket2 vmarket3
		index1 index2 index3 index4 index5 index6 if year==2006;
...
outreg2 using HDND_province_regression2006.xls,  bdec(3) tdec(3) e(rmse) replace;
reg index2 time treatment time_treatment  city i.reg8, robust cluster(tinh) ;
outreg2 using HDND_province_regression2006.xls,  bdec(3) tdec(3) e(rmse);
...
outreg2 using HDND_province_regression2006.xls,  bdec(3) tdec(3) e(rmse) excel;
...
/* Note; full outcome list is
	# delimit ;
	
		foreach x in pro1 pro2 pro3 pro4 pro5 pro6 
		...
		xi: xtreg `x' time treatment time_treatment lnarea lnpopden rnongnghiep rcongnghiep rdichvu, i(tinh) fe cluster(tinh);
