...
* Plots: Standard deviation of payouts


// LIT
twoway ///
	(contour t1_sd_points y x, scolor(white) ecolor(gs4) crule(linear) ccuts(0 (25) 400)) ///	
	...
	zlabel(0 (50) 400, labsize(vsmall)) ///
	clegend( title("Standard" "deviation" "of tokens", size(vsmall)) width(4) forcesize region(margin(vsmall)) bmargin(vsmall)) ///
	ztitle("") ///
	ytitle("") ///
	xtitle("") ///
	scheme(s1mono) ///
	...
	name(LIT_polspa, replace)


// HIT
twoway ///
	(contour t2_sd_points y x, scolor(white) ecolor(gs4) crule(linear) ccuts(0 (25) 400)) ///	
	...
	if (x>9 & x<121 & y>39 & y<111), ///
	zlabel(0 (50) 400, labsize(vsmall)) ///
	clegend( title("Standard" "deviation" "of tokens", size(vsmall)) width(4) forcesize region(margin(vsmall)) bmargin(tiny)) ///
	ztitle("") ///
	ytitle("") ///
	xtitle("") ///
	scheme(s1mono) ///
	...
	graphregion(color(white) margin(vsmall)) ///
	plotregion(margin(vsmall)) ///
	name(HIT_polspa, replace)


// MT
twoway ///
	(contour t3_sd_points y x, scolor(white) ecolor(gs4) crule(linear) ccuts(0 (25) 400)) ///	
	...
	graphregion(color(white) margin(vsmall)) ///
	plotregion(margin(vsmall)) ///
	name(MT_polspa, replace)


graph combine LIT_polspa ///
			HIT_polspa ///
			MT_polspa, ///
			imargin(tiny) rows(3) xsize(3.5) ysize(7) iscale(1) graphregion(color(white) margin(vsmall))
graph export "Figure_3.png", width(2000) replace
