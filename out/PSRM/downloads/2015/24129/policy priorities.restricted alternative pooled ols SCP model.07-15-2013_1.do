* KRAUSE & COOK "POLICY PRIORITIES" (JULY 15, 2103): ALTERNATIVE MODEL 2 ANALYSIS (RESTRICTED MODEL : MODEL 6: EXCLUDE ALL AGENCY-LEVEL AND TIME-WISE DUMMY FIXED EFFECTS IN SCP MODEL TO ASSESS ROBUSTNESS OF THE ESTIMATED PARTISAN PRESIDENTIAL BDGETARY PREFERENCE ESTIMATES) 

* OPEN STATA OUTPUT FILE LOG

log using "D:\Asymmetric Loss Project\New Version (2012)\Data Folder\policy priorities.restricted alternative EIP models.pooled ols SCP model.07-15-2013.smcl", replace
...
use "D:\Asymmetric Loss Project\New Version (2012)\Data Folder\policy priorities.02-01-12.dta", clear
  ...
  *************************************************************************************************************************************************************************************************************************************************************************************** SCP MODEL: MODEL 6 (ROBUSTNESS CHECK OF UNRESTRICTED MODEL REPORTED IN MANUSCRIPT: SANS AGENCY-LEVEL AND TIME-WISE DUMMY FIXED EFFECTS IN SCP MODEL: BUT WITH SEs CLUSTER-ADJUSTED BY AGENCY) ********
...
** MODEL 6 (REPORTED IN THE SUPPLEMENTARY APPENDIX): (USE FOLLOWING 'AGENCY-IDEOLOGY' ADJUSTED COVARIATES: HDEMPCTAI, SDEMPCTAI, ASCIHA, & ASCTHA INTERACTION TERMS FROM SCP PROPOSAL MODEL SPECIFICATION)
...
test scpprop + scppropupg = 0 

*

test ip = scpprop

test ip + ipupg = scpprop + scppropupg
...
xtreg appgrowth scpprop scppropasym ip ipasym asymmetric hdempctai sdempctai congelyr cpartmajchangecorrected asctha asciha lagbudgetgap ueratecong lagfedsurpdefpctgdp vietiraqwardefense budget74amends grh any_supp nixonford carter reagan ghwbush clinton gwbush, fe vce(bootstrap, nodots bca reps(10000) seed(123))
...
save "D:\Asymmetric Loss Project\New Version (2012)\Data Folder\policy priorities.unrestricted alternative pooled ols SCP model.07-15-2013.allvars.dta", replacelog close