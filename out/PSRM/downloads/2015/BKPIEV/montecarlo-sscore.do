...
log using montecarlo-sscore.log, text replace;
...
/*      Purpose:	Run Monte Carlo of endogeneous S scores as in Boehmke, Chyzh,	*/
/*			and Thies (PSRM 2016). Note that we do not set a seed, so you 	*/
/*			will not get identical results.					*/
/*      Output File:	montecarlo-sscore.log,						*/
/*			montecarlo-sscore.dta						*/
/*			graphs/montecarlo-sscore-XXX.gph				*/
/*	Requires:	scompute.ado (Signorino and Ritter)				*/
/*	************************************************************************	*/	
...
program define mkdyads, rclass;
  ...
  if "`saving'" != "" {;
	...
	/* error terms, calculates the S scores and runs the models.	*/
	/****************************************************************/

	
program define mc_sscore, rclass;

  syntax, indvars(string) [rho(real 0)];
  
	tempfile sscore data;

	use `indvars', clear;
  
		/* Set standard deviation of the network data. */
	...
	generat y2 = z1_01 + z1_02 - distance + e2;
	  ...
	  save `data', replace;
			...
			/* Generate the S scores using the observed dyadic variable. */

	  keep id_01 id_02 year y2;

		scompute id_01 id_02, id(year) svar(y2);
		
		save `sscore', replace;
		
	use `data', clear;

	merge 1:1 id_01 id_02 using `sscore', generat(_merge_y2);
	  ...
	  merge 1:1 id_01 id_02 using `data', generat(_merge_y22) update;

	generat y1 = 0 + 0.25*x1_01 + 0.25*x1_02 - 0.25*distance - 2.5*S_y2 + e1;

			/* Generate the S scores using the predicted dyadic variable. */
	...
	regress y2 z1_01 z1_02 distance if id_01!=id_02;
	  ...
	  keep id_01 id_02 year y2_hat;
	  
		scompute id_01 id_02, id(year) svar(y2_hat);
		
		save `sscore', replace;
		
	use `data', clear;

	  merge 1:1 id_01 id_02 using `sscore', generat(_merge_y2_hat);
	  ...
	  merge 1:1 id_01 id_02 using `data', generat(_merge_y2_hat2) update;
	  
		/*Drawing y2_hat 5 times to capture uncertainty*/
	  
	  predict stdp, stdp;
	  
	  forvalues num= 1/5 {;
	  
		generat y2_hat`num'=y2_hat+invnormal(uniform())*stdp;
	
		replace y2_hat`num' = 0 if id_01==id_02;
	  
		save `data', replace;
		
		keep id_01 id_02 year y2_hat`num';
	  
		scompute id_01 id_02, id(year) svar(y2_hat`num');
		
		save `sscore', replace;
		
		use `data', clear;

		  merge 1:1 id_01 id_02 using `sscore', generat(_merge_y2_hat_`num');
		...
		merge 1:1 id_01 id_02 using `data', generat(_merge_y2_hat2_`num') update;  
	  
		}; 	  

		/* Generate the IV S scores running a regression. */
		/* First generate second degree polynomials for all first stage regressors. */
	...
	generat z1_0102dist=z1_01*z1_02*distance;
		...
		/*Second estimate the IV. */
	
	regress S_y2 z1_01 z1_01_sq z1_01_cube z1_02 z1_02_sq z1_02_cube distance distance_sq distance_cube 
		z1_0102 z1_0102_sq z1_0102_cube z1_01dist z1_02dist z1_0102dist;

	  predict S_y2_iv;

	  replace S_y2_iv = 0 if id_01==id_02;

		/* Now run the models. */

	regress y1 x1_01 x1_02 S_y2 distance if id_01!=id_02;
	
	  return scalar b0 = _b[_cons];
	  return scalar b1 = _b[x1_01];
	  return scalar b2 = _b[x1_02];
	  return scalar b3 = _b[S_y2];
	  return scalar b4 = _b[distance];
	
	  return scalar se_b0 = _se[_cons];
	  return scalar se_b1 = _se[x1_01];
	  return scalar se_b2 = _se[x1_02];
	  return scalar se_b3 = _se[S_y2];
	  return scalar se_b4 = _se[distance];
	  
	  	/* Loop over the multiply imputed S score values. */
	
	forvalues num=1/5 {;
	
	  regress y1 x1_01 x1_02 S_y2_hat`num' distance if id_01!=id_02;
	  ...
	  return scalar a3 = _b[S_y2_hat5];
	  return scalar a4 = _b[distance];
		  	  
	  return scalar se_a0 = _se[_cons];
	  return scalar se_a1 = _se[x1_01];
	  return scalar se_a2 = _se[x1_02];
	  return scalar se_a3 = _se[S_y2_hat5];
	  return scalar se_a4 = _se[distance];
		  	  
	ivregress 2sls y1 x1_01 x1_02 distance (S_y2 = z1_01 z1_01_sq z1_01_cube z1_02 z1_02_sq z1_02_cube distance distance_sq distance_cube 
...
drawnorm x1 z1, means(0 , 0) sd(1 , 1);
  ...
  save montecarlo-sscore-indvars, replace;
...
save montecarlo-sscore, replace emptyok;
  
forvalues rho=-.75(0.25)0.75 {;
  ...
  simulate b0=r(b0) b1=r(b1) b2=r(b2) b3=r(b3) b4=r(b4) 
	...
	reps(500) saving("sims/montecarlo-sscore-(rho=`rho').dta", replace):  
	mc_sscore, indvars(montecarlo-sscore-indvars) rho(`rho');
	
	append using montecarlo-sscore;
	
	save montecarlo-sscore, replace;
  
  };

		/* Now create some figures summarizing the results. */

use montecarlo-sscore, clear;
...
collapse (mean) a* b* g* se*
  ...
  twoway connected a1 a2 g1 g2 b1 b2 rho, scheme(s1color) 
	...
	legend(label(1 b_x1 Corrected (S-Score of IV)) label(2 b_x2 Corrected (S-Score of IV)) 
	  label(3 b_x1 Corrected (IV of S-Score)) label(4 b_x2 Corrected (IV of S-Score))
	  label(5 b_x1 Uncorrected) label(6 b_x2 Uncorrected) order(1 2 3 4 5 6) cols(2))
	xtitle(rho)
	saving(graphs/montecarlo-sscore-b_x, replace);
  ...
  twoway connected b3 g3 a3 rho, scheme(s1color) 
	...
	title(S-Score)
	saving(graphs/montecarlo-sscore-b_sscore-bw, replace);
  ...
  twoway connected b4 g4 a4 rho, scheme(s1color) 
	...
	xtitle(Error Correlation)
	title(S-Score)
	saving(graphs/montecarlo-sscore-b_distance-bw, replace);
  ...
  graph bar se_b3 sd_b3 se_g3 sd_g3 se_a3 sd_a3, over(rho) scheme(s1color)
	...
	legend(label(1 SE Uncorrected) label(2 SD Uncorrected) label(3 SE IV of S-Score) label(4 SD IV of S-Score)
	   label(5 SE S-Score of IV) label(6 SD S-Score of IV))
	saving(graphs/montecarlo-sscore-se_sscore, replace);
  ...
  graph bar se_b1 sd_b1 se_g1 sd_g1 se_a1 sd_a1, over(rho) scheme(s1color)
	...
	legend(label(1 SE Uncorrected) label(2 SD Uncorrected) label(3 SE IV of S-Score) label(4 SD IV of S-Score)
	   label(5 SE S-Score of IV) label(6 SD S-Score of IV))
	saving(graphs/montecarlo-sscore-se_x1, replace);
  ...
  graph bar se_b2 sd_b2 se_g2 sd_g2 se_a2 sd_a2, over(rho) scheme(s1color) 
	...
	legend(label(1 SE Uncorrected) label(2 SD Uncorrected) label(3 SE IV of S-Score) label(4 SD IV of S-Score)
	   label(5 SE S-Score of IV) label(6 SD S-Score of IV))
	saving(graphs/montecarlo-sscore-se_x2, replace);
  ...
  graph bar se_b4 sd_b4 se_g4 sd_g4 se_a4 sd_a4, over(rho) scheme(s1color) 
	...
	legend(label(1 SE Uncorrected) label(2 SD Uncorrected) label(3 SE IV of S-Score) label(4 SD IV of S-Score)
	   label(5 SE S-Score of IV) label(6 SD S-Score of IV))
	saving(graphs/montecarlo-sscore-se_distance, replace);
  ...
  graph bar b1_rmse g1_rmse a1_rmse, over(rho) scheme(s1color)
	...
	legend(label(1 Uncorrected) label(2 IV of S-Score) label(3 S-Score of IV) rows(1))
	saving(graphs/montecarlo-sscore-rmse_x1, replace);
  ...
  graph bar b2_rmse g2_rmse a2_rmse, over(rho) scheme(s1color)
	...
	bar(3, color(dkgreen) fintensity(inten100))
	legend(label(1 Uncorrected) label(2 IV of S-Score) label(3 S-Score of IV) rows(1))
	saving(graphs/montecarlo-sscore-rmse_x2, replace);
  ...
  graph bar b3_rmse g3_rmse a3_rmse, over(rho) scheme(s1color)
	...
	title(S-Score)
	legend(label(1 Uncorrected) label(2 IV of S-Score) label(3 S-Score of IV) rows(1))
	saving(graphs/montecarlo-sscore-rmse_sscore-bw, replace);
  ...
  graph bar b4_rmse g4_rmse a4_rmse, over(rho) scheme(s1color)	
	...
	bar(3, color(dkgreen) fintensity(inten100))
	legend(label(1 Uncorrected) label(2 IV of S-Score) label(3 S-Score of IV) rows(1))
	saving(graphs/montecarlo-sscore-rmse_distance, replace);
  
		/* Clean up program-generated files. */
		
erase S_y2_computed.dta;
erase S_y2_hat_computed.dta;
erase S_y2_hat1_computed.dta;
erase S_y2_hat2_computed.dta;
erase S_y2_hat3_computed.dta;
erase S_y2_hat4_computed.dta;
erase S_y2_hat5_computed.dta;
erase montecarlo-sscore-indvars.dta;
