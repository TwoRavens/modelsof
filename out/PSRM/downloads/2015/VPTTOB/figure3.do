...
clear;
save se_data_oda_map1.dta, replace emptyok;

clear;
save coeff_draws_oda_map1.dta, replace emptyok;
...
use coeff_draws_oda_map1.dta, clear;
set obs 500;
drawnorm b1-b162, mean(B) cov(V);
save coeff_draws_oda_map1, replace;

forvalues num=1/100 {;
use coeff_draws_oda_map1, clear;
...
di `num';
di `t';
append using se_data_oda_map1.dta;
save se_data_oda_map1, replace;
save "sims/se_data_oda_num`num'_t`t'", replace;
local h=`num'-1;
local d=`t'-1;
capture erase "sims/se_data_oda_num`h'_t`d'.dta";
};
...
/*Make the map*/
use "se_data_oda_map1.dta", clear;
...
use map_data_oda.dta, clear;
joinby ccode using worlddb.dta, _merge(_merge_country) unmatched(both);
replace yhat_change=yhat_change*(-1);

spmap yhat_change using worldcoord if t==19, id(ID) 
	fcolor(Greys)	
	clmethod(custom)
	clbreaks(-.0000119 0 .00000179 .0005842 .0005844)
	saving("tables/yhat_change_Libya_oda", replace)
	ysize(3) xsize(5)
	legend(placement(9))
	legt("Change in Tax Ratio")	
	ndfcolor(blue)
	legend(label (5 "Tunisia") label(4 "Moderate") label(3 "Small") label(2 "Positive") label(1 "No Data"));
	
	graph export "tables/yhat_change_Libya_oda.eps", replace;
	
/*Delete unnecessary files*/	
erase se_data_oda_map1.dta;
erase coeff_draws_oda_map1.dta;
erase  map_data_oda.dta;
erase cont_st_W.dta;
erase riv_st_W.dta;
