/*******************************
* This file uses hitlevel data to calculate reliability coefficients for
* RAs, individual coders, and for metacoders
*******************************/
// LaTeX log files
...
// CONTROL PARAMS -- set to * to skip re-doing laborious calculations
...
local typeopt type(alpha)
local wgtopt wgt(pervar)
local stat  alpha
local statF Krippendorff’s $\alpha$
...
// generate list of ads for oranges:oranges comparison
preserve 
by adName, sort: keep if _n==1
keep adName
save dataSets/metaAdNames, replace
restore

// single codingType so we include all possible metacoders
replace codingType = 100 // mTurk meta-coders
...
save auxds/T1_reliabilityTable_data, replace
...
local caption 		Inter-coder reliability (`statF')
local `stat'list 	S`stat'20 S`stat'10 S`stat'_agg_diff S`stat'_agg_pct
local width 		4in
local univ		forsumtable 
local notes		`tablenotes'`tablenotesD'

makeltparts  ``stat'list', caption(`caption') width(`width') notes(`notes')
listtab Var ``stat'list' if `univ', type rstyle(tabular)  head(`"`headline'"') foot(`"`footline'"') appendto(`tlog')

//Summary of meta-coder Gain (Table 3)
local caption 		Reliability gains from aggregation (`statF')
local `stat'list 	S`stat'110 S`stat'100 Smetagain S`stat'120 SmetaVsRA SmetaVsRApct
local width 		5.25in
local univ		forsumtable & !(Var==6)
local notes 		`tablenotes'`tablenotesM'`tablenotesD' 
...
local caption 		Inter-coder reliability by item (`statF')
local `stat'list 	S`stat'20 S`stat'10 S`stat'_agg_diff S`stat'_agg_pct
local width 		4.25in
...
local `stat'list 	S`stat'110 S`stat'100 Smetagain S`stat'120 SmetaVsRA SmetaVsRApct
local width 		5.25in
local univ		!inlist(Var,6,11,12,13,14,23,25) & !(Var==6)
