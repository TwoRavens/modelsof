...
program define mycmd
	...
	if ("`quantile'" ~= "") `anything' `if' `in', quantile(`quantile')
	if ("`quantile'" == "") `anything' `if' `in', cluster(`cluster') `robust' 
	testparm `testvars'
	global k = r(df)
	gen `touse' = e(sample)
	...
	forvalues i = 1/$reps {
		...
		preserve
			...
			capture `anything' `if' `in', cluster(`newcluster') `robust' 
			if (_rc == 0) {
...
svmat double B
aorder
save results\BootstrapKN, replace
