...
program define mycmd
	...
	if ("`cmd'" == "reg" | "`cmd'" == "regress") {
		...
		if ("`cens'" ~= "") `cmd' `dep' `testvars' `anything' [`weight' `exp'] `if' `in', cens(`cens')
		if ("`cens'" == "") `cmd' `dep' `testvars' `anything' [`weight' `exp'] `if' `in', 
		quietly generate Ssample$i = e(sample)
		local newtestvars = ""
		foreach var in `testvars' {
		...
		if ("`cens'" ~= "") `cmd' `dep' `newtestvars' `anything' [`weight' `exp'] `if' `in', cens(`cens')
		if ("`cens'" == "") `cmd' `dep' `newtestvars' `anything' [`weight' `exp'] `if' `in', 
		}
	estimates store M$i
	local i = 0
...
mycmd (Apicshowb_sib Apicraceb_sib Apicobscur_sib Apicshowblack Apicraceb Apicobscur $Amanip) reg Agiving Apicshowb_sib Apicraceb_sib Apicobscur_sib Apicshowblack Apicraceb Apicobscur $Amanip Asubj_iden_blk $Acntrldems Aconst [aw = tweight] if S == 1 & white, robust noconstant
mycmd (Bpicshowb_sib Bpicraceb_sib Bpicobscur_sib Bpicshowblack Bpicraceb Bpicobscur $Bmanipb) reg Bgiving Bpicshowb_sib Bpicraceb_sib Bpicobscur_sib Bpicshowblack Bpicraceb Bpicobscur $Bmanipb Bsubj_iden_blk $Bcntrldems Bconst [aw = tweight] if S == 1 & black, robust noconstant
