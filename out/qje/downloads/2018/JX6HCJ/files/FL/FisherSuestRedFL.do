...
mycmd (Epicshowb_sib Epicshowblack $Emanip) reg Egiving Epicshowb_sib Epicraceb_sib Epicobscur_sib Epicshowblack Epicraceb Epicobscur $Emanip Esubj_iden_blk $Ecntrldems Econst [aw = tweight] if S == 1 & white, robust noconstant
mycmd (Fpicshowb_sib Fpicshowblack $Fmanipb) reg Fgiving Fpicshowb_sib Fpicraceb_sib Fpicobscur_sib Fpicshowblack Fpicraceb Fpicobscur $Fmanipb Fsubj_iden_blk $Fcntrldems Fconst [aw = tweight] if S == 1 & black, robust noconstant
...
foreach var in picshowb_sib picraceb_sib picobscur_sib picshowblack picraceb picobscur $manip {
	quietly replace E`var' = `var'*sqrt($mean5/tweight) if subsample5 == 1
	}
foreach var in picshowb_sib picraceb_sib picobscur_sib picshowblack picraceb picobscur $manipb { 
	quietly replace F`var' = `var'*sqrt($mean6/tweight) if subsample6 == 1
	}
...
foreach var in picshowblack picraceb picobscur $nraudb {
	quietly replace A`var' = `var'*sqrt(mweight/$mean1)*sqrt($m1/tweight) if subsample1 == 1
	}

foreach var in picshowblack picraceb picobscur $nraudb {
	quietly replace B`var' = `var'*sqrt(mweight/$mean2)*sqrt($m2/tweight) if subsample2 == 1
	}

foreach var in picshowblack picraceb picobscur $nraudb {
	quietly replace C`var' = `var'*sqrt(mweight/$mean3)*sqrt($m3/tweight) if subsample3 == 1
	}

foreach var in picshowblack picraceb picobscur $nraudb {
	quietly replace D`var' = `var'*sqrt(mweight/$mean4)*sqrt($m4/tweight) if subsample4 == 1
	}
...
gen N = _n
sort N
save results\FisherSuestredFL, replace
