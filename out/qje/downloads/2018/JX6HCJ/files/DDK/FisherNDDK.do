...
mata ResF = J($reps,6,.); ResD = J($reps,6,.); ResDF = J($reps,6,.); ResB = J($reps,12,.); ResSE = J($reps,12,.)
forvalues c = 1/$reps {
	matrix FF = J(6,3,.)
	matrix BB = J(12,2,.)
	display "`c'"
...
aorder
save results\FisherDDK, replace
