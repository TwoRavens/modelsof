...
if (_rc == 0) {
	capture test arblosnu_3 arblosnu_6 arblosnu_9 scarce parbetslos 
	if (_rc == 0) {
...
capture suest $M, cluster(id)
if (_rc == 0) {
	capture test arbetsloshet arbetsloshetnu arbetsloshetforr duration2 
	if (_rc == 0) {
...
if (_rc == 0) {
	capture test arblosnu_3 arblosnu_6 arblosnu_9 scarce parbetslos 
	if (_rc == 0) {
		mata ResF[`c',11..15] = (`r(p)', `r(drop)', `r(df)', `r(chi2)', 8)
		}
	}
...
capture suest $M, cluster(id)
if (_rc == 0) {
	capture test erfar2 erfar3_5 erfar
	if (_rc == 0) {
...
mata st_store(.,.,ResF)
svmat double F
save results\FisherSuestredER, replace
