...
program define mycmd
	...
	if ("`absorb'" ~= "") `anything' `if' `in', `robust' absorb(`absorb')
	testparm `testvars'
	global k = r(df)
...
preserve
	...
	forvalues i = 1/$N {
		...
		if ("`absorb'" == "") quietly `anything' if M ~= `i', `robust'
		if ("`absorb'" ~= "") quietly `anything' if M ~= `i', `robust' absorb(`absorb')
		matrix BB = J($k,2,.)
		local c = 1
...
svmat double B
aorder
save results\JackknifeGRS, replace
