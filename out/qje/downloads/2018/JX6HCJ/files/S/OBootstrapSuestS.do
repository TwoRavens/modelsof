...
mata test = chi2tail(test[1,2],test[1,1]), cols(bb)-test[1,2], test[1,2], test[1,1], 5 ; st_matrix("test",test); st_matrix("dc",DC); b5 = bb
matrix F = test
matrix DC = dc
...
mata gg = select(G,Select'); bb = select(B,Select); v = ($ncluster/($ncluster-1))*gg*gg'; v = invsym(v); test =  bb*v*bb',sum(rowsum(abs(v)):>0)
mata test = chi2tail(test[1,2],test[1,1]), cols(bb)-test[1,2], test[1,2], test[1,1], 6 ; st_matrix("test",test); st_matrix("dc",DC); b6 = bb
matrix F = F \ test
matrix DC = DC \ dc
...
mata gg = select(G,Select'); bb = select(B,Select); v = ($ncluster/($ncluster-1))*gg*gg'; v = invsym(v); test =  bb*v*bb',sum(rowsum(abs(v)):>0)
mata test = chi2tail(test[1,2],test[1,1]), cols(bb)-test[1,2], test[1,2], test[1,1], 7 ; st_matrix("test",test); st_matrix("dc",DC); b7 = bb
matrix F = F \ test
matrix DC = DC \ dc
...
mata gg = select(G,Select'); bb = select(B,Select); v = ($ncluster/($ncluster-1))*gg*gg'; v = invsym(v); test =  bb*v*bb',sum(rowsum(abs(v)):>0)
mata test = chi2tail(test[1,2],test[1,1]), cols(bb)-test[1,2], test[1,2], test[1,1], 104 ; st_matrix("test",test); st_matrix("dc",DC); b104 = bb
matrix F = F \ test
matrix DC = DC \ dc
...
mata gg = select(G,Select'); bb = select(B,Select); v = ($ncluster/($ncluster-1))*gg*gg'; v = invsym(v); test =  bb*v*bb',sum(rowsum(abs(v)):>0)
mata test = chi2tail(test[1,2],test[1,1]), cols(bb)-test[1,2], test[1,2], test[1,1], 105 ; st_matrix("test",test); st_matrix("dc",DC); b105 = bb
matrix F = F \ test
matrix DC = DC \ dc
...
if (scalar(t) == 0) {
mata gg = select(G,Select'); bb = select(B,Select); v = ($ncluster/($ncluster-1))*gg*gg'; v = invsym(v); test = (bb-b5)*v*(bb-b5)',sum(rowsum(abs(v)):>0)
...
if (scalar(t) == 0) {
mata gg = select(G,Select'); bb = select(B,Select); v = ($ncluster/($ncluster-1))*gg*gg'; v = invsym(v); test = (bb-b6)*v*(bb-b6)',sum(rowsum(abs(v)):>0)
...
if (scalar(t) == 0) {
mata gg = select(G,Select'); bb = select(B,Select); v = ($ncluster/($ncluster-1))*gg*gg'; v = invsym(v); test = (bb-b7)*v*(bb-b7)',sum(rowsum(abs(v)):>0)
...
if (scalar(t) == 0) {
mata gg = select(G,Select'); bb = select(B,Select); v = ($ncluster/($ncluster-1))*gg*gg'; v = invsym(v); test = (bb-b104)*v*(bb-b104)',sum(rowsum(abs(v)):>0)
...
if (scalar(t) == 0) {
mata gg = select(G,Select'); bb = select(B,Select); v = ($ncluster/($ncluster-1))*gg*gg'; v = invsym(v); test = (bb-b105)*v*(bb-b105)',sum(rowsum(abs(v)):>0)
...
svmat double DC
gen N = _n
save results\OBootstrapSuestS, replace
