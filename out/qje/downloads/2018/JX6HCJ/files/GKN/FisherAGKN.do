...
egen Strata = group(tourn cat) if round == 1, label
sort round Strata OriginalOrder
rename OriginalOrder Order
global N = 8801
mata Y = st_data((1,$N),"Group")
generate double U = .
...
forvalues c = 1/$reps {
	...
	quietly sort round Strata U 
	mata st_store((1,$N),"Group",Y)
	sort tourn player round
	quietly replace Group = Group[_n-1] if player == player[_n-1] & round == 2 & tourn == tourn[_n-1]
	quietly recalculate
	capture drop grouping_id
	quietly egen grouping_id = group(Group round)
...
gen N = _n
sort N
save ip\FisherAGKN2, replace

***************************

*Combine files

use ip\FisherAGKN2, clear
...
aorder
save results\FisherAGKN, replace
