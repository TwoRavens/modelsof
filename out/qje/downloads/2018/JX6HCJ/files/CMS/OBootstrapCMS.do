...
program define mycmd
	...
	if ("`iterate'" == "") `anything' `if' `in', `mle' `robust'
	if ("`iterate'" ~= "") `anything' `if' `in', `mle' `robust' iterate(`iterate')
...
program define mycmd1
	...
	gettoken testvars anything: anything, match(match)
	global k = wordcount("`testvars'")
	capture if ("`iterate'" == "") `anything' `if' `in', `mle' `robust'
	capture if ("`iterate'" ~= "") `anything' `if' `in', `mle' `robust' iterate(`iterate')
	if (_rc == 0) {
		capture testparm `testvars'
		if (_rc == 0) {
...
gen Order = _n
sort id_number Order
gen N = 1
gen Dif = (id_number ~= id_number[_n-1])
...
svmat double B
gen N = _n
save ip\OBootstrapCMS1, replace
...
svmat double B
gen N = _n
save ip\OBootstrapCMS2, replace

*********
*********

use ip\OBootstrapCMS2, clear
...
svmat double B
aorder
save results\OBootstrapCMS, replace
