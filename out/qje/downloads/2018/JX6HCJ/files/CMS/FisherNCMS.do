...
program define mycmd1
	...
	if ("`iterate'" ~= "") {
		...
		quietly `anything' `if' `in', `robust' `mle'
		}
	capture testparm `testvars'
	if (_rc == 0) {
...
forvalues c = 1/$reps {
	...
	foreach j in tournament tournament_sabotage {
		quietly replace `j' = `j'[_n-1] if id_number == id_number[_n-1] 
		}
...
svmat double B
gen N = _n
save ip\FisherCMS2, replace

********************************

use ip\FisherCMS2, clear
...
svmat double B
aorder
save results\FisherCMS, replace
