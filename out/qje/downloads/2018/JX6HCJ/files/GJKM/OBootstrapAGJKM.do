*Bootstrapping at level of treatment (actually, probably should do day_num/sessions, which is above game_num/treatment, but want this to be analogous to randomization test)
...
sort game_num Order
gen N = 1
gen Dif = (game_num ~= game_num[_n-1])
...
egen mmm = group(game_num pairid)
quietly replace pairid = mmm if pairid ~= .
...
svmat double B
gen N = _n
save results\OBootstrapAGJKM, replace
