...
program define mycmd
	syntax anything [if] [in] [, cluster(string) robust absorb(string) ll]
	gettoken testvars anything: anything, match(match)
	if ("`absorb'" == "") `anything' `if' `in', cluster(`cluster') `robust' `ll'
...
preserve
	...
	forvalues i = 1/$N {
		...
		if ("`absorb'" == "") quietly `anything' if M ~= `i', cluster(`cluster') `robust' `ll'
...
svmat double B
aorder
save results\JackKnifeCHKL, replace
