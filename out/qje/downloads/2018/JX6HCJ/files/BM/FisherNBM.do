...
mata ResF = J($reps,22,.); ResD = J($reps,22,.); ResDF = J($reps,22,.); ResB = J($reps,140,.); ResSE = J($reps,140,.)
forvalues c = 1/$reps {
	matrix FF = J(22,3,.)
	matrix BB = J(140,2,.)
	display "`c'"
...
mata ResB[`c',1..140] = BB[.,1]'; ResSE[`c',1..140] = BB[.,2]'
