...
program define mycmd
	syntax anything [if] [in] [, ul ll cluster(string)]
	gettoken testvars anything: anything, match(match)
	gettoken cmd anything: anything
	global k = wordcount("`testvars'")
	if ("`cmd'" == "mlogit") global k = 2*$k
	`cmd' `anything' `if' `in', `ul' `ll' cluster(`cluster')
...
program define mycmd1
	...
	global k = wordcount("`testvars'")
	if ("`cmd'" == "mlogit") global k = 2*$k
	capture `cmd' `anything' `if' `in', `ul' `ll' cluster(`cluster')
	if (_rc == 0) { 
		capture testparm `testvars'
		if (_rc == 0 & r(df) == $k) {
...
svmat double B
gen N = _n
save results\OBootstrapAAFGH, replace
