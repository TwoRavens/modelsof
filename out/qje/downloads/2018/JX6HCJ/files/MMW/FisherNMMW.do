...
*All of these xtregs are simple regressions with fixed effects (no re) - but stata's method of calculation involves different finite sample correction 
...
mata ResF = J($reps,51,.); ResD = J($reps,51,.); ResDF = J($reps,51,.); ResB = J($reps,366,.); ResSE = J($reps,366,.)
forvalues c = 1/$reps {
	matrix FF = J(51,3,.)
	matrix BB = J(366,2,.)
	display "`c'"
...
mata ResB[`c',1..366] = BB[.,1]'; ResSE[`c',1..366] = BB[.,2]'
...
gen N = _n
sort N
save results\FisherMMW, replace
