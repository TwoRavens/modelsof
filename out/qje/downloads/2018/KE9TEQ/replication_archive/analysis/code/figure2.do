...
log using $PATHlogs/log_figure2.txt, replace;
...
postfile sca bin d_r_ad_diff using $PATHdata/figure2, replace;
...
forvalues b=1(1)`bins' {;
	...
	quietly {;
		...
		reg r_ptyd_base_own  if BIN==`b';
		local d_r_ad_diff = _b[_con];
	
	
		local mp=(`lb'+(`b'-1)*(`ub'-`lb')/`bins' + `lb'+`b'*(`ub'-`lb')/`bins')/2;
	};
	
	post sca (`mp') (`d_r_ad_diff');
};
postclose sca;


use $PATHdata/figure2, clear;

twoway (scatter d_r_ad_diff  bin if bin<0) (scatter d_r_ad_diff  bin if bin>0)  (lfitci d_r_ad_diff bin if bin<0, ciplot(rline)) (lfitci d_r_ad_diff bin if bin>0, legend(off) ciplot(rline));
graph play "$PATHcode/figure2.grec";
graph save Graph "$PATHlogs/figure2.gph", replace;
graph export "$PATHlogs/figure2.eps", replace;
