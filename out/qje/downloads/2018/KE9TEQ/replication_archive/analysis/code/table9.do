...
global VARs "r_vote r_D_ptyd_base r_ptyd_base_own r_dem ID YEAR BORDER*";
global SampleVars "r_registered r_matched_none r_MD_ptyd_base r_distance r_rep";


log using $PATHlogs/log_table9.txt, replace;
...
forvalues p=1(1)4 {;
	...
	foreach bw in 500 1000 5000 {;
		...
		local s=_se[T_x_dem];
		local N=e(N);
		putexcel_wrapper  , coeff(`c') se(`s') p(`p') bw(`bw') n(`N') file("$PATHlogs/RDresults") sheet("main_rf") opt("modify");
...
forvalues p=1(1)4 {;
	...
	foreach bw in 500 1000 5000 {;
		...
		putexcel_wrapper, coeff(`c') se(`s') p(`p') bw(`bw') n(`N') file("$PATHlogs/RDresults") sheet("main_tsls") opt("modify");
...
foreach bw in 500 1000 5000 {;
		...
		tsls r_vote  (PDIFFdem r_ptyd_base_own = T T_x_dem) , cluster(BORDER) fe(ID) demean;
...
foreach v in r_vote PDIFFdem r_ptyd_base_own T T_x_dem {;
...
foreach bw in 500 1000 5000 {;
		...
		tsls dr_vote  (dPDIFFdem dr_ptyd_base_own = dT dT_x_dem) , cluster(BORDER);
