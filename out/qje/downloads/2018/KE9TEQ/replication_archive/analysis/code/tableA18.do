...
global VARs "r_oth_ptyd_base_own r_D_ptyd_base BORDER* YEAR";
global SampleVars "r_registered r_matched_none r_MD_ptyd_base r_distance";


log using $PATHlogs/log_tableA18.txt, replace;
...
forvalues p=1(1)4 {;
	...
	foreach bw in 500 1000 5000 {;
		...
		local s=_se[T];
		local N=e(N);
		putexcel_wrapper  , coeff(`c') se(`s') p(`p') bw(`bw') n(`N') file("$PATHlogs/RDresults") sheet("disc_cmag_other_party_diff") opt("modify");
