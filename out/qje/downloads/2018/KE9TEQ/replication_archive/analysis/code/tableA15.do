...
global SAMPLE "r_registered==1 & r_matched_none==0 & r_MD_ptyd_base==0  & abs(r_distance)<=5000 & r_Mfemale==0";
global VARs "r_female r_Mfemale r_D_ptyd_base BORDER* YEAR";
global SampleVars "r_registered r_matched_none r_MD_ptyd_base r_distance r_Mfemale";


log using $PATHlogs/log_tableA15.txt, replace;
...
forvalues p=1(1)4 {;
	...
	foreach bw in 500 1000 5000 {;
		...
		local s=_se[T];
		local N=e(N);
		putexcel_wrapper  , coeff(`c') se(`s') p(`p') bw(`bw') n(`N') file("$PATHlogs/RDresults") sheet("disc_female") opt("modify");
