...
global VARs "r_vote r_D_ptyd_base r_ptyd_base_own r_dem BORDER CD_BORDER_10k_PARTY_YEAR";
global SampleVars "r_registered r_matched_none r_MD_ptyd_base r_distance r_rep";


log using $PATHlogs/log_tableA27.txt, replace;
...
forvalues p=1(1)4 {;
	...
	foreach bw in 500 1000 5000 {;
		...
		if abs(r_D_ptyd_base)<=`bw', cluster(BORDER) absorb(CD_BORDER_10k_PARTY_YEAR);

	
		local c=_b[T_x_dem];
		local s=_se[T_x_dem];
		local N=e(N);
		putexcel_wrapper  , coeff(`c') se(`s') p(`p') bw(`bw') n(`N') file("$PATHlogs/RDresults") sheet("CDfe_rf") opt("modify");
...
forvalues p=1(1)4 {;
	...
	foreach bw in 500 1000 5000 {;
		...
		tsls r_vote  (PDIFFdem r_ptyd_base_own = T T_x_dem) distance1-distance`p' dem_x_distance1-dem_x_distance`p' T_x_distance1-T_x_distance`p' T_x_dem_x_distance1-T_x_dem_x_distance`p', cluster(BORDER) fe(CD_BORDER_10k_PARTY_YEAR) demean;
		...
		putexcel_wrapper, coeff(`c') se(`s') p(`p') bw(`bw') n(`N') file("$PATHlogs/RDresults") sheet("CDfe_tsls") opt("modify");
