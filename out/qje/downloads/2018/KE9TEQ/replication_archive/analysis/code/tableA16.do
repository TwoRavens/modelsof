...
global SAMPLE "r_registered==1 & r_matched_none==0 & r_MD_ptyd_base==0  & abs(r_distance)<=5000";
global VARs "r_dem r_rep r_other_none r_D_ptyd_base BORDER* YEAR";
global SampleVars "r_registered r_matched_none r_MD_ptyd_base r_distance";


log using $PATHlogs/log_tableA16.txt, replace;
...
forvalues p=1(1)4 {;
	...
	foreach bw in 500 1000 5000 {;
		...
		local s=_se[T];
		local N=e(N);
		putexcel_wrapper  , coeff(`c') se(`s') p(`p') bw(`bw') n(`N') file("$PATHlogs/RDresults") sheet("disc_party") opt("modify");
...
forvalues p=1(1)4 {;
	...
	foreach bw in 500 1000 5000 {;
		...
		distance1-distance`p' 
		T_x_distance1-T_x_distance`p'
		if abs(r_D_ptyd_base)<=`bw' & (r_dem==1 | r_rep==1), cluster(BORDER) absorb(BORDER_10k_YEAR);
		
		local c=_b[T];
		local s=_se[T];
		local N=e(N);
		putexcel_wrapper  , coeff(`c') se(`s') p(`p') bw(`bw') n(`N') file("$PATHlogs/RDresults") sheet("disc_dem") opt("modify");
