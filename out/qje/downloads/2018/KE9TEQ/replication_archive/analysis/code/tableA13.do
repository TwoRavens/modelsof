...
global SAMPLE "r_registered==1 & r_matched_none==0 & r_MD_ptyd_base==0  & abs(r_distance)<=5000 & r_Mhouse_tenure==0";
global VARs "r_house_tenure r_D_ptyd_base BORDER* YEAR";
global SampleVars "r_registered r_matched_none r_MD_ptyd_base r_distance r_Mhouse_tenure";


log using $PATHlogs/log_tableA13.txt, replace;
...
forvalues p=1(1)4 {;
	...
	foreach bw in 500 1000 5000 {;
		...
		local s=_se[T];
		local N=e(N);
		putexcel_wrapper  , coeff(`c') se(`s') p(`p') bw(`bw') n(`N') file("$PATHlogs/RDresults") sheet("disc_house_tenure") opt("modify");
