...
global VARs "r_female r_Mfemale r_house_tenure r_Mhouse_tenure r_age r_Mage  r_oth_ptyd_base_own r_vote r_D_ptyd_base r_ptyd_base_own r_dem BORDER*";
global SampleVars "r_registered r_matched_none r_MD_ptyd_base r_distance r_rep";


log using $PATHlogs/log_tableA25.txt, replace;
...
forvalues p=1(1)4 {;
	...
	foreach bw in 500 1000 5000 {;
		...
		local s=_se[T_x_dem];
		local N=e(N);
		putexcel_wrapper  , coeff(`c') se(`s') p(`p') bw(`bw') n(`N') file("$PATHlogs/RDresults") sheet("rob_controls_rf") opt("modify");
...
forvalues p=1(1)4 {;
	...
	foreach bw in 500 1000 5000 {;
		...
		tsls r_vote  (PDIFFdem r_ptyd_base_own = T T_x_dem) distance1-distance`p' dem_x_distance1-dem_x_distance`p' T_x_distance1-T_x_distance`p' T_x_dem_x_distance1-T_x_dem_x_distance`p' `controls', cluster(BORDER) fe(BORDER_10k_PARTY_YEAR) demean;
		...
		putexcel_wrapper, coeff(`c') se(`s') p(`p') bw(`bw') n(`N') file("$PATHlogs/RDresults") sheet("rob_controls_tsls") opt("modify");
