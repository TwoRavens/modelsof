...
gen week_T3=week(date_T3)
gen week_T7=week(date_T7)

eststo clear
foreach time in T3 T7{
	foreach var in $AggregateDep_Karthik{
		eststo est_`var'_`time':   reg `var'_`time' ${treatmentlist}  i.DistID $studentcontrol $schoolcontrol $HHcontrol i.week_`time', vce(cluster SchoolID) 
		estadd ysumm
...
esttab  using "$latexcodesfinals/RegTestScores_WeekFE.tex", fragment se ar2 booktabs label b(%9.2fc)se(%9.2fc)nocon nonumber /// 
star(* 0.10 ** 0.05 *** 0.01) ///
...
gen week_T4=week(Date_T4)
gen week_T8=week(date_T8)
preserve
eststo clear
foreach time in  T4{
	foreach var in $AggregateDep_Karthik{
		eststo est_`var'_`time':   reg `var'_`time'  i.GradeID_`time'  i.DistID  $schoolcontrol i.week_`time', vce(cluster SchoolID) 
		estadd scalar p=.
		estadd scalar suma=.
...
foreach time in  T8{
	foreach var in $AggregateDep_Karthik{
		eststo est_`var'_`time':   reg `var'_`time' ${treatmentlist_int} i.GradeID_`time'  i.DistID  $schoolcontrol i.week_`time', vce(cluster SchoolID) 
		estadd ysumm
		test (_b[TreatmentBoth] - _b[TreatmentCOD]=0)
		estadd scalar p=r(p)
...
esttab  using "$latexcodesfinals/RegTestScores_highstakes_timeFE.tex", fragment se ar2 booktabs label b(%9.2fc)se(%9.2fc)nocon nonumber nolines /// 
...
foreach var in $AggregateDep_Karthik{
		...
		reg `var'_T3  i.DistID $studentcontrol $schoolcontrol $HHcontrol i.week_T3, vce(cluster SchoolID)
		...
		reg `var'_T4  i.GradeID_T4  i.DistID  $schoolcontrol i.week_T4, vce(cluster SchoolID) 
...
foreach var in $AggregateDep_Karthik{
		...
		reg `var'_T7  i.DistID $studentcontrol $schoolcontrol $HHcontrol i.week_T7, vce(cluster SchoolID)
		...
		save `file1'
		restore
		reg `var'_T8 $treatmentlist_int i.GradeID_T8  i.DistID  $schoolcontrol i.week_T8, vce(cluster SchoolID) 
...
esttab *_yr1 *_yr2 using "$latexcodesfinals/RegTestScores_Difference_TimeFE.tex", se ar2 label nonumb /// 
