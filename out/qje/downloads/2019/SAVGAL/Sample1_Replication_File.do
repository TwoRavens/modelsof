...
*** Notes: This dofile reproduces the results for Sample 1. 
*******************************************************************************
*******************************************************************************
set more off 
...
*open data set 
use "`path'\Sample1_Data.dta", replace
...
using "`path2'/sample1_table2.tex", replace nonumbers varlabels noobservations pvalues vce(cluster schoolid) ///
...
* label variables to be used as control variables in the analysis
...
using "`path2'\sample1_table3.tex", replace label compress  b(%8.3f) se(2) noconstant 
...
mtitles ("Round 1" "Round 2" "Round 3" "Round 4" "Round 5" "All"  "Failure" "Next") keep(grit)
;
#delimit cr
...
using "`path2'\sample1_table4.tex", replace label compress  b(%8.3f) se(2) noconstant 
...
using "`path2'\sample1_table5.tex", replace label compress  b(%8.3f) se(2) noconstant 
...
using "`path2'\sample1_table6.tex", replace label compress  b(%8.3f) se(2) noconstant 
title("Treatment Effect on Follow-up Grades") star(* 0.1 ** 0.05 *** 0.01) 
nonotes addnotes("Standard errors clustered at the school level." "* p<0.10, ** p<0.05, *** p<0.01") 
s(Pvalue Cmean N r2, labels("Permutation p-value" "Control Mean" N R-Squared) fmt(3 2 0 2)) keep(grit)
mtitles ("Math Grade" "Verbal Grade")  
;
#delimit cr
...
replace attrit=1 if mathscore2==. & task_ability~=. /* students we couldnt find*/
replace attrit=0 if task_ability==. & mathscore2~=. /* students we find but theye were not in original sample*/
logit attrit `controls1', cluster(schoolid)
predict prob1
ge ipw=1/prob1

ritest grit  _b, reps(1000) cluster(schoolid) seed(3) force:  reg mathscore2 grit  `controls1' [aw=ipw]
matrix coeffs=r(p)
scalar b0=coeffs[1,1] 
sum mathscore2 if grit==0
scalar b1=r(mean)
reg mathscore2 grit  `controls1' [aw=ipw] , cluster(schoolid) 
estimates store math_w
estadd scalar Pvalue=b0
estadd scalar Cmean=b1
 
ritest grit  _b, reps(1000) cluster(schoolid) seed(3) force:  reg verbalscore2 grit  `controls1' [aw=ipw] 
matrix coeffs=r(p)
scalar b0=coeffs[1,1]  
sum verbalscore2  if grit==0
scalar b1=r(mean)
reg verbalscore2  grit  `controls1' [aw=ipw] , cluster(schoolid) 
estimates store verbal_w
estadd scalar Pvalue=b0
estadd scalar Cmean=b1

#delimit ;
esttab math_w verbal_w
using "`path2'\sample1_table7.tex", replace label compress  b(%8.3f) se(2) noconstant 
title("Treatment Effect on Follow-up Test Scores") star(* 0.1 ** 0.05 *** 0.01) 
nonotes addnotes("Standard errors clustered at the school level." "* p<0.10, ** p<0.05, *** p<0.01")  
s(Pvalue Cmean N r2, labels("Permutation p-value" "Control Mean" N R-Squared) fmt(3 2 0 2)) keep(grit)
mtitles ("Math Score" "Verbal Score") 
;
#delimit cr
...
using "`path2'\sample1_appendix_tableA17.tex", replace label compress  b(%8.3f) se(2) noconstant 
...
#delimit ;
coefplot 
(math_w , keep(grit)  ciopts(recast(rcap) color(navy))  color(navy) citop color(dknavy)  pstyle(p3))
(verbal_w , keep(grit)  ciopts(recast(rcap) color(navy))  color(navy) citop color(dknavy)  pstyle(p3))
...
graph export "`path2'\sample1_figure1.png", replace
graph save "`path2'\sample1_figure1.pgh", replace
...
twoway (kdensity res2 if grit==0, legend(label(1 "Control")) lcolor(gs0) lpattern(solid)) (kdensity res2 if grit==1, legend(label(2 "Treatment"))  lcolor(gs0) lpattern(dash)) , graphregion(color(white)) xtitle("Residuals") ytitle("Kernel Density")  name(kernel12, replace) title("A: Sample 1")   yscale(range(0 .6))   ylabel(0 (.1) .6)
graph export "`path2'\sample1_figure2.png", replace 
graph save "`path2'\sample1_figure2.pgh", replace 
...
predict res2, residuals
ksmirnov res2, by(grit)
twoway (kdensity res2 if grit==0, legend(label(1 "Control"))  lcolor(gs0) lpattern(solid)) (kdensity res2 if grit==1, legend(label(2 "Treatment"))  lcolor(gs0) lpattern(dash)) , graphregion(color(white)) xtitle("Residuals") ytitle("Kernel Density")  name(kernel22, replace) title("A: Sample 1")   yscale(range(0 .6))   ylabel(0 (.1) .6)
graph export "`path2'\sample1_figure3.png", replace
graph save "`path2'\sample1_figure3.pgh", replace
...
***************************************************************************************
*** Appendix Table A.1: Treatment Effect of Patience Treatment on Choice of Difficult Task
* Outcome variables: Difficult R1, R2, R3, R4, R5, All difficult, After Failure (only for imposed and failed), Next week 
***************************************************************************************
...
preserve 
keep if grit!=1 
ritest patience  _b, reps(1000) cluster(schoolid) seed(3): logit choicer`i' patience `controls'
restore
matrix coeffs=r(p)
scalar b0=coeffs[1,1]  
sum choicer`i' if patience==0 & grit!=1 
scalar b1=r(mean)
logit choicer`i' patience  `controls' if grit!=1 , cluster(schoolid)
margins, dydx(patience  `controls' ) post
estimates store a`i'
estadd scalar Pvalue=b0
estadd scalar Cmean=b1
}
...
preserve 
keep if grit!=1 
ritest patience  _b, reps(1000) cluster(schoolid) seed(3): logit alldiff patience  `controls'
restore
matrix coeffs=r(p)
scalar b0=coeffs[1,1]  
sum alldif if patience==0 & grit!=1 
scalar b1=r(mean)
logit alldiff patience  `controls' if grit!=1 , cluster(schoolid)
margins, dydx(patience `controls') post
estimates store a6
estadd scalar Pvalue=b0
estadd scalar Cmean=b1
...
*Difficult after failure
preserve 
keep if difficult_imposedr1==1 & successr1==0 & grit!=1 
ritest patience  _b, reps(1000) cluster(schoolid) seed(3): logit choicer2 patience  `controls' 
restore 
matrix coeffs=r(p)
scalar b0=coeffs[1,1] 
sum choicer2 if difficult_imposedr1==1 & successr1==0 & patience==0 & grit!=1 
scalar b1=r(mean)
logit choicer2 patience  `controls' if difficult_imposedr1==1 & successr1==0 & grit!=1 , cluster(schoolid)
margins,  dydx(patience  `controls') post
estimates store a7
estadd scalar Pvalue=b0
estadd scalar Cmean=b1
...
preserve 
keep if grit!=1 
ritest patience  _b, reps(1000) cluster(schoolid) seed(3): logit choicev2 patience `controls'  
restore
matrix coeffs=r(p)
scalar b0=coeffs[1,1] 
sum choicev2 if patience==0  & grit!=1 
scalar b1=r(mean)
logit choicev2 patience `controls' if grit!=1, cluster(schoolid)
margins,  dydx(patience `controls' ) post
estimates store a8
estadd scalar Pvalue=b0
estadd scalar Cmean=b1
...
#delimit ;
esttab  a1 a2 a3 a4 a5 a6 a7 a8
using "`path2'\sample1_appendix_tableA1.tex", replace label compress  b(%8.3f) se(2) noconstant 
title("Effect of Patience Treatment on Choice of Difficult Task") star(* 0.1 ** 0.05 *** 0.01) 
nonotes addnotes("Standard errors clustered at the school level. "
"Reported estimates are marginal effects from logit regressions." )
s(Pvalue Cmean N, labels("Permutation p-value" "Control Mean" N) fmt(3 2 0))
...
***************************************************************************************
* Appendix Table A.2: Treatment Effect of Patience Treatment on Success and Payoffs in the Second Visit 
* Outcome variables: Succes imposed, Payoff all, imposed, not imposed. total payoff (1st and 2nd visit)
***************************************************************************************

preserve 
keep if difficult_imposedv2==1 & grit!=1
ritest patience  _b, reps(1000) cluster(schoolid) seed(3):  logit successv2 patience  `controls' 
restore 
matrix coeffs=r(p)
scalar b0=coeffs[1,1]  
sum successv2 if patience==0 & difficult_imposedv2==1 & grit!=1
scalar b1=r(mean)
logit successv2 patience  `controls' if difficult_imposedv2==1 & grit!=1, cluster(schoolid)
margins,  dydx(patience  `controls') post
estimates store successv2
estadd scalar Pvalue=b0
estadd scalar Cmean=b1
...
preserve 
keep if grit!=1 
ritest patience  _b, reps(1000) cluster(schoolid) seed(3):  reg payoffv2 patience  `controls' difficult_imposedv2
restore
matrix coeffs=r(p)
scalar b0=coeffs[1,1]  
sum payoffv2 if patience==0 & grit!=1
scalar b1=r(mean)
reg payoffv2 patience  `controls' difficult_imposedv2 if grit!=1 , cluster(schoolid)
estimates store payoffv2
estadd scalar Pvalue=b0
estadd scalar Cmean=b1
...
preserve
keep if difficult_imposedv2==1 & grit!=1
ritest patience  _b, reps(1000) cluster(schoolid) seed(3):  reg payoffv2 patience  `controls'  
restore
matrix coeffs=r(p)
scalar b0=coeffs[1,1]  
sum payoffv2 if patience==0 & difficult_imposedv2==1 & grit!=1
scalar b1=r(mean)
reg payoffv2 patience  `controls'  if difficult_imposedv2==1 & grit!=1, cluster(schoolid)
estimates store spec3
estadd scalar Pvalue=b0
estadd scalar Cmean=b1

preserve 
keep  if difficult_imposedv2==0 & grit!=1
ritest patience  _b, reps(1000) cluster(schoolid) seed(3):  reg payoffv2 patience  `controls'
restore 
matrix coeffs=r(p)
scalar b0=coeffs[1,1]  
sum payoffv2 if patience==0 & difficult_imposedv2==0 & grit!=1
scalar b1=r(mean)
reg payoffv2 patience  `controls' if difficult_imposedv2==0 & grit!=1, cluster(schoolid)
estimates store spec4
estadd scalar Pvalue=b0
estadd scalar Cmean=b1
...
preserve 
keep if grit!=1 
ritest patience  _b, reps(1000) cluster(schoolid) seed(3):  reg bothpayoffs patience `controls' 
restore
matrix coeffs=r(p)
scalar b0=coeffs[1,1]  
sum bothpayoffs if patience==0 & grit!=1
scalar b1=r(mean)
reg bothpayoffs patience `controls' if grit!=1 , cluster(schoolid)
estimates store spec5
estadd scalar Pvalue=b0
estadd scalar Cmean=b1
...
#delimit ;
esttab  successv2 payoffv2 spec3 spec4 spec5  
using "`path2'\sample1_appendix_tableA2.tex", replace label compress  b(%8.3f) se(2) noconstant 
title("Effect of Patience Treatment on Success and Payoffs in Second Visit") star(* 0.1 ** 0.05 *** 0.01) 
nonotes addnotes("Standard errors clustered at the school level. * p<0.10, ** p<0.05, *** p<0.01"
"Reported estimates in column 1 are marginal effects from logit regression.")
s(Pvalue Cmean N, labels("Permuted p-value" "Control Mean" N) fmt(3 2 0))
mtitles ("Imposed" "All" "Imposed" "Not Imposed" "Total") 
mgroups("Success" "Payoff" "Max", pattern(1 1 0 0 0) prefix(\multicolumn{@span}{c}{) suffix(}) span erepeat(\cmidrule(lr){@span})) 
keep(patience)
;
#delimit cr
...
***************************************************************************************
* Appendix Table A.3: Treatment Effect of Patience Treatment on Test Scores
***************************************************************************************

preserve 
keep if grit!=1 
ritest patience  _b, reps(1000) cluster(schoolid) seed(3) force:  reg mathscore2 patience  `controls1' [aw=ipw]
restore
matrix coeffs=r(p)
scalar b0=coeffs[1,1] 
sum mathscore2 if patience==0 & grit!=1 
scalar b1=r(mean)
reg mathscore2 patience  `controls1' if grit!=1 [aw=ipw] , cluster(schoolid) 
estimates store math_w
estadd scalar Pvalue=b0
estadd scalar Cmean=b1
...
preserve 
keep if grit!=1 
ritest patience  _b, reps(1000) cluster(schoolid) seed(3) force:  reg verbalscore2 patience  `controls1' [aw=ipw] 
restore
matrix coeffs=r(p)
scalar b0=coeffs[1,1]  
sum verbalscore2  if patience==0 & grit!=1 
scalar b1=r(mean)
reg verbalscore2  patience  `controls1' if grit!=1 [aw=ipw] , cluster(schoolid) 
estimates store verbal_w
estadd scalar Pvalue=b0
estadd scalar Cmean=b1
...
#delimit ;
esttab math_w verbal_w
using "`path2'\sample1_appendix_tableA3.tex", replace label compress  b(%8.3f) se(2) noconstant 
title("Effect of Patience Treatment on Follow-up Test Scores") star(* 0.1 ** 0.05 *** 0.01) 
nonotes addnotes("Standard errors clustered at the school level." "* p<0.10, ** p<0.05, *** p<0.01")  
s(Pvalue Cmean N r2, labels("Permutation p-value" "Control Mean" N R-Squared) fmt(3 2 0 2))
mtitles ("Math Score" "Verbal Score") keep(patience)
;
#delimit cr
...
tab grit if mathscore2!=. | verbalscore2!=. 
balancetable grit belief_survey1 grit_survey1 male age raven risk wealth ///
success csize mathscore1 verbalscore1 task_ability  if mathscore2!=. | verbalscore2!=.   ///
using "`path2'/sample1_appendix_tableA4.tex", replace nonumbers varlabels noobservations pvalues vce(cluster schoolid) ///
ctitles("Control Mean (SD)" "Treatment Mean (SD)" "Difference (p-value)") mean(fmt(%9.2f)) sd(fmt(%9.2f) par([ ])) ///
diff(fmt(%9.2f)) pval(fmt(%9.2f))  
...
using "`path2'\sample1_appendix_tableA5.tex", replace label compress  b(%8.3f) se(2) noconstant 
...
#delimit ;
esttab  a1 a2 a3 a4 a5 a6 a7 a8
using "`path2'\sample1_appendix_tableA6.tex", replace label compress  b(%8.3f) se(2) noconstant 
...
#delimit ;
esttab  successv2 payoffv2 spec3 spec4 spec5 spec11 spec21
using "`path2'\sample1_appendix_tableA7.tex", replace label compress  b(%8.3f) se(2) noconstant 
...
#delimit ;
esttab  a1 a2 a3 a4 a5 a6 a7 a8
using "`path2'\sample1_appendix_tableA8.tex", replace label compress  b(%8.3f) se(2) noconstant 
...
#delimit ;
esttab  successv2 payoffv2 spec3 spec4 spec5 spec11 spec21
using "`path2'\sample1_appendix_tableA9.tex", replace label compress  b(%8.3f) se(2) noconstant 
...
#delimit ;
esttab  a1 a2 a3 a4 a5 a6 a7 a8
using "`path2'\sample1_appendix_tableA10.tex", replace label compress  b(%8.3f) se(2) noconstant 
...
#delimit ;
esttab  successv2 payoffv2 spec3 spec4 spec5 spec11 spec21
using "`path2'\sample1_appendix_tableA11.tex", replace label compress  b(%8.3f) se(2) noconstant 
...
***************************************************************************************
ritest grit  _b, reps(1000) cluster(schoolid) seed(3) force:  reg mathscore2 grit   mathscore1_c verbalscore1_c  [aw=ipw]
matrix coeffs=r(p)
scalar b0=coeffs[1,1] 
sum mathscore2 if grit==0
scalar b1=r(mean)
reg mathscore2 grit  mathscore1_c verbalscore1_c  [aw=ipw] , cluster(schoolid) 
estimates store math_w
estadd scalar Pvalue=b0
estadd scalar Cmean=b1
 
ritest grit  _b, reps(1000) cluster(schoolid) seed(3) force:  reg verbalscore2 grit  mathscore1_c verbalscore1_c  [aw=ipw] 
matrix coeffs=r(p)
scalar b0=coeffs[1,1]  
sum verbalscore2  if grit==0
scalar b1=r(mean)
reg verbalscore2  grit  mathscore1_c verbalscore1_c  [aw=ipw] , cluster(schoolid) 
estimates store verbal_w
estadd scalar Pvalue=b0
estadd scalar Cmean=b1
...
#delimit ;
esttab math_w verbal_w
using "`path2'\sample1_appendix_tableA12.tex", replace label compress  b(%8.3f) se(2) noconstant 
...
***************************************************************************************
* Appendix Table A.13: Romano-Wolf p-values
***************************************************************************************
local controls1 male raven_c mathscore1_c verbalscore1_c belief_survey1_c grit_survey1_c csize
 
rwolf mathscore2 verbalscore2 mathgrade2 verbalgrade2, /// 
