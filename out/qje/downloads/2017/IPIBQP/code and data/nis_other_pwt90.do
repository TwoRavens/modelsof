...
Steps:
1.  Prepare miscellaneous data sets and crosswalks
...
*
	...
	* Read and clean PWT 9.0
	import excel "./raw data/pwt/pwt90.xlsx", sheet("Data") firstrow clear
	rename countrycode iso
	
	* Move–Year GDP, relative to US
	gen rgdpwok = rgdpo/emp
...
* PWT 9.0: baseline.
	* Read in NA data first
	import excel "./raw data/pwt/na_data.xlsx", sheet("Data") firstrow clear
	rename countrycode iso
	keep iso v_gdp year
	save ./temp/nagdp.dta, replace
	
	* COLA
	import excel "./raw data/pwt/pwt90.xlsx", sheet("Data") firstrow clear
	rename countrycode iso
	merge 1:1 iso year using ./temp/nagdp.dta
	drop _merge*
	
	gen cola = (v_gdp/xr)/rgdpo
	gen ppp = cola*xr
	rename year incwage_refyear
	
	preserve
	keep incwage_refyear iso cola
	save ./temp/cola.dta, replace
	restore
	
	* Clean exchange rates.  In particular, clean the pre-euro eurozone exchange rates to reflect the old currencies. 
	* Basically, the idea is as follows: keep Malta and the exchange rates from 1999+ as the real, Euro exchange rate
	* For the rest, convert back to the old exchange rates at the fixed conversion factors
	* The resulting database has pre- and post-euro exchange rates. 
	rename currency_unit currency_name
	rename xr xrat
	
	drop if iso == "MLT" & incwage_refyear < 1999
	replace currency_name = "Austrian Schilling" if iso == "AUT"
	replace xrat = xrat*13.7603 if iso == "AUT"
	replace currency_name = "Belgian Franc" if iso == "BEL"
	replace xrat = xrat*40.3388 if iso == "BEL"
	replace currency_name = "Cyprus Pound" if iso == "CYP"
	replace xrat = xrat*0.5852 if iso == "CYP"
	replace currency_name = "German Marc" if iso == "DEU"
	replace xrat = xrat * 1.95583 if iso == "DEU"
	replace currency_name = "Spanish Peseta" if iso == "ESP"
	replace xrat = xrat * 166.386 if iso == "ESP"
	replace currency_name = "Finnish Marka" if iso == "FIN"
	replace xrat = xrat * 5.94573 if iso == "FIN"
	replace currency_name = "French Franc" if iso == "FRA"
	replace xrat = xrat * 6.55957 if iso == "FRA"
	replace currency_name = "Greek Drachma" if iso == "GRC"
	replace xrat = xrat * 340.75 if iso == "GRC"
	replace currency_name = "Irish Punt" if iso == "IRL"
	replace xrat = xrat * 0.787564 if iso == "IRL"
	replace currency_name = "Italian Lira" if iso == "ITA"
	replace xrat = xrat * 1936.27 if iso == "ITA"
	replace currency_name = "Luxembourg Franc" if iso == "LUX"
	replace xrat = xrat * 40.3399 if iso == "LUX"
	replace currency_name = "Dutch Guilder" if iso == "NLD"
	replace xrat = xrat * 2.20371 if iso == "NLD"
	replace currency_name = "Portuguese Escudo" if iso == "PRT"
	replace xrat = xrat * 200.482 if iso == "PRT"
	replace currency_name = "Slovak Koruna" if iso == "SVK"
	replace xrat = xrat*30.1260 if iso == "SVK"
	replace currency_name = "Slovenia Tolar" if iso == "SVN"
	replace xrat = xrat*230.64 if iso == "SVN"
	replace currency_name = "Lithuanian Litas" if iso == "LTU"
	replace xrat = xrat*3.4528 if iso == "LTU"
	replace currency_name = "Estonian Kroon" if iso == "EST"
	replace xrat = xrat*15.6466 if iso == "EST"
	replace currency_name = "Latvian Lats" if iso == "LVA"
	replace xrat = xrat*0.702804 if iso == "LVA"
	drop if iso == "MNE"

	replace currency_name = "CFA Franc" if currency_name == "CFA Franc BEAC"
	replace currency_name = "Turkish Lira" if currency_name == "New Turkish Lira"
	replace currency_name = "Zambian Kwacha" if currency_name == "Kwacha" & iso == "ZMB"
	merge m:1 currency_name using ./temp/devaluation.dta
	
	* Some non-Euro countries have similar complications. Clean them up.
	* Drop non-US countries that use the dollar
	drop if (iso == "ECU" | iso == "LBR" | iso == "PSE" | iso == "SLV" | iso == "TCA" | iso == "VGB" | iso == "ZWE") & currency == 65
	* Drop years with missing Soviet rubles, Czechoslovak korunas. 
	drop if (currency == 16 | currency == 50) & incwage_refyear < 1990
	* Many countries share the East Caribean dollar.  Save St. Vincent and the Grenadines, delete the rest. 
	drop if currency == 84 & iso != "VCT"
	* Many countries share the CFA Franc.  Select one at random, rename, delete the rest. 
	drop if currency == 79 & iso != "GAB"
	keep currency incwage_refyear xrat
	drop if missing(currency) | currency < 0
	duplicates drop 
	save ./temp/xrate.dta, replace
	
* PWT 6.2: Exchange rates for pre–dollarization Zimbabwe and El Salvador.
	* Exchange rates
...
* PWT 5.6: USSR, Yugoslavia, Czechoslovakia, and Myanmar
	* Exchange rates
	...
	drop if currency == 16 & incwage_refyear >= 1990
...
*
	...
	keep iso ynona ya
	gen temp = ynona if iso == "USA"
	egen temp2 = max(temp)
	replace ynona = temp2/ynona
	drop temp*
	gen temp = ya if iso == "USA"
	egen temp2 = max(temp)
	replace ya = temp2/ya
	keep iso ynona ya
...
*
* 3.2: Mean raw wage by occupation and industry
*
