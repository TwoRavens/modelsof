version 8.0
log using N:\AlexB\braith_statecap_jprB.log, replace
set mem 300m
set more off
#delimit ;
use N:\AlexB\Braith_statecap_JPR.dta;

summ allons3 confbord rpc confbord_rpc neighlgdp polity2l polity2sq lgdp96l lnpop post peaceall if year>1959;
corr allons3 confbord rpc confbord_rpc neighlgdp polity2l polity2sq lgdp96l lnpop post peaceall if year>1959;

**graphing interaction for full model 6**;
probit allons3 confbord neighlgdp polity2l polity2sq lgdp96l lnpop post peaceall if year>1959, nolog robust;
probit allons3 confbord rpc neighlgdp polity2l polity2sq lgdp96l lnpop post peaceall if year>1959, nolog robust;
probit allons3 confbord rpc confbord_rpc neighlgdp polity2l polity2sq lgdp96l lnpop post peaceall if year>1959, nolog robust;
...
scalar h_constant=1;
    ...
    generate x_betahat0 = AB_b1*h_confbord
                            + AB_b2*`a'
                            + AB_b3*h_confbord*`a'
				    + AB_b4*h_neighlgdp
                            + AB_b5*h_polity21
    ...
    generate x_betahat1 = AB_b1*(h_confbord+1)
                            + AB_b2*`a'
                            + AB_b3*(h_confbord+1)*`a'
				    + AB_b4*h_neighlgdp
                            + AB_b5*h_polity21
                            + AB_b6*h_polity2sq
                            + AB_b7*h_lgdp96l
...
graph twoway line diff_hat rpc, clwidth(medium) clcolor(blue) clcolor(black)
          ...
          ||,   
            ...
            subtitle(" " "Changing Conflict at Border from 0 to 1" " ", size(3))
            xtitle(Relative Political Capacity, size(3.5)  )
            ytitle("Change in Probability of Conflict", size(3.5))
            xsca(titlegap(2))
            ysca(titlegap(2))
            scheme(s2mono) graphregion(fcolor(white));

graph export  N:\AlexB\braith_statecap_jpr_confbord.eps, replace;

translate @Graph N:\AlexB\braith_statecap_jpr_confbord.wmf, replace;
clear;
log close;
exit;
