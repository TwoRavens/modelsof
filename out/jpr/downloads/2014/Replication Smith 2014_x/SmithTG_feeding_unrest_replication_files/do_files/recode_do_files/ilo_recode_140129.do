...
insheet using "data/raw_data/ILO_foodindices_dld_140129.csv", comma clear

drop codesource src_id d13 notes
...
collapse (first) country codecountry (mean) year id d1 d2 d3 d4 d5 d6 d7 d8 d9 d10 d11 d12, by(c_year)
reshape long d, i(c_year) j(month)
rename d food_price
...
sort id time
gen food_chg = (food_price - l.food_price) / l.food_price
*gen gen_chg = (gen_price - l.gen_price) / l.gen_price
