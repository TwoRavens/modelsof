...
*quietly summarize x2, detail /* Change x2 to variable that you want to appear on X-axis */
quietly summarize lag_xpolity, detail
local min=r(min)
local max=r(max)
local mean=r(mean)
local cen25=r(p25)
...
#delimit ;
	xtfevd	x1x2a_1 SPI_year interact2  lag_xpolity pop_growth lpop lag_lGDP_pc   
			Oil ethfrac lmtnest GDP_initial NAF SSA eastasia westasia MEA LAM NAM trend if year>=1980, 
			invariant(Oil ethfrac lmtnest GDP_initial NAF SSA eastasia westasia MEA LAM NAM) ar1 corr;
#delimit cr
...
gen lag_x2a=l.x2a
 ...
 #delimit ;
	logit	onset_new lag_gr lag_x2a x1x2a pop_growth lpop lag_lGDP_pc Oil ethfrac lmtnest 
			GDP_initial NAF SSA eastasia westasia MEA LAM NAM
			peaceyrs peaceyrs2 peaceyrs3 if year>=1980, vce(boots);
#delimit cr
   ...
   drop x2a x1x2a lag_x2a x1x2a_2 x1x2a_1
   local iter=`iter'+1
   }
...
graph export "Interaction_XPolity_Separate_SPI_FEVD_1980.pdf", replace
