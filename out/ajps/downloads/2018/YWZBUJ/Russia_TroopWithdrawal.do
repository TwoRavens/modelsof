*********************************
** RUSSIA AND TROOP WITHDRAWAL **
*********************************

***April 2018
***This file reproduces the results for the generalizability analysis reported in the manuscript and the web appendix.
***We draw on data collected in an MTURK online survey. In this case, we combine two waves of surveys, from July and October 2016
***Stata/IC 14.2 for Mac (64-bit Intel) was used for data analysis.

use  "Russia_TroopWithdrawal.dta", clear
...
gen rapproche=0
replace rapproche=1 if rapproche_t=="neutral"
replace rapproche=2 if rapproche_t=="cooperate"
*drop conflict cases (i.e. rapproche=0) (This survey included an addional conflict treatment apart from neutral and rapprochement)
drop if rapproche==0 
recode rapproche (1=0) (2=1)
* rapproche= 0 (neutral), 1 (rapprochement)
...
* rapproche
label define mck_mill 1 "Increased" 2 "Decreased" 3 "Maintained" 4 "Don't Know"
...
drop mck_hawk mck_mil mck_par mck_succ
...
*in some cases subjects coded more than one age variable: usually "18" and another. We treat the second age as the correct one. 
egen age_check= rowtotal(age_*)
tab age_check
list age_check age if age_check>1
drop age_*
rename age age_f
gen age= (age_f-18)/62
...
* rescale to with min 0 max 1rename hawk5 hawk5_f
...
local cklist hawk5 intl trust male age educ relig_impt pid7 polint voted12
