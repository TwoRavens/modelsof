...
reg racpolicy racresent1 anger angerres1 fearres1 fear disgust disgusres1 ideology education income1 age south openissuejo1 if baddata2==0;
...
gen conb_anger=b1+b3*MV if _n<18;

gen conb_fear=b1+b4*MV if _n<18;

gen conb_disgust=b1+b7*MV if _n<18;

gen conse_anger=sqrt(varb1+varb3*(MV^2)+2*covb1b3*MV) if _n<18;

gen conse_fear=sqrt(varb1+varb4*(MV^2)+2*covb1b4*MV) if _n<18;

gen conse_disgust=sqrt(varb1+varb7*(MV^2)+2*covb1b7*MV) if _n<18;
...
(line upper_anger MV, lcolor(black) lwidth(thin) lpattern(dash)) ///
(line lower_anger MV, lcolor(black) lwidth(thin) lpattern(dash)) ///
...
xtitle(Symbolic Racism Scale) xtitle(, size(small)) xtitle(, margin(medium)) ///
title(Figure 1: Marginal Effect of Each Emotion on Policy Opinion as Symbolic Racism Changes) title(, size(small)) ///
