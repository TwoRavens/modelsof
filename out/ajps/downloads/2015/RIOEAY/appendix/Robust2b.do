...
gen fed=0
replace fed=1 if cntry=="AT"
replace fed=1 if cntry=="BE"
replace fed=1 if cntry=="DE"
replace fed=1 if cntry=="ES"
replace fed=1 if cntry=="CH"


program m1_est, eclass properties(mi)
bootstrap, clu(region cntry) rep(100): cmp (rincd= c.incdist##c.Rgini age female eduyrs transue transnlf hhmmb lowsup smempl whcol blcol noclass skillshg skillslg skillssp Rforeign Ruerate Rtech Rgdp fed), ind(5) nolr qui
end
...
bootstrap, clu(region cntry) rep(100): cmp (rincd= c.incdist##c.Rgini fear c.fear#c.incdist age female eduyrs transue transnlf hhmmb lowsup smempl whcol blcol noclass skillshg skillslg skillssp Rforeign Ruerate Rtech Rgdp fed) (fear=incdist Rgini age female eduyrs transue transnlf hhmmb lowsup smempl whcol blcol noclass victim Rgdp Rforeign), ind(5 5) nolr qui
end
