...
replace age = age/10
replace Rpopdens = Rpopdens/1000
...
forvalues i = 1/5 {
  foreach v in Rgini Ruerate Rforeign Rgdp Rtech Rpopdens {
    qui sum `v' if _mi_m==`i'
    replace `v' = (`v'-`r(mean)') if _mi_m==`i'
  }
}

* Urban region measure
gen urban = 0
replace urban = 1 if domicil==1
bys region: egen Rurban = mean(urban)

program m1_est, eclass properties(mi)
bootstrap, clu(region cntry) rep(100): cmp (rincd= c.incdist##c.Rgini age female eduyrs transue transnlf hhmmb lowsup smempl whcol blcol noclass skillshg skillslg skillssp Rforeign Ruerate Rtech Rgdp Rurban), ind(5) nolr qui
end
...
bootstrap, clu(region cntry) rep(100): cmp (rincd= c.incdist##c.Rgini fear c.fear#c.incdist age female eduyrs transue transnlf hhmmb lowsup smempl whcol blcol noclass skillshg skillslg skillssp Rforeign Ruerate Rtech Rgdp Rurban) (fear=incdist Rgini age female eduyrs transue transnlf hhmmb lowsup smempl whcol blcol noclass victim Rgdp Rforeign), ind(5 5) nolr qui
end
