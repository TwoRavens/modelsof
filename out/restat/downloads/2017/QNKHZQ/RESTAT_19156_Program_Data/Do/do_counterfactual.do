...
use $data/data_subsidyline_fuelcons_domestic_kei,clear
g obs2008 = 0

* Shift the subsidy line
if `difference'<0 loc shift = -`shift'
loc k = (1000+`shift')/1000
...
use $data/data_obs2008_fuelcons_domestic_kei,clear
...
* NEW
if  `rc'==0 loc difference = -(`de_ABR_RCL' - s_de_ABR)
if  `rc'~=0 loc difference = 0
if `rc'==0  macro list _difference _de_ABR_RCL
...
* Datafile
clear
use $data/data_obs2008_fuelcons_domestic_kei
if "`sample'"=="domestic" keep if n_type==1
if "`sample'"=="kei" keep if n_type==2
...
file open myfile using "TableText/Results_`filename'.txt", write replace
foreach s in `slist' {
