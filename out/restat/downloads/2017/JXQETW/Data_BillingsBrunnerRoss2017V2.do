...
use "RawData\mort_income03.dta", clear
keep year- y
drop tract1 tract0 tract
destring taxpid, generate(T2) force
replace taxpid = "00" + taxpid if T2<=999999
replace taxpid = "0" + taxpid if T2<=9999999 & T2>999999
tostring mort_id, replace
gen day=substr( mortdate,1,2)
destring day, generate(test) force
replace day = substr(day,1,1) if test==.
destring day,generate(test2)

gen month=substr( mortdate,4,3) if test2>9
replace month=substr( mortdate,3,3) if test2<=9
drop test test2
