...
********************************************************************************
* THIS DO FILE GENERATES FIGURE A7

* START LOG SESSION
	log using "LOGS/FIG_A7", replace
...
* DATA PREP ********************************************************************	
	...
	* SAVE TEMPORARY DATA SET 
	save "DATA/TEMP/temp_IV_lwr.dta", replace
	...
	* OPEN PERIOD LOOP 
		...
		* OPEN LAND USE LOOP
			...
			* LOAD TEMPORARY DATA SET
			u "DATA/TEMP/temp_IV_lwr.dta", clear 
			...
			* PERARE FOR LWR LOOPS
			...
			local N = r(N)
				...
				* OPEN LWR LOOP 
				forval num = 1/ `N' {
					* DEFINE BANDWIDTH ADJUSTEMENT FACTOR DUE TO  MULTIPLE COMPONENTS ENTERING WEIGHT (HIGHER VALUES IMPLY MORE SMOOTHING)
					...
					// RUN LOCALLY WEIGHTED IV REGRESSION
					qui ivreg2 lHEIGHT (lLV = ldist_cbd  ldist_lm Dr ) i.CC [w=w] if _n != `num' 
				...
				* SAVE TEMPORARY DATA SET
				save "DATA/TEMP/temp_IV_`USE'_P`PER'", replace
...
* COMBINE DATA SETS WITH HEIGHT ELASTICITIES BY USE AND PERIOD
	u DATA/TEMP/temp_IV_COMMERICIAL_P1, clear
		append using DATA/TEMP/temp_IV_RESIDENTIAL_P1
		append using DATA/TEMP/temp_IV_OTHER_P1		
		append using DATA/TEMP/temp_IV_COMMERICIAL_P2
		append using DATA/TEMP/temp_IV_RESIDENTIAL_P2
		append using DATA/TEMP/temp_IV_OTHER_P2
		append using DATA/TEMP/temp_IV_COMMERICIAL_P3
		append using DATA/TEMP/temp_IV_RESIDENTIAL_P3
		append using DATA/TEMP/temp_IV_OTHER_P3
	...
	* SAVE FIGURE 8	
		graph export "FIGS/FIG_A7.png", width(2400) height(1800) replace			
