clear
*-------------------------------USER INPUT!!!------------------------------------
local yearlong = "2005"		// USER INPUT!! Year must contain exactly 4 digits (eg "2005")
...
merge 1:m cuid using membdcuid`yearshort'`hhtype', keepusing(age wagexm cu_code1) force 
keep if _merge==3
...
reshape wide age cu_code1 wagexm, i(cuid) j(count)
...
gen children=0
gen adults=0
gen refpersonwage=wagexm1 if cu_code11=="1"
gen spousewage=wagexm1 if cu_code11=="2"
...
replace refpersonwage=wagexm`type' if cu_code1`type'=="1"
replace spousewage=wagexm`type' if cu_code1`type'=="2"
drop age`type' wagexm`type' cu_code1`type'
}
