...
* county FE;
      areg `yvar' `treat' `fixed' `cb60' `reis' ripc [aweight=nbirths], absorb(stcntyfips) cluster(stcntyfips);
...
* county FE, RIPC, state x linear time;
      areg `yvar' `treat' linear_fe_state* `fixed' `cb60' `reis' ripc [aweight=nbirths],
...
* county FE, RIPC, county x linear time;
    areg `yvar' `treat' linear_fe_cnty* `fixed' `reis' ripc [aweight=nbirths],
      absorb(stcntyfips) cluster(stcntyfips);
    egen samplepop = sum(nbirths*e(sample));
    local samplepop = samplepop;
    drop samplepop;
