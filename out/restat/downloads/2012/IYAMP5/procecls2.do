...
sort childid;
merge childid using classmeasures2;
tab _m;
drop _m;
...
hist p1costpk if p1costpk ~= 0 & (hsprim == 1 | cbcarepk == 1) & p1costpk < 800, bin(80) saving(procecls2_costpkhscb, asis replace);
hist p1costpk if p1costpk ~= 0 & (hsprim == 1) & p1costpk < 800, bin(80) saving(procecls2_costpkhs, asis replace);
hist p1costpk if p1costpk ~= 0 & (cbcarepk == 1) & p1costpk < 800, bin(80) saving(procecls2_costpkcb, asis replace);
...
for var b?age b?teach b?enjoy b?mkdiff b?yrsch b?yrspre b?yrskin b?yrsfst b?yrs2t5 b?yrs6pl b?typcer b?hghstd b?tgend: replace X = . if X < 0;
for var b1age b1teach b1enjoy b1mkdiff b1yrsch b1yrspre b1yrskin b1yrsfst b1yrs2t5 b1yrs6pl b1typcer b1hghstd b1tgend \ 
	var b2age b2teach b2enjoy b2mkdiff b2yrsch b2yrspre b2yrskin b2yrsfst b2yrs2t5 b2yrs6pl b2typcer b2hghstd b2tgend: 
	replace X = Y if X == . & Y ~= .;
*age*;
ren b1age aget1;
*love teaching*;
...
***teacher's chars - grades 1 & 3 (added 4/29/08)***;
*age*;
ren b4age aget4;
*love teaching*;
gen lovteach4 = 1 if b4teach >= 4 & b4enjoy >= 4 & b4mkdiff >= 4 & b4teach ~= . & b4enjoy ~= . & b4mkdiff ~= .;
replace lovteach4 = 0 if b4teach < 4 | b4enjoy < 4 | b4mkdiff < 4;
label var lovteach4 "enjoy (b4enjoy), make difference (b4mkdiff), choose teach again (b4teach)";
drop b4teach b4enjoy b4mkdiff;
gen lovteach5 = 1 if b5teach >= 4 & b5enjoy >= 4 & b5mkdiff >= 4 & b5teach ~= . & b5enjoy ~= . & b5mkdiff ~= .;
replace lovteach5 = 0 if b5teach < 4 | b5enjoy < 4 | b5mkdiff < 4;
label var lovteach5 "enjoy (b5enjoy), make difference (b5mkdiff), choose teach again (b5teach)";
drop b5teach b5enjoy b5mkdiff;
*gender - suppressed*;
drop b4tgend;
drop b5tgend;
*race/ethnicity*;
gen twhite4 = 1 if b4race5 == 1 & b4hisp == 2;
replace twhite4 = 0 if b4race5 == 2 | b4hisp == 1;
label var twhite4 "teacher white, non-hispanic";
drop b4hisp b4race*;
gen twhite5 = 1 if b5race5 == 1;
replace twhite5 = 0 if b5race5 == 2;
label var twhite5 "teacher white (hispanic suppressed)";
drop b5race*;
*educ*;
gen tba4 = b4hghstd == 1 | b4hghstd == 2;
gen tgs4 = b4hghstd == 3;
gen tma4 = b4hghstd >= 4;
for var tba4 tgs4 tma4: replace X = . if b4hghstd == .;
*drop b4hghstd;
label var tba4 "teacher received BA as highest degree";
label var tgs4 "teacher had some grad school";
label var tma4 "teacher received MA or higher as highest degree";
gen tba5 = b5hghstd == 1;
gen tgs5 = b5hghstd == 2;
gen tma5 = b5hghstd >= 3;
for var tba5 tgs5 tma5: replace X = . if b5hghstd == .;
*drop b5hghstd;
label var tba5 "teacher received BA as highest degree";
label var tgs5 "teacher had some grad school";
label var tma5 "teacher received MA or higher as highest degree";
*experience*;
ren b4yrsch tyrsch4;
ren b5yrsch tyrsch5;
gen totyrstchkp4 = b4yrskin + b4yrsfst + b4yrs2t5 + b4yrs6pl;
label var totyrstchkp4 "total years teaching kindergarten plus";
ren b4yrsfst tyrsfst4;
*cert - dummy up in reg program*;
ren b4typcer tcert4;
ren b5typcer tcert5;
...
***class chars - grades 1 & 3***;
for var a4totag a5totag: replace X = . if X == -9;
ren a4totag class4size;
ren a5totag class5size;
