*This file takes the original 1980 data and creates three datasets: one for wives (with husbands vars matched and child vars added), one for single women (with child vars added) and one for single men

clear
set mem 4g
global path "U:\user3\klp27\Blau and Kahn\Immigration\"

use "$path\Census 1980 data", clear
...
save "$path\Census 1980 data", replace
...
*The following creates a matched wife-husband dataset (Census 1980 Wife data.dta)
...
*The next section creates a temporary dataset (full child) containing the number of male children at each age up to 18 and 19plus, excluding children who are married
local i=0
while `i'<19 {
	...
	keep if age==`i' & sex==1 & marst~=1 & nchild==0 & parent~=0
      ...
      *The child's mother's line number might be up to 26, hence the next line
...
*The next section adds the number of female children at each age level to the temporary dataset
local i=0
while `i'<19 {
	use  "$path\interim", clear
...
*The next section merges in the number of children to their fathers (only those children who don't have a mother in the household)
use "$path\Census 1980 data", clear
keep if sex==1 & marst==1 & age>17 & age<66
merge year serial pernum using "$path\full child"
drop if _merge==2
keep year serial pernum slwt perwt momloc momrule poploc poprule sploc sprule relate age sex race marst nchild racgen00 racdet00 racamind racasian racblk racpacis racwht racother racnum bpl citizen yrimmig yrsusa1 yrsusa2 speakeng hispan school educrec educ99 gradeatt schltype empstat labforce occ ind wkswork1 uhrswork inctot incwage incinvst migplac5 migmet5 migtype5 migtyp00 migcity5 migpuma migpumas movedin nchm0-nchf19plus
for var pernum slwt perwt momloc momrule poploc poprule sploc sprule relate age sex race marst nchild racgen00 racdet00 racamind racasian racblk racpacis racwht racother racnum bpl citizen yrimmig yrsusa1 yrsusa2 speakeng hispan school educrec educ99 gradeatt schltype empstat labforce occ ind wkswork1 uhrswork inctot incwage incinvst migplac5 migmet5 migtype5 migtyp00 migcity5 migpuma migpumas movedin nchm0-nchf19plus: rename X spX
rename sppernum sploc
sort year serial sploc
save "$path\Census 1980 Husb data", replace

*The next section merges in the number of children to their mothers and then merges in the fathers' child data
use "$path\Census 1980 data", clear
...
joinby year serial sploc using "$path\Census 1980 Husb data"
*merge year serial sploc using "$path\Census 1980 Husb data"
*keep if _merge==3
*drop _merge

*The next section adds the mothers' and fathers' child data at each sex/age level
...
erase "$path\Census 1980 Husb data.dta"
save "$path\Census 1980 Wife data", replace


*The following creates a dataset for single men (with no child data)
use "$path\Census 1980 data", clear
keep if sex==1 & marst~=1 & age>17 & age<66
/*To use the next two lines, I would have to alter the child dataset code so that I only used poploc as the parent locator
merge year serial pernum using "$path\full child"
drop if _merge==2*/
keep year datanum serial numprec subsamp hhwt region stateicp statefip metro city gq gqtype vacancy pernum slwt perwt momloc momrule poploc poprule sploc sprule relate age sex race marst nchild racgen00 racdet00 racamind racasian racblk racpacis racwht racother racnum bpl citizen yrimmig yrsusa1 yrsusa2 speakeng hispan school educrec educ99 gradeatt schltype empstat labforce occ ind wkswork1 uhrswork inctot incwage incinvst migplac5 migmet5 migtype5 migtyp00 migcity5 migpuma migpumas movedin

sort year serial sploc
save "$path\Census 1980 Single Men data", replace
...
*Once again, the next section creates the child data for each mother and merges the files into the mothers' dataset
local i=0
while `i'<19 {
	use  "$path\interim", clear
...
sort year serial sploc
erase "$path\full child.dta"
save "$path\Census 1980 Single Women data", replace
