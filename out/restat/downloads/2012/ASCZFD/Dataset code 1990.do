...
use "$path\Census 1990 data", clear
...
save "$path\Census 1990 data", replace
...
*The following creates a matched wife-husband dataset (Census 1990 Wife data.dta)
...
drop if _merge==2
keep year serial pernum slwt perwt momloc momrule poploc poprule sploc sprule relate age sex race marst nchild bpl citizen yrimmig yrsusa2 hispan school educrec educ99 schltype empstat labforce occ ind wkswork1 uhrswork inctot incwage incinvst migplac5 migmet5 migtype5 migcity5 migpuma migpumas movedin incbus incfarm classwkd qwkswork quhrswor qincbus qincfarm qincinvs qincwage qbpl qyrimm speakeng occmo nchm0-nchf19plus
for var pernum slwt perwt momloc momrule poploc poprule sploc sprule relate age sex race marst nchild bpl citizen yrimmig yrsusa2 hispan school educrec educ99 schltype empstat labforce occ ind wkswork1 uhrswork inctot incwage incinvst migplac5 migmet5 migtype5 migcity5 migpuma migpumas movedin incbus incfarm classwkd qwkswork quhrswor qincbus qincfarm qincinvs qincwage qbpl qyrimm speakeng occmo nchm0-nchf19plus: rename X spX
rename sppernum sploc
sort year serial sploc
save "$path\Census 1990 Husb data", replace
...
merge year serial pernum using "$path\full child"
drop if _merge==2
keep year datanum serial numprec subsamp hhwt region stateicp statefip city gq gqtype vacancy pernum slwt perwt momloc momrule poploc poprule sploc sprule relate age sex race marst nchild bpl citizen yrimmig yrsusa2 hispan school educrec educ99 schltype empstat labforce occ ind wkswork1 uhrswork inctot incwage incinvst migplac5 migmet5 migtype5 migcity5 migpuma migpumas movedin incbus incfarm classwkd qwkswork quhrswor qincbus qincfarm qincinvs qincwage qbpl qyrimm speakeng occmo nchm0-nchf19plus
sort year serial sploc

joinby year serial sploc using "$path\Census 1990 Husb data"
*merge year serial sploc using "$path\Census 1990 Husb data"
*keep if _merge==3
*drop _merge
...
erase "$path\Census 1990 Husb data.dta"
save "$path\Census 1990 Wife data", replace
...
merge year serial pernum using "$path\full child"
drop if _merge==2*/
keep year datanum serial numprec subsamp hhwt region stateicp statefip city gq gqtype vacancy pernum slwt perwt momloc momrule poploc poprule sploc sprule relate age sex race marst nchild bpl citizen yrimmig yrsusa2 hispan school educrec educ99 schltype empstat labforce occ ind wkswork1 uhrswork inctot incwage incinvst migplac5 migmet5 migtype5 migcity5 migpuma migpumas movedin incbus incfarm classwkd qwkswork quhrswor qincbus qincfarm qincinvs qincwage qbpl qyrimm speakeng occmo

sort year serial sploc
save "$path\Census 1990 Single Men data", replace
...
sort year serial sploc
erase "$path\full child.dta"
save "$path\Census 1990 Single Women data", replace
