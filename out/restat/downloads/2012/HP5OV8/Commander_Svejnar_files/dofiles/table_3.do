...
log using "$logs/table3.txt", text replace
...
outreg2 $ENDOGEN numcomp3 $EXOGEN using "$tables/table3", replace bdec(3) se aster(se) bracket label
tests, column(model1) file(`m1') df(1) insts($INSTRB) exog(numcomp3 $EXOGEN) cluster(clstr)

local i = 2 
foreach var of global EXOGEN{ 
  xi: ivreg2 logS ($ENDOGEN = $INSTRB) numcomp3 `var', ffirst robust cluster(clstr) 
  outreg2 $ENDOGEN numcomp3 `var' using "$tables/table3", append bdec(3) se aster(se) bracket label
  tests, column(model`i') file(`m`i'') df(1) insts($INSTRB) exog(numcomp3 `var') cluster(clstr)
  local i = `i'+1
} 

xi: ivreg2 logS ($ENDOGEN = $INSTRB) numcomp3 BI_constrqALL15, ffirst robust cluster(clstr) 
outreg2 $ENDOGEN numcomp3 BI_constrqALL15 using "$tables/table3", append bdec(3) se aster(se) bracket label
tests, column(model11) file(`m11') df(1) insts($INSTRB) exog(numcomp3 BI_constrqALL15) cluster(clstr)

xi: ivreg2 logS ($ENDOGEN = $INSTRB) numcomp3 $EXOGEN, ffirst robust cluster(clstr)
outreg2 $ENDOGEN numcomp3 $EXOGEN using "$tables/table3", append bdec(3) se aster(se) bracket label
tests, column(model12) file(`m12') df(1) insts($INSTRB) exog(numcomp3 $EXOGEN) cluster(clstr)
...
outsheet using "$tables/table3_tests.csv", replace comma
restore
...
xi: regress logS $ENDOGEN $EXOGEN numcomp3, robust cluster(clstr) 
outreg2 $ENDOGEN numcomp3 $EXOGEN using "$tables/tableA3", replace bdec(3) se aster(se) bracket label 


foreach var of global EXOGEN{ 
xi: regress logS $ENDOGEN numcomp3 `var', robust cluster(clstr) 
outreg2 $ENDOGEN numcomp3 `var' using "$tables/tableA3", append bdec(3) se aster(se) bracket label      
}

xi: regress logS $ENDOGEN numcomp3 BI_constrqALL15, robust cluster(clstr) 
outreg2 $ENDOGEN numcomp3 BI_constrqALL15 using "$tables/tableA3", append bdec(3) se aster(se) bracket label      

xi: regress logS $ENDOGEN $EXOGEN numcomp3, robust cluster(clstr) 
outreg2 $ENDOGEN numcomp3 $EXOGEN using "$tables/tableA3", append bdec(3) se aster(se) bracket label sortvar($ENDOGEN numcomp3 $EXOGEN)
