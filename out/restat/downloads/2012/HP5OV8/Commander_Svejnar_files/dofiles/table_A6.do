...
xi: ivreg2 logS ($ENDOGEN = $INSTRB) numcomp3 rm $EXOGEN $DUMMIES, ffirst robust cluster(clstr) 
outreg2 $ENDOGEN numcomp3 rm $EXOGEN using "$tables/tableA6", replace bdec(3) se aster(se) bracket label
tests, column(model1) file(`m1') df(1) insts($INSTRB) exog(numcomp3 rm $EXOGEN $DUMMIES) cluster(clstr)

local i = 2 
foreach var of global EXOGEN{ 
  xi: ivreg2 logS ($ENDOGEN = $INSTRB) numcomp3 rm `var' $DUMMIES, ffirst robust cluster(clstr) 
  outreg2 $ENDOGEN numcomp3 rm `var' using "$tables/tableA6", append bdec(3) se aster(se) bracket label
  tests, column(model`i') file(`m`i'') df(1) insts($INSTRB) exog(numcomp3 rm `var' $DUMMIES) cluster(clstr)

local i = `i'+1
} 

xi: ivreg2 logS ($ENDOGEN = $INSTRB) numcomp3 rm $DUMMIES, ffirst robust cluster(clstr) 
outreg2 $ENDOGEN numcomp3 rm using "$tables/tableA6", append bdec(3) se aster(se) bracket label
tests, column(model11) file(`m11') df(1) insts($INSTRB) exog(numcomp3 rm $DUMMIES) cluster(clstr)

xi: ivreg2 logS ($ENDOGEN = $INSTRB) numcomp3 rm $EXOGEN $DUMMIES, ffirst robust cluster(clstr) 
outreg2 $ENDOGEN numcomp3 rm $EXOGEN using "$tables/tableA6", append bdec(3) se aster(se) bracket label
...
outsheet using "$tables/tableA6_tests.csv", replace comma
restore
