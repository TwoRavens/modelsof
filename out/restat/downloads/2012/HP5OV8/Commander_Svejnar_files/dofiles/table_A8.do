...
log using "$logs/tableA8.txt", text replace
...
keep if perfqJ !=. & perfqG !=. & perfqH !=.

egen clstr = group(year country prod2DIG sizeb)
...
outreg2 $PARS using $tables/tableA8, replace bdec(3) se bracket label
...
outreg2 $PARS logExp using $tables/tableA8, append bdec(3) se bracket label  
...
outreg2 $PARS numcomp3 using $tables/tableA8, append bdec(3) se bracket label
...
outreg2 $PARS logExp numcomp3 using $tables/tableA8, append bdec(3) se bracket label   
...
outreg2 $PARS2 using $tables/tableA8, append bdec(3) se bracket label  
tests, column(model5) file(`m5') df(4) insts($INSTRB) exog($DUMMIES) cluster(clstr)




xi: ivreg2 logS ($ENDOGEN = $INSTRB) $DUMMIES, ffirst robust cluster(clstr) 
outreg2 $ENDOGEN using $tables/tableA8, append bdec(3) se bracket label
...
outreg2 $PARS2 numcomp3 using $tables/tableA8, append bdec(3) se bracket label  
...
outreg2 $ENDOGEN numcomp3 using $tables/tableA8, append bdec(3) se bracket label sortvar(logE logA logExp numcomp3 owner_new1 owner_new2 owner_new5)

tests, column(model8) file(`m8') df(5) insts($INSTRB) exog(numcomp3 $DUMMIES) cluster(clstr)
...
outsheet using $tables/tableA8_tests.csv, replace comma
