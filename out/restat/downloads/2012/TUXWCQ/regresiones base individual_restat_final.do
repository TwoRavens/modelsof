...
clear
set mem 600m
global a "C:\Documents and Settings\rvidarte\Mis documentos\resultados bbs\restat cambios\modificacion rosa"
...
**** aventas y all_ventas solo es restriccion para tablas 1-3, no para 5 y 7
gen aventas = lventasc~=. & lvgood~=. & lvnormal~=. & lvbad~=. & lgap~=. 
bys idclient: egen all_ventas=min(aventas)

* Creating missing constraints for dd estimates *

foreach X of varlist lventasc lvgood lvnormal lgap lvbad work wk_out ruc fixed rec_sales rec_take2 local sources credit /*
...
*OLS Without Covariates (clustering)
areg `var' treatment sucurs if all_ventas==1 & post==1 & idclient~=. & miss_`var'==1, absorb(cofficer_lb) cl(idbank) 
*OLS With Covariates (clustering)
areg `var' treatment sucurs `covariates' if all_ventas==1 & post==1 & idclient~=. & miss_`var'==1, absorb(cofficer_lb) cl(idbank)  
log close
}
...
display "*T-C DIFFERENCE AT FOLLOW UP"
*OLS Without Covariates (clustering)
areg `var' treatment sucurs if post==1 & idclient~=. & miss_`var'==1, absorb(cofficer_lb) cl(idbank) 
*OLS With Covariates (clustering)
areg `var' treatment sucurs `covariates' if post==1 & idclient~=. & miss_`var'==1, absorb(cofficer_lb) cl(idbank)  
log close
}
...
*T-C DIFFERENCE AT FOLLOW UP
*OLS Without Covariates (clustering)
areg rec_take2 treatment if cod_sucurs==1 & post==1 & idclient~=. & miss_rec_take2==1, absorb(cofficer_lb) cl(idbank)	
*OLS With Covariates (clustering)
areg rec_take2 treatment `covariates' if cod_sucurs==1 & post==1 & idclient~=. & miss_rec_take2==1, absorb(cofficer_lb) cl(idbank)	
log close
...
*T-C DIFFERENCE AT FOLLOW UP
*OLS Without Covariates (clustering)
areg sources treatment if cod_sucurs==5 & post==1 & idclient~=. & miss_sources==1, absorb(cofficer_lb) cl(idbank)
*OLS With Covariates (clustering)
areg sources treatment `covariates' if cod_sucurs==5 & post==1 & idclient~=. & miss_sources==1, absorb(cofficer_lb) cl(idbank)
log close
...
for var lventasc lvgood lvnormal lvbad work wk_out : gen p_X=X if treatment==0 & post==0 
foreach var of varlist p_lventasc p_lvgood p_lvnormal p_lvbad p_work p_wk_out  {
sum `var'
scalar `var'_mean=r(mean) 
scalar `var'_sd=r(sd) 
}
drop p_lventasc p_lvgood p_lvnormal p_lvbad p_work p_wk_out

for X in any lventasc lvgood lvnormal lvbad work wk_out : gen kl_X=(X-p_X_mean)/p_X_sd
egen restr_kling1=rownonmiss(kl_lventasc kl_lvgood kl_lvnormal kl_lvbad kl_work kl_wk_out )
gen kling1=(kl_lventasc+kl_lvgood+kl_lvnormal+kl_lvbad+kl_work+kl_wk_out)/6 if restr_kling1==6
...
log using "$a\results\table5_bbss_results2.log", replace

***T-C DIFFERENCE AT FOLLOW UP
*Base model without Covariates
areg kling1 treatment sucurs if post==1 & miss_kling1==1 & idclient~=., absorb(cofficer_lb) cl(idbank)  
*Base model with Covariates
areg kling1 treatment sucurs `covariates' if post==1 & miss_kling1==1 & idclient~=., absorb(cofficer_lb) cl(idbank)  
*Ex-ante attitude towards training
areg kling1 treatment treatint1 high_interest1 sucurs missing2_new if miss_kling1==1 & post==1 & idclient~=., absorb(cofficer_lb) cl(idbank)  
*Education
areg kling1 treatment treduc ed_mas sucurs missing3_new if miss_kling1==1 & post==1 & idclient~=., absorb(cofficer_lb) cl(idbank)  
*Business size
areg kling1 treatment trsize2 size2 sucurs missing5_new if miss_kling1==1 & post==1 & idclient~=., absorb(cofficer_lb) cl(idbank)  
...
*Ex-ante attitude towards training
areg kling1 post postint1 treatment treatint1 high_interest1 dd ddint1 sucurs missing2_new if miss_kling1==1, absorb(cofficer_lb) cl(idbank)  
*Education
...
log using "$a\results\table5_bbss_practices2.log", replace
...
display "*** T-C DIFFERENCE ESTIMATE AT FOLLOW UP"

display "*Base model without Covariates"
areg `var' treatment sucurs if post==1 & miss_`var'==1 & idclient~=., absorb(cofficer_lb) cl(idbank)
display "*Base model with Covariates"
areg `var' treatment sucurs `covariates' if post==1 & miss_`var'==1 & idclient~=., absorb(cofficer_lb) cl(idbank)
display "*Ex-ante attitude towards training"
areg `var' treatment treatint1 high_interest1 sucurs missing2_new if post==1 & miss_`var'==1 & idclient~=., absorb(cofficer_lb) cl(idbank)
display "*Education"
areg `var' treatment treduc ed_mas sucurs missing3_new if post==1 & miss_`var'==1 & idclient~=., absorb(cofficer_lb) cl(idbank)
display "*Business size"
areg `var' treatment trsize2 size2 sucurs missing5_new if post==1 & miss_`var'==1 & idclient~=., absorb(cofficer_lb) cl(idbank)
...
areg `var' post treatment dd sucurs `covariates' if miss_`var'==1, absorb(cofficer_lb) cl(idbank) 
display "*Ex-ante attitude towards training"
areg `var' post postint1 treatment treatint1 high_interest1 dd ddint1 sucurs missing2_new if miss_`var'==1, absorb(cofficer_lb) cl(idbank)
display "*Education"
