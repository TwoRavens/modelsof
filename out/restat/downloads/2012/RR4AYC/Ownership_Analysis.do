// This program runs the Ownership analysis for Cass county (County ID 17).
// Input data: Base.dta, cnty_parcel_qq.dta
// Log file: Ownership_Analysis.txt
// This code is based on the county admin analysis code, but differs in that the clustering is 
// at the township level and that county dummies are dropped (County 17 dummy variable remains and acts as a constant term).
...
use Data\Base.dta if county == "017"
drop DCnty2-DCnty12

/* Merge County Parcel Info */
sort county section_id q qq
merge county section_id q qq using data\cnty_parcel_qq.dta, nokeep
drop _merge

gen name5= substr(itrim(trim(name)),1,5)
drop if name5 == ""
drop name parcel_id
...
global expvar
foreach tempvar of varlist slope_r - name5 {
...
rename o_township_id township_id
rename o_q q
...
foreach type in a1 a2 b {
	...
	sort county section_id q qq
	merge county section_id q qq using `base_name'
...
foreach x in a1 a2 b {
	gen D_`x' = 0
	replace D_`x' = 1 if o_name5 != `x'_name5
}


global expvar
foreach tempvar of varlist o_slope_r - o_DCnty1 {
...
log using logs\Ownership_Analysis.txt, replace text
...
program nlscaleagP
	...
	quietly {
		regress crop_$cropnum x_* , noconstant /* We run regression without constant term because we have a county dummy for Cass county that is turned on for every observation. */
...
foreach cropnumloc in $croplist {
	...
	regress resid gA gB gD x_*, noconstant cluster(township_id)
