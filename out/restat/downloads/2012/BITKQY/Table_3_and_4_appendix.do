...
log using Table_3_and_4_appendix.log,replace 

use data_Table_3_and_4_appendix.dta
...
outreg using Table_3_appendix.txt, replace se bdec(4) coefastr 3aster bracket 

reg sdrcnd rhs $state
outreg using Table_3_appendix.txt, append se bdec(4) coefastr 3aster bracket 
...
estout Table_3* using Table3_appendix.out, style(tex)  keep (rhs u_rate _cons)  cells(b(star fmt(%9.4f)) se(par))  ///
...
outreg using Table_4_appendix.txt, replace se bdec(4) coefastr 3aster bracket 

reg mrcnd rhs $state
outreg using Table_4_appendix.txt, append se bdec(4) coefastr 3aster bracket 
...
estout Table_4* using Table4_appendix.out, style(tex)  keep (rhs u_rate _cons)  cells(b(star fmt(%9.4f)) se(par))  ///
starlevels(* 0.10 ** 0.05 *** 0.01) legend label collabels(, none) prehead(\begin{tabular}{l*{@M}{r}}) postfoot(\end{tabular}) replace
