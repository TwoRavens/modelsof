...
keep id year convert* startup* dob_2 freel redl othecon recent_immig migrant grade structural_* nonstructural_* outofdist_*  lep atrisk gifted speced schoolid
...
*LIMIT TO POST 1993 SINCE S IS 1 BY DEFINITION IN 1993
drop if year < 1994

*DUE TO S BEING DEFINED AS 1 IN GRADE 1, LIMIT TO GRADES 2 & HIGHER
keep if grade >= 2 & grade != .

drop if convert_zoned_l == . | startup_elem_middle_l == . | startup_high_l == .
...
xi: xtreg s inelig_adj convert_zoned_l startup_elem_middle startup_high_l freelunch_2-freelunch_11 redlunch_2-redlunch_11 othecon_2-othecon_11 recent_immigrant migrant grade_2-grade_11 year_2-year_13  gradeyear_1-gradeyear_132 gradeyear_133-gradeyear_143, fe cluster(schoolid) nonest


xi: xtlogit s inelig_adj convert_zoned_l startup_elem_middle startup_high_l freelunch_2-freelunch_11 redlunch_2-redlunch_11 othecon_2-othecon_11 recent_immigrant migrant grade_1-grade_11 year_2-year_13 gradeyear_1-gradeyear_132 gradeyear_133-gradeyear_143, fe


predict pred, xb
compress

save /work/s/simberman/lusd/charter1/bandwidth_b.dta, replace
