/* This program estimates all of the main and appendix tables in the paper using the NAEP */
...
*CREATE LOG;
log using "${logs}create_tables_NAEP.log", replace;
...
foreach q in all q80_1 q80_3 {;
	...
	estimates store t9_nounion_`q'_nc;

	reghdfe naep_score_std post_trend upm3_post_trend 
		if `q'==1 & great_recession==0 & KS_KY_MO_TX_MI_WY==0, absorb(ncesid region#year sub_gr) cluster(ncesid state_yr);
	estimates store t9_union_`q'_nc;
...
foreach q in all q80_1 q80_3 {;
	reghdfe naep_score_std post_trend 
		enr_trend inc_trend btel 
		if `q'==1 & great_recession==0 & KS_KY_MO_TX_MI_WY==0, absorb(ncesid region#year sub_gr) cluster(ncesid state_yr);
	estimates store t9_nounion_`q'_bc;

	reghdfe naep_score_std post_trend upm3_post_trend 
		enr_trend enr_trend_upm3 inc_trend inc_trend_upm3 btel
		if `q'==1 & great_recession==0 & KS_KY_MO_TX_MI_WY==0, absorb(ncesid region#year sub_gr) cluster(ncesid state_yr);
	estimates store t9_union_`q'_bc;
...
estout t9_* using "${tabs}tab9_naep_6_6_18.txt", replace
