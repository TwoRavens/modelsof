capture program drop estimate_tau_IDA_50_cluster
capture program drop estimate_tau_50_collapsed
mata: mata clear


program define estimate_tau_IDA_50_cluster, eclass
	...
	*save point_pre_collapse.dta, replace
	...
	estimate_tau_50_collapsed freq, max_epdf_size(`max_epdf_size') bandwidth(`bandwidth') graph(1) dummies(`dummies') max_size_for_naive(`max_size_for_naive')
	...
	*save point_post_collapse.dta, replace
	...
	if `n_b' > 0 & `n_clusters' > 1 {
		...
		forvalues b=1/`n_b' {
			...
			estimate_tau_50_collapsed freq, max_epdf_size(`max_epdf_size') bandwidth(`bandwidth') graph(0) dummies(`dummies') max_size_for_naive(`max_size_for_naive')
...
program define estimate_tau_50_collapsed, eclass
	...
	* naive estimator
	* -----------------------
	qui reg log_kdens log_workers gte_50 n_worker_1 if total_workers <= `max_size_for_naive'
	...
	* naive theta and tau
	...
	matrix delta_mat = coeffs[1, "gte_50"]
	...
	* ---------------------
	...
	if `graph' {
		reg log_kdens log_workers gte_50 `dummy_var_names'
	}
	else {
		qui reg log_kdens log_workers gte_50 `dummy_var_names'
