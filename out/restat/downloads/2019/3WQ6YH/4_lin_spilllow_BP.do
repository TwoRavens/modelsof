...
*
* This file relates to figure 4(c)
*-------------------------------------------------------------------------------
...
* globals
global expe "lin_spilllow_BP" // name of the experiment


* dataset
use "${path}panel_Q.dta", clear
keep stateid statename quarter gdprea govrea low1 // keep only analyzed variables
...
forvalues j=0/$hor { // rename the variables
	qui summ low1 if stateid==1 //create star variables which equal gov.spending/output of partner state(s)
	mkmat g`j' if stateid==r(mean), matrix(gstar`j')
	qui summ low1 if stateid==1
	mkmat y`j' if stateid==r(mean), matrix(ystar`j')
	forvalues i=2/51 {
		qui summ low1 if stateid==`i'
...
graph combine fig_mult fig_multstar, xcommon ycommon c(2) imargin(small) title("(c) least important trade partner",size(medsmall)) ///
