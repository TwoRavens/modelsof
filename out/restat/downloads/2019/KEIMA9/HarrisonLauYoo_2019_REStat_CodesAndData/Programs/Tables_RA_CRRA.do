//-----------------------------------------------------------------------------------------------------
// Post-estimation results for EUT model without selection + Fechner errors
//-----------------------------------------------------------------------------------------------------

	// set globals for latent preference parameters
	global parameters	"rra1 rra2"
	global labels		"rra1 rra2"
	
	// set global to indicate which of the above parameters are normal (instead of log-normal)
	global normals		"rra1 rra2"
...
log using Tables\Fechner\TableF3_EUT_NoSel.log, replace text 
//-------------------------------------------
// EUT with CRRA utility and Fechner errors
//-------------------------------------------
	//----------------------------------------
	// No correction for endogenous selection
	//----------------------------------------
	est use  Results/nrep100_MSL_CRRA_RC_12_V1.ster
	qui est store RAW
	ereturn list
	estimates replay	
	
	MSLdisplayN, parameters($parameters) labels($labels) normals($normals) obshet($obshet)
...
//-----------------------------------------------------------------------------------------------------
// Post-estimation results for EUT model with selection + Fechner errors
//-----------------------------------------------------------------------------------------------------

	// set globals for latent preference parameters
	global parameters	"sel1 sel2 rra1 rra2"
	global labels		"sel1 sel2 rra1 rra2"
...
log using Tables\Fechner\TableF1_EUT_Sel.log, replace text 
//-------------------------------------------
// EUT with CRRA utility and Fechner errors
//-------------------------------------------
	//----------------------------------------
	...
	//----------------------------------------
	est use  Results/nrep100_MSL_CRRA_RC_H12_V1.ster
	qui est store RAW
	ereturn list
	estimates replay	
	...
	test (_b[cor_UN_sel1_sel2] = 0) (_b[cor_UN_sel1_rra1] = 0) (_b[cor_UN_sel1_rra2] = 0)  
	...
	test (_b[cor_UN_sel1_sel2] = 0) (_b[cor_UN_sel1_rra1] = 0) (_b[cor_UN_sel1_rra2] = 0) ///
		 (_b[cor_UN_sel2_rra1] = 0) (_b[cor_UN_sel2_rra2] = 0) 
