...
// Set the figure scheme and font
set scheme s1color
...
//===========================================================================
// Fig 1. Population Distribution of Relative Risk Aversion (rra) under EUT
//===========================================================================
set seed 12345
capture drop ERR
...
twoway (line y_rra1_no_low RRA1_no_low, sort lpattern(dash) lwidth(medthin)) ///
	   ...
	   ), saving(Graphs\Context\tmp2, replace)
...
twoway (line y_rra1_yes_low RRA1_yes_low, sort lpattern(dash) lwidth(medthin)) ///
	   ...
	   plotregion(margin(zero)) scheme(s1color) ///
	   ), saving(Graphs\Context\tmp1, replace) 

// Combine the two RRA figures into Figure 1
graph combine Graphs\Context\tmp1.gph Graphs\Context\tmp2.gph, col(1) title("{bf:Figure 1: Population Distributions of}" "{bf:Risk Aversion under EUT}") saving(Graphs\Context\figure1, replace) imargin(small) xcommon ycommon scheme(s1color) xsize(1) ysize(1.5)
graph export Graphs\Context\figure1.png, replace
...
//========================================================================
// Fig 2a. Population Distribution of Relative Risk Aversion under RDU
//========================================================================
set seed 12345
capture drop ERR
foreach v in RRA1_no y_rra1_no RRA2_no y_rra2_no RRA1_yes y_rra1_yes RRA2_yes y_rra2_yes {
...
est use Results\nrep`=$nrep'_MSL_CRRA1PRE_RC_12_C1
gen RRA1_no_low   = [rra1]_cons + sqrt([c11]_cons^2)*ERR
gen RRA2_no_low   = [rra2]_cons + sqrt([c21]_cons^2 + [c22]_cons^2)*ERR
gen RRA1_no_high  = [rra1]_cons + [rra1]RAhigh + sqrt([c11]_cons^2)*ERR
...
est use Results\nrep`=$nrep'_MSL_CRRA1PRE_RC_H12_C1
gen RRA1_yes_low  = [rra1]_cons + sqrt([c31]_cons^2 + [c32]_cons^2 + [c33]_cons^2)*ERR
gen RRA2_yes_low  = [rra2]_cons + sqrt([c41]_cons^2 + [c42]_cons^2 + [c43]_cons^2 + [c44]_cons^2)*ERR
gen RRA1_yes_high = [rra1]_cons + [rra1]RAhigh + sqrt([c31]_cons^2 + [c32]_cons^2 + [c33]_cons^2)*ERR
gen RRA2_yes_high = [rra2]_cons + [rra2]RAhigh + sqrt([c41]_cons^2 + [c42]_cons^2 + [c43]_cons^2 + [c44]_cons^2)*ERR
...
// Combine the two RRA figures into Figure 2a.
graph combine Graphs\Context\tmp1.gph Graphs\Context\tmp2.gph, col(1) title("{bf:2a: Risk Aversion Due to}" "{bf:Utility Curvature}") saving(Graphs\Context\figure2a, replace) imargin(small) xcommon ycommon scheme(s1color) xsize(1) ysize(1.5)
graph export Graphs\Context\figure2a.png, replace

//=============================================================================
// Fig 2b. Population Distribution of Weighting Parameter (phi) under RDU
//=============================================================================
set seed 12345
capture drop ERR
...
// Combine two shape-parameter figures into Figure 2b
graph combine Graphs\Context\tmp1.gph Graphs\Context\tmp2.gph, col(1) title("{bf:2b: Risk Aversion Due to}" "{bf:Probability Weighting}") saving(Graphs\Context\figure2b, replace) imargin(small) xcommon ycommon scheme(s1color) xsize(1) ysize(1.5)
graph export Graphs\Context\figure2b.png, replace

//===========================================================
// Fig 2. Population Distribution of Risk Aversion under RDU
//===========================================================
// Combine figure 2a and 2b into figure 2
graph combine Graphs\Context\figure2a.gph Graphs\Context\figure2b.gph, col(2) title("{bf:Figure 2: Population Distributions of}" "{bf:Risk Aversion under RDU}") saving(Graphs\Context\figure2, replace) imargin(small) xcommon ycommon scheme(s1color) xsize(1.5) ysize(1)
graph export Graphs\Context\figure2.png, replace
...
//=============================================================================
// Fig E1. Population Distribution of Risk Aversion for Men & Women under RDU
//=============================================================================
// Population distribution of RRA
set seed 12345
capture drop ERR
foreach v in RRA1_no RRA2_no RRA1_yes RRA2_yes y_rra1_no y_rra2_no y_rra1_yes y_rra2_yes {
...
est use Results\nrep`=$nrep'_MSL_CRRA1PRE_RC_12_C3
...
est use Results\nrep`=$nrep'_MSL_CRRA1PRE_RC_H12_C3
...
set seed 12345
capture drop ERR
foreach v in LNphi1_no LNphi2_no LNphi1_yes LNphi2_yes phi1_no phi2_no phi1_yes phi2_yes y_phi1_no y_phi2_no y_phi1_yes y_phi2_yes {
	foreach stake in men women {
		capture drop `v'_`stake'
	}
}
...
// Combine RRA and weighting parameter figures into Figure E1
graph combine Graphs\Context\tmp1.gph Graphs\Context\tmp2.gph, col(1) title("{bf:Figure E1: Population Distributions of}" "{bf:Risk Aversion for Men}" "{bf:and Women under RDU}") saving(Graphs\Context\figureE1, replace) imargin(small) scheme(s1color) xsize(1) ysize(1.5)
graph export Graphs\Context\figureE1.png, replace

// erase temporary graphs
erase Graphs\Context\tmp1.gph
erase Graphs\Context\tmp2.gph

//=================================================================================================
// Fig 3. Decision Weights and Certainty Equivalents of Lotteries in Decision Tasks under RDU
//=================================================================================================

// Preliminaries: a series of variables that are needed to generate figures 3a and 3b
set seed 12345
capture drop ERR
...
//---------------------------------------------------------------
// Figure 3a, define globals to customize the figure's appearance
//---------------------------------------------------------------
...
//---------------------------------------------------------
	...
	graph combine Mdw_1L.gph Mdw_1H.gph, imargin(vsmall) col(2) title("{bf:First Wave}") saving(Figure_3a_1_mean, replace) 
	...
	graph combine Mdw_2L.gph Mdw_2H.gph, imargin(vsmall) col(2) title("{bf:Second Wave}") saving(Figure_3a_2_mean, replace) 

	graph combine Figure_3a_1_mean.gph Figure_3a_2_mean.gph, imargin(vsmall) rows(2) xsize(1) ysize(1) title("{bf:3a: Decision Weights under RDU}") subtitle("{bf:Mean Parameter Values}") saving(Graphs\Context\figure3a, replace) 
	graph export Graphs\Context\figure3a.png, replace
...
//------------------------------------------------------------------------
// Figure 3b, percent, define globals to customize the figure's appearance
//------------------------------------------------------------------------
* style options for control, no control and 45-degree line plots 
	global control_style   ", lwidth(thick) lpattern(solid)"
	global nocontrol_style ", lwidth(thick) lpattern(dash)"
...
//-----------------------------------------------------
// Figure 3b, fourth prize set and mean values, percent
//-----------------------------------------------------
	use Mrr4A_1, clear
	gen neutral = 0
	...
	graph combine Mrr4A_1.gph Mrr4B_1.gph, imargin(vsmall) col(2) title("{bf:First Wave}") saving(Figure_3b_41_mean_pct, replace)
	...
	graph combine Mrr4A_2.gph Mrr4B_2.gph, imargin(vsmall) col(2) title("{bf:Second Wave}") saving(Figure_3b_42_mean_pct, replace)

	graph combine Figure_3b_41_mean_pct.gph Figure_3b_42_mean_pct.gph, imargin(vsmall) row(2) xsize(1) ysize(1) title("{bf:3b: Relative Risk Premia under RDU}") subtitle("{bf:Mean Parameter Values}") saving(Graphs\Context\figure3b, replace)
	graph export Graphs\Context\figure3b.png, replace

//===========================================================
// Fig 3. Decision Weights and Relative Risk Premia under RDU
//===========================================================
// Combine figure 3a and 3b into figure 3
graph combine Graphs\Context\figure3a.gph Graphs\Context\figure3b.gph, col(2) title("{bf:Figure 3: Decision Weights and Relative}" "{bf:Risk Premia under RDU}") saving(Graphs\Context\figure3, replace) imargin(small) xcommon ycommon scheme(s1color) xsize(1.5) ysize(1)
graph export Graphs\Context\figure3.png, replace	
	
	
//------------------------------------------------	
// Clean up hard disk before moving forward
//------------------------------------------------
* erase intermediate output files for figure 3a
	erase tmp.dta
	erase tmp_data.dta
	erase tmpCE.dta
	forvalues k = 1/2 {
		...
		erase Mdw_`k'L.gph
		erase Mdw_`k'H.gph
		erase Figure_3a_`k'_mean.gph
		erase dw_`k'L.dta
		erase dw_`k'H.dta
	}	
	
* erase intermediate output files for figure 3b
	forvalues j = 1/4 {
		forvalues k = 1/2 {
			...
			capture erase Figure_3b_`j'`k'_mean_pct.gph
