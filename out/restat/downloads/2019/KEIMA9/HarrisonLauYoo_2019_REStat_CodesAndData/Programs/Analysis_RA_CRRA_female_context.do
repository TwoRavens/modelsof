...
// Contextual Utility.
// Observed heterogeneity w.r.t. gender instead of treatment indicators.
//==============================================================================
...
keep if (sample_1 == 1 & repeat == 0)| (sample_2 == 1 & repeat == 1)
	...
	//-----------------------------------------------------------
	est use Results/ML_CRRA_12_C0.ster
	ml model lf ML_CRRA_12() (rra1: $LHS_RA repeat = female) (rra2: female) (LNmuRA: female), ///
				   cluster(${id}) missing maximize diff technique(bfgs 5 nr 5) continue
	ml display
	est save Results/ML_CRRA_12_C3.ster, replace
	...
	est use Results/nrep50_MSL_CRRA_RC_12_C0.ster

	ml model gf0 MSL_CRRA_RC_12() (rra1: ${LHS_RA} repeat = female) (rra2: female) (LNmuRA: female) (c11: ) (c21: ) (c22: ), ///
					cluster(${id}) missing maximize diff technique(bfgs 5 nr 5) continue
	ml display	
	est save Results/nrep${nrep}_MSL_CRRA_RC_12_C3.ster		
	...
	est use Results/nrep50_MSL_CRRA_RC_12_C3.ster
	matrix start = e(b)
...
est save Results/probit_sample_2.ster, replace
	...
	est use Results/nrep50_MSL_CRRA_RC_H12_C0.ster

	ml model gf0 MSL_CRRA_RC_H12() (sel1: ${LHS_RA} sample_1 sample_2 repeat first = ${RHS_1}) ///
								   (sel2: = ${RHS_2A}) (rra1: = female) (rra2: = female) (LNmuRA: female) (c21: ) (c31: ) (c32: ) (c33: ) (c41: ) (c42: ) (c43: ) (c44: ), ///
								   cluster(${id}) missing maximize diff technique(bfgs 5 nr 5) continue
	ml display
	est save Results/nrep${nrep}_MSL_CRRA_RC_H12_C3.ster
	...
	est use Results/nrep50_MSL_CRRA_RC_H12_C3.ster
	matrix start = e(b)
