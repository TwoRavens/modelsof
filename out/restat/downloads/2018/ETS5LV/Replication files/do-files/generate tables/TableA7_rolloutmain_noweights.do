...
sum nsuicidespc if never_treat==1 & year>2011
...
********************************************************************************
	...
	xtreg nsuicidespc treat_post   $year if year>=2005, fe cluster(kecid)
...
********************************************************************************
	...
	xtreg nsuicidespc treat_post     $year if year>=2005 & rctsample!=1, fe cluster(kabuid)
...
sum nsuicidespc if never_treat==1 &rctsample!=1& year>2011
...
********************************************************************************
	...
	xtreg nsuicidespc treat_post    $year if year>=2000, fe cluster(kabuid)
...
********************************************************************************
	 ...
	 hdfe nsuicidespc  treat_post if year>=2000, absorb(i.kecid2000##c.year $year) clear  keepvars(kabuid never_treat) clustervars(kabuid)
...
loc rowlabels " "Dependent variable: Suicide rate " "\cline{1-1} " "Treatment" " " " " "\hline Subdistrict FE" "Time FE" "Cluster subdistrict" "Exclude RCT sample" "Include pre-treatment periods" "Subdistrict trends" " " "\hline Control mean (2011 \& 2014)" "N" "Census waves" "
loc rowstats ""
...
esttab * using "$tables/TableA7_rolloutmain_noweights.tex", replace cells(none) booktabs nonotes nomtitle compress ///
