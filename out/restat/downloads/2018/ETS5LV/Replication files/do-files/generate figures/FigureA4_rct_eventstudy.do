...
use "$data/podes_pkhrollout.dta", clear
 ...
 set scheme  lean2 
	...
	gen treat_pkhrct_2000 = treat_pkhrct*year2000
	...
	xtreg nsuicidespc treat_pkhrct_2000 treat_pkhrct_2003 treat_pkhrct_2011 treat_pkhrct_2014  $year [aw=pop_sizebaseline] , fe cluster(kecid)
		
	
gen treatmenteffect=0 if year==2005
gen standarderror=0 if year==2005


	 

replace treatmenteffect=_b[treat_pkhrct_2000] if  year==2000
replace standarderror=_se[treat_pkhrct_2000] if  year==2000
 ...
 gen zero = 0
   ...
   twoway    (connected treatmenteffect  year)  (rcap hiz_avg lowz_avg year)  , ///
	...
	legend(order(1 "Treatment Effect" 2 "95 % CI") ring(1) position(6) row(1)) ///
	 ...
	 graph export "$figures/FigureA4_rct_eventstudy.pdf", as(pdf) replace
