...
sum year                         
drop _merge
replace TBill_3M = max(0, TBill_3M) if TBill_3M<. // egen TBill_3M0 = rowmax(YTM_3M DGS3MO_TBi DTB3_Tbil) //egen TBill_3M = rowmin(YTM_3M DGS3MO_TBi DTB3_Tbil)
...
foreach var of varlist DFII5 DFII10 T5YIE T10YIE T5YIFR TBill_3M {  // ED (Eurodollar futures), ES (E-mini S&P500 futures)
g `var'0 = L.`var'
...
local ylist "DFII5_exr DFII10_exr T5YIE_exr T10YIE_exr T5YIFR_exr TBill_3M0_exr" //S_P500_p_e NASDAQ_p_e DAX_p_e FTSE100_p_e CAC40_p_e IBEX_p_e SMI_p_e
...
outreg2 using "$path0T2\\Tips_ally.xls", `esave`es''
reg `vy' i.(window3), `varR' //
outreg2 using "$path0T2\Tips_noy.xls", `esave`es''
qreg `vy' i.(window3) dissent_lag0 `xvars', `varR' //
outreg2 using "$path0T2\\Tips_QR_ally.xls", `esave`es''
qreg `vy' i.(window3), `varR' // 
outreg2 using "$path0T2\Tips_QR_noy.xls", `esave`es''
local es=2
}
drop if window1==.
local ylist "DFII5_exr DFII10_exr T5YIE_exr T10YIE_exr T5YIFR_exr TBill_3M0_exr"
...
save "$path0T2\tips_data_sig_pub.dta", replace
keep window1 sig_*
export excel using "$path0T2\tips_data_sig_pub.xls", firstrow(variables) nolabel replace
keep window1 *d0
export excel using "$path0T2\tips_data_sig_d0_pub.xls", firstrow(variables) nolabel replace
use "$path0T2\tips_data_sig_pub.dta", clear
keep window1 sig_*
keep window1 *dun
export excel using "$path0T2\tips_data_sig_dun_pub.xls", firstrow(variables) nolabel replace
clear 
