* robots - skill shares 
		...
		// specifications
	local outcomes "h_sh_HS h_sh_MS h_sh_LS"
	local groups `""High" "Middle" "Low""' 
		
	local robvar "ch_rob_pctile"
	
	#delimit ;
	local xvars1 ""; local xvars2 "i.num_c lw0 kl0"; local xvars3 "`xvars2' ch_kl ch_kitsh";
		
	local iv1 "\`iv'"; local iv2 "\`iv'"; local iv3 "\`iv'";
	
	local cntr_basic_1 ""; local cntr_basic_2 "$\checkmark$"; local cntr_basic_3 "$\checkmark$";
	local cntr_cap_1 ""; local cntr_cap_2 ""; local cntr_cap_3 "$\checkmark$";
	...
	local tabopt_btm `"`tabopt' append nonum keep("") stats(cntr_basic cntr_cap N, l("Country trends \& controls" "Changes in other capital" "Observations")) posthead(\`header')"'
...
u if ind_rob!="" using "$maindataset", clear
	...
	#delimit cr
	foreach case in ols hours_replace repl_arms arms bottom {
		local list_`case' ""
		foreach y in `outcomes' {
			forval i = 1/`imax' {
