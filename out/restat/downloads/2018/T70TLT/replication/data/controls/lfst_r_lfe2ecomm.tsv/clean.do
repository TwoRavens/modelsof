...
import delimited  lfst_r_lfe2ecomm.tsv,  varnames(1) clear
split unitc_workgeotime, generate(_) parse(,)
rename _1 unit
rename _2 work
rename _3 geotime
...
keep yr* code_ccaa
replace yr_2004 = substr(yr_2004, 1, 4)
replace yr_2004 = "" if yr_2004==": c"
destring yr_2004, replace
reshape long yr_ , j(year) i(code_ccaa)
rename yr_ interstate
ipolate interstate year if code_ccaa == 4, gen(interstatecommute)
replace interstate = interstatecommute if code_ccaa == 4 & year == 2008
drop interstatecommute
save interstate.dta, replace
