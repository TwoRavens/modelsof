...
void ezop(real matrix QF0, real matrix QG0, real matrix QF1, real matrix QG1, real matrix SF0, real matrix SG0, real matrix SF1, real matrix SG1, real scalar alpha, real scalar beta, real scalar nF, real scalar nG, real scalar q, string scalar TstatEZOP, string scalar PvalueEZOP)
	{
	real matrix R, RG, Tstat, Pvalue, DQTEa, OQTEa, T1QTEa, T2QTEa, DQTEb, OQTEb, T1QTEb, T2QTEb, pvalue
	real scalar nFG, n, nF0, nG0, nF1, nG1
	string scalar fail
	Tstat = J(1,3,.)
	Pvalue = J(1,3,.)
	R = (I(q), -I(q))
	...
	if (alpha == 0 | beta == 0) {
		...
		if (alpha==1 & beta==1) {
			test_eqDD(QF0, QG0, SF0, SG0, QF1, QG1, SF1, SG1, nF0, nG0, nF1, nG1, q, RG, "DQTEa", "OQTEa", "T1QTEa")
			test_eqDD(QG0, QF0, SG0, SF0, QG1, QF1, SG1, SF1, nG0, nF0, nG1, nF1, q, RG, "DQTEb", "OQTEb", "T1QTEb")
			}
		else if (alpha==1 & beta==-1) {
			test_eqDD(QF0, QG0, SF0, SG0, QG1, QF1, SG1, SF1, nF0, nG0, nG1, nF1, q, RG, "DQTEa", "OQTEa", "T1QTEa")
			test_eqDD(QG0, QF0, SG0, SF0, QF1, QG1, SF1, SG1, nG0, nF0, nF1, nG1, q, RG, "DQTEb", "OQTEb", "T1QTEb")
			}
		else if (alpha==-1 & beta==1) {
			test_eqDD(QG0, QF0, SG0, SF0, QF1, QG1, SF1, SG1, nG0, nF0, nF1, nG1, q, RG, "DQTEa", "OQTEa", "T1QTEa")
			test_eqDD(QF0, QG0, SF0, SG0, QG1, QF1, SG1, SF1, nF0, nG0, nG1, nF1, q, RG, "DQTEb", "OQTEb", "T1QTEb")
			}
		else {
			test_eqDD(QG0, QF0, SG0, SF0, QG1, QF1, SG1, SF1, nG0, nF0, nG1, nF1, q, RG, "DQTEa", "OQTEa", "T1QTEa")
			test_eqDD(QF0, QG0, SF0, SG0, QF1, QG1, SF1, SG1, nF0, nG0, nF1, nG1, q, RG, "DQTEb", "OQTEb", "T1QTEb")
			}
		n = 2*nFG
		...
		test(st_matrix("OQTEb"), st_matrix("DQTEb"), n, "T2QTEb")
		Tstat[1,3] = st_matrix("T2QTEb")
		pvalue(st_matrix("OQTEb"),Tstat[1,2])
		Pvalue[1,3] = st_matrix("pvalue")
		}
...
void test_eqDD(real matrix Qa, real matrix Qb, real matrix Sa, real matrix Sb, real matrix Qc, real matrix Qd, real matrix Sc, real matrix Sd, real scalar na, real scalar nb, real scalar nc, real scalar nd, real scalar q, real matrix R, string scalar DD, string scalar  ODD, string scalar T1DD)
	{
	...
	Lambda = Qa \ Qb \ Qc \ Qd
	Gamma = R*Lambda
	O1 = ( ((ntot/na)*Sa), J(q,q,0) \ J(q,q,0) , ((ntot/nb)*Sb) )
	O2 = ( ((ntot/nc)*Sc), J(q,q,0) \ J(q,q,0) , ((ntot/nd)*Sd) )
	O3 = ( O1, J(2*q,2*q,0) \ J(2*q,2*q,0) , O2 )
	O = R*(O3)*(R')
	T1 = ntot*(Gamma')*invsym(O)*Gamma
	st_matrix(T1DD, T1)
	st_matrix(DD, Gamma)
	st_matrix(ODD, O)
	}
