...
cd "yourresultpath"
log using table10_young, replace
...
areg growthrate young younghigh old oldhigh , absorb(state_industry) vce(cluster state_fips)
...
gen employment_2007_young=employment_2007*young
save temp, replace
...
replace g_counterfactual=ghat+DD_estimate if young==1 & high==1
...
collapse (sum) employment_2009_hat employment_2009_counterfactual employment_2007  employment_2007_young
...
export excel using "C:\Users\siemer_m\Documents\Results_new\Aggregate\aggregate_impact_DD_young_KFS.xlsx", firstrow(variables) sheet(`small_shock') sheetreplace
use temp, clear
