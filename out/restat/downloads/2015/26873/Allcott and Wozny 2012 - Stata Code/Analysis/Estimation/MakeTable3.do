...
local params2 =" 1,15,1999,2008.167,1950,2008,1, 9,1000000006025024100000, 20, 0,0" /* Martingale */
local params3 =" 1,15,1999,2008.167,1950,2008,1, 9,1000000006025024100000, 20, 4,0" /* G(futures), G(Martingale)-G(futures) */
local params4 =" 1,15,1999,2008.167,1950,2008,1, 9,1000000006025024100000, 20, 4.01,0" /* G(martingale), G(futures)-G(Martingale) */
...
while `row'<=`maxrow' {
    ...
    * QInGroup is the market share of a car within its group
    ...
    if `specialcode'==4.01 {
				gen DeltaGasCost = GasCostPrimary - GasCost
				*qui replace GasCost = GasCostPrimary
				local ExtraG = "DeltaGasCost"
    }
    if `specialcode'==4.1 {
