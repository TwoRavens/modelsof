...
forvalues i=1/10 {
	local params`i' =" 1,15,1999,2008.167,1950,2008,1, 9,1000000006025024100110, 20,0,0" /*  Futures */
}
forvalues i=11/20 {
	local params`i' =" 1,15,1999,2008.167,1950,2008,1, 9,1000000006025024100000, 20,0,0" /*  Martingale */
}
...
while `row'<=`maxrow' {
    ...
    * load data set and choose a subset
    ...
    if `adjustintensive'>0 {
    		if `specialcode'==0 | `specialcode'==15.1 replace Price = Price - I
    		if `specialcode'==15 replace GasCost = GasCost - I
    		if `specialcode'==15.1 replace GasCost = G`primarycode'
    }
    ...
    if `specialcode'>4 & `specialcode'<6 {
    		if `row'<=10 {
	        rename G1001000006025024100110 GasCost1
	        rename G1002000006025024100110 GasCost2
	        rename G1003000006025024100110 GasCost3
	        rename G1004000006025024100110 GasCost4
	        rename G1005000006025024100110 GasCost5
	        rename G1006000006025024100110 GasCost6
				}
				if `row'>=11 {    		
    ...
    if `specialcode'>5 & `specialcode'<6 {
    		...
    		if (`specialcode'-5)-`maxlag'*0.1==0.01 {
		    		...
		    		forvalues i=1/`maxlag' {
		    				replace GasCostAvg = GasCostAvg + GasCost`i'
		    		}
		    		replace GasCostAvg = GasCostAvg / (`maxlag'+1)
				}
				else {    			
		    		gen GasCostAvg = 0
		    		forvalues i=1/`maxlag' {
		    		...
		    		replace GasCostAvg = GasCostAvg / (`maxlag')
		    	}
		}
    ...
    gen int Time = (Year-1999)*12 + Month-1
		...
		if `specialcode'>=50 & `specialcode'<=59 {
				egen FEGroup = group(CarID ModelYear)
		}
		else {
    ...
    * QInGroup is the market share of a car within its group
    ...
    replace Quantity = . if ModelYear==2008
		...
		if `specialcode'==31.1 replace Price = 10^6*ln(Liters)
		if `specialcode'==31.2 replace Price = 10^6*ln(PolkWheelbase)
		if `specialcode'==31.3 replace Price = 10^6*ln(ShippingWeight)
		if `specialcode'==32.1 replace Price = 10^6*ln(HP)
		if `specialcode'==32.2 replace Price = 10^6*ln(Weight)
		if `specialcode'==32.3 replace Price = 10^6*ln(Wheelbase)
		if `specialcode'==32.4 replace Price = MSRP
		if `specialcode'==32.5 replace Price = 10^6*ln(Torque)
		if `specialcode'==32.6 replace Price = 100*Traction
		if `specialcode'==32.7 replace Price = 100*ABS
		if `specialcode'==32.8 replace Price = 100*Stability
    ...
    sort FEGroup Time
		...
		/* Collapse by time */
		if `row'<=4 local nmonth=`row'
		if `row'>=6 & `row'<=9 local nmonth=`row'-5
		if `row'==5 | `row'==10 local nmonth=6 
		if `row'>10 & `row'<=14 local nmonth=`row'-10
		if `row'>=16 & `row'<=19 local nmonth=`row'-15
		if `row'==15 | `row'==20 local nmonth=6 

		gen NewTime = floor(Time/`nmonth')
		gen NewMonth = floor((Month-1)/`nmonth')+1
		sort FEGroup NewTime Time
		by FEGroup NewTime: egen TotalObs=total(NumObs)
		foreach var in Price GasCost {
				by FEGroup NewTime: egen SumProd=total(NumObs*`var')
				replace `var'=SumProd/TotalObs
				drop SumProd
		}
		by FEGroup NewTime: keep if _n==1
		replace Time=NewTime
		replace Month=NewMonth
		replace NumObs=TotalObs
		drop NewTime NewMonth TotalObs
    ...
    if `specialcode'==4 {
				...
				replace GasCost = GasCostPrimary
    ...
    if `specialcode'==4.7 {
				replace GasCost = GasCost - GasCost1
				local ExtraG = ""
				forvalues i=1/5 {
						local i1=`i'+1
						gen DeltaG`i1'= GasCost`i'-GasCost`i1'
    ...
    */
    ...
    if `specialcode'>=17 & `specialcode'<=19.1 {
		...
		if `specialcode'==20 {
				...
				replace GasCost = GasCost * (Year>=2004)
		}
		if `specialcode'==21 {
				gen G_pre05 = GasCost * (Year<2005)
				local ExtraG = "`ExtraG' G_pre05"
				replace GasCost = GasCost * (Year>=2005)
		}
		if `specialcode'==22 | `specialcode'==22.1 | `specialcode'==22.2 {
				...
				replace GasCost = GasCost * (Year>=2004 & (Year<2008 | Month<=3))
		}
		if `specialcode'==23 {
				gen G_pre05 = GasCost * (Year<2005)
				gen G_post0308 = GasCost * (Year==2008 & Month>3)
				local ExtraG = "`ExtraG' G_pre05 G_post0308"
				replace GasCost = GasCost * (Year>=2005 & (Year<2008 | Month<=3))
		}
		if `specialcode'==60 {
		...
		if `specialcode'==63 {
				xi i.Time*i.VClass, prefix(_TC)
				local ExtraCtrl = "_TC*"
		}
		if `specialcode'==64 {
				xi i.Time*i.Nameplate, prefix(_TN)
				local ExtraCtrl = "_TN*"
		}
		/* lnShare on LHS */		
		*if `specialcode'==30 replace Price = 10^6*lnShare
		...
		if floor(`specialcode')==1 | floor(`specialcode')==2 {
				...
				if `ctrlcode'==2.3 {
						gen MYmod = ModelYear
						replace MYmod = 1990 if MYmod<1990
...
*								if (`i'>1 | `j'>`minage') & `numinbin'>0 gen _MPG`i'Age`j'=(MPGBin==`i'&AgeBin==`j')
		...
		if `row'<=5 | (`row'>10 & `row'<=15) {
			...
			foreach var of varlist Price GasCost `ExtraG' `ExtraCtrl' _IN* {
		...
		if (`row'>5 & `row'<=10) | (`row'>15 & `row'<=20) {
			/* Differences */
			local lag=1
			foreach var of varlist Price GasCost `ExtraG' `ExtraCtrl' _IN* {
				gen Tmp=`var'-L`lag'.`var'						
				replace `var' = Tmp
				drop Tmp
			}
			gen NumObsPair=(NumObs+L`lag'.NumObs)/2
		}
    ...
    if `row'==1 local rep = "replace"
		...
		if (`row'>5 & `row'<=10) | `row'>15 {
			di "ivreg Price (GasCost `ExtraG' = _IN*) `ExtraCtrl' [aw=NumObsPair], r cl(ClVar) first"
			ivreg Price (GasCost `ExtraG' = _IN*) `ExtraCtrl' [aw=NumObsPair], r cl(ClVar) first
		}
