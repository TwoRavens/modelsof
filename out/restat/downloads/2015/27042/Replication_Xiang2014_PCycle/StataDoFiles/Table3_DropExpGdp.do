...
cd "C:\Main\p-cycle\Writing\ReStatRevision\VS";

set logtype text; 
log using DropExpGdp, replace; 


*****************************************************************
***************Drop all countries whose exp/gdp ratio ever exceed 1 for 1978-2001******************************
*****************************************************************; 

*************step 1 distribution of exp/gdp ratios**********************************;
use ExpGDPRatio; drop if exgdpratio==.; count; sum exgdpratio, detail; tab countryname if exgdpratio>0.5;
tab countryname if exgdpratio>1; drop _all; 

***above 50%: 453440 (HK), 457020 (Singapore), 530560 (Belgium),  583480 (Hungary), 533720 (ireland), 454580 (Malaysia);
***above 50%: 574700 (Malta); 

***above 100%: HK and Singapore; 

*************step 2 Construction*************************;log off; 
...
***drop countries above 50%; 
drop if ccode==453440 | ccode==457020 | ccode==530560 | ccode==583480 | ccode==533720 | ccode==454580 | ccode==574700;
...
save C:\Main\p-cycle\Writing\ReStatRevision\VS\NS2DropExpGdp, replace; d; log off; 

erase ctsusa.dta; erase chs.dta; erase richcountries.dta; log on;

*************step 3. regression results********************; log off;
...
outreg2 t t2 ddlogtao2 using "C:\Main\p-cycle\Writing\ReStatRevision\Table3_Main", append ctitle(No Exp GDP);
