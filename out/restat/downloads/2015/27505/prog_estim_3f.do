...
program define step2
	...
	gen Rq3 = normal((ln(`cut3'/(1-`cut3'))-xb)/sig)-normal((ln(`cut2'/(1-`cut2'))-xb)/sig)
	gen Rq4 = 1-normal((ln(`cut3'/(1-`cut3'))-xb)/sig)

	local thresh = .001
	gen ins = (min(Rq1,Rq2,Rq3,Rq4)>=`thresh')
	qui su ins
...
est_boot, nb(500) dz(183) dy(366) mgeo(regionU) mnp(mgeo) mp(occ_* y_*) name(nq4)
