...
outreg2 using pwages_new_panelA, se bracket coefastr bdec(3) excel ctitle("predicted") title("Predicted and Residual Wages") replace
xi: reg migus i.ln_returns $region $cost_vars [pw=$weight] if $restrict , robust cluster(folio)
outreg2 using pwages_new_panelA, se bracket coefastr bdec(3) excel append ctitle("predicted")

replace ln_returns=ln_earnings_yr_cat_res

xi: reg migus i.ln_returns $region [pw=$weight] if $restrict , robust cluster(folio)
outreg2 using pwages_new_panelA, se bracket coefastr bdec(3) excel append ctitle("residual")
xi: reg migus i.ln_returns $region $cost_vars [pw=$weight] if $restrict , robust cluster(folio)
outreg2 using pwages_new_panelA, se bracket coefastr bdec(3) excel append ctitle("residual")
