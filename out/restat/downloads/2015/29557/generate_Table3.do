...
outreg2 using wages_new, se bracket coefastr bdec(3) excel ctitle("TEST") title("Predicted wages") replace

******** earnings quintiles *******

xi: regress migus i.ln_returns $region [pw=$weight] if $restrict , robust cluster(cat)
outreg2 using wages_new, se bracket coefastr bdec(3) excel append ctitle("educ*age*ability")
xi: regress migus i.ln_returns $region $cost_vars [pw=$weight] if $restrict , robust cluster(cat)
outreg2 using wages_new, se bracket coefastr bdec(3) excel append ctitle("educ*age*ability")
xi: regress migus i.ln_returns $region diff_returns [pw=$weight] if $restrict , robust cluster(cat)
outreg2 using wages_new, se bracket coefastr bdec(3) excel append ctitle("educ*age*ability")
xi: regress migus i.ln_returns $region diff_returns $cost_vars [pw=$weight] if $restrict , robust cluster(cat)
outreg2 using wages_new, se bracket coefastr bdec(3) excel append ctitle("educ*age*ability")
xi: regress migus i.ln_returns $region lvl_returns [pw=$weight] if $restrict , robust cluster(cat)
outreg2 using wages_new, se bracket coefastr bdec(3) excel append ctitle("educ*age*ability")
xi: regress migus i.ln_returns $region lvl_returns $cost_vars [pw=$weight] if $restrict , robust cluster(cat)
outreg2 using wages_new, se bracket coefastr bdec(3) excel append ctitle("educ*age*ability")
