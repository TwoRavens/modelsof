...
capture log close
log using ETR_Butchers.smcl,replace;
...
/* read in data --> first save filename.xls as filename.txt */
	insheet using X:\Indicators\ETR\Results\raw_data\ETR_BUTCHERS_2007.txt, delimiter(";") case;};
	compress;
...
/* 2. merge with market demographics and put zeros in zips with NO observation of firms */
	...
	saveold MMS_Butchers.dta, replace;				/* dataset to be used for Gauss analysis */
	outsheet using MMS_Butchers.xls, replace;	
...
/* 4. initial regressions: single equations */
	...
	/* 4.1 initial regressions on revenues */
		...
		reg lnRf lnsurf foreign female lninc unempl kid young adult2 old FLA D2 D3 D4 D5 D6 D7 D8, noconstant;
