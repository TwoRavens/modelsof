...
capture log close
log using ETR_Bakeries.smcl,replace;
...
/* read in data --> first save filename.xls as filename.txt */
	insheet using X:\Indicators\ETR\Results\raw_data\ETR_BAKERIES_2007.txt, delimiter(";") case;};
	compress;
...
/* 2. merge with market demographics and put zeros in zips with NO observation of firms */
	...
	saveold MMS_Bakeries.dta, replace;				/* dataset to be used for Gauss analysis */
	outsheet using MMS_Bakeries.xls, replace;	
...
/* 1. sample selection: only non-urban markets and no markets with lacking revenue information */
		...
		gen Rpf=R/help;				
		hist Rpf if N~=0; 
		/*ERR;*//* look at the histogram and check for outliers --> select at cutoff value HERE  */	
		drop if Rpf >= 1000000;	
		hist Rpf if N~=0;
		drop help;
...
/* 4. initial regressions: single equations */
	...
	/* 4.1 initial regressions on revenues */
		...
		reg lnRf lnsurf foreign female lninc unempl kid young adult2 old FLA D2 D3 D4 D5 D6 D7 D8 D9 D10 D11, noconstant;
