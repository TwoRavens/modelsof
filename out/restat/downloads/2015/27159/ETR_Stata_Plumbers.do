...
capture log close
log using ETR_Plumbers.smcl,replace;
...
/* read in data --> first save filename.xls as filename.txt */
	insheet using X:\Indicators\ETR\Results\raw_data\ETR_PLUMBERS_2007.txt, delimiter(";") case;};
	compress;
...
/* 2. merge with market demographics and put zeros in zips with NO observation of firms */
	...
	saveold MMS_Plumbers.dta, replace;				/* dataset to be used for Gauss analysis */
	outsheet using MMS_Plumbers.xls, replace;	
...
/* 4. initial regressions: single equations */
	...
	/* 4.1 initial regressions on revenues */
		...
		/*ERR;*/
		replace D15=1 if N>14;		
		replace N=14 if N>=14;

		/* 3. revenue regressions - single equation = there is an endogeneity problem but nonetheless gives some insights */		
		reg lnRf lnsurf foreign female lninc unempl kid young adult2 old FLA D2 D3 D4 D5 D6 D7 D8 D9 D10 D11 D12 D13 D14 D15, noconstant;
