...
capture log close;
*log using $masterpath/logfiles/premium_data.log, replace;

/*================================================
 Program: premium_man7090.do
 Author:  Avi Ebenstein
 Created: March 2007
 Purpose: Prepare wage data from CPS MORG respondents
          and store individual-level data in wage_data.dta.
          Collapse by industry and year into premium_man7090.dta.

=================================================*/

!gunzip $masterpath/cpsfiles/morg_man7090.dta.gz;  
...
forvalues i=1982/2002 {;
  reg lwage _Ieduc* _Iexper* _IeduXexp* female nonwhite _Istate* if year==`i' [w=ihwt];
  predict yhat`i' if year==`i';
  replace yhat_year=yhat`i' if year==`i';
};
...
save $masterpath/datafiles/micro_man7090, replace;
...
(mean) emp_cps=emp (mean) wages_cps=wages (sum) respondents=dummy [w=ihwt],by(man7090 year) fast;
...
save $masterpath/datafiles/premium_man7090, replace;

************************************;
* Calculate changes in wage premium ;
************************************;
