...
/*================================================
 Program: premium_occ8090_mfg.do
 ...
 Purpose: Prepare wage data from CPS MORG respondents
          ...
          Collapse by industry and year into premium_occ8090_mfg.dta.
...
save $masterpath/datafiles/micro_occ8090_mfg, replace;

**************************************************;
* The MORG files are 12 times the true population ;
**************************************************;

bysort man7090 year: egen emp=sum(orgwgt/12);
bysort man7090 year: egen wages=sum(w_ln_ot*uhourse*52*orgwgt/12);
...
save $masterpath/datafiles/premium_occ8090_mfg, replace;

/* We were using $masterpath/datafiles/prices/premium_occ8090 in premium_prices_1digit.do 
  (just mfg workers are occ8090 x year level) which did not include trade & offshoring variables below 
  (those are read in in premium_prices_1digit.do). To match that, I will read in 
  $masterpath/datafiles/prices/premium_occ8090_mfg without below variables into premium_prices_1digit.do.
  The only difference between datafiles/prices/premium_occ8090 and now datafiles/premium_occ8090_mfg
  is the way that wage premiums are calculated. Calculations in datafiles/premium_occ8090_mfg match those 
  in datafiles/premium_occ8090_all, which we use in GR regressions, whereas datafiles/premium_occ8090 do not. 
...
use ${y}micro_man7090, clear;
sort occ8090 year;
merge occ8090 year using ${x}premium_occ8090_mfg;
tab _merge;
drop if _merge==1;
drop _merge;
...
save ${x}premium_occ8090_mfg.dta, replace;
