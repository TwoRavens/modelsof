...
* This version considers only products which are exported by the country at some point over the period
...
************************************************
*
use "$trade\comtrade_hs4_all", clear
collapse (sum) export, by(iso3 hs4)
save comtrade_hs4_sum, replace
...
rename hs hs4
sort iso3 hs4 
merge iso3 hs4 using comtrade_hs4_sum, nokeep
tab _merge
drop _merge
rename hs4 hs
*
/* keep if product exported at some point */
drop if export_ikt == .
drop export_ikt
*
sort hs
...
*
collapse (sum) shock_fao_x = shock_fao, by(gid year region_fao)
*
label var shock_fao "World demand weighted by hs spec."
*
drop region_fao
...
*
cd "$Output_data"
save FAO_PRIO_GRID_X, replace
*
cd "$fao"
erase FAO_ALL_AFRICA.dta
erase FAO_PRIO_GRID.dta
erase comtrade_hs4_sum.dta
cd "$Output_data"
