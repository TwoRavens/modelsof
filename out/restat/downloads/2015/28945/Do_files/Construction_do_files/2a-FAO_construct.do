...
save temp, replace
/*keep years*/
collapse (mean) production, by(country_code year)
drop if production == 0 | production == .
drop production
rename country_code iso3
g fao_coverage = 1
label var fao_coverage "Country-year covered by FAO Agro-Maps"
sort iso3 year
save "$Output_data\fao_years", replace
*
/* take average production */
use temp, clear
collapse (mean) production, by(country_code area_name year)
rename country_code iso3
rename area_name    region
sort iso3 region year
save production_fao, replace
use temp, replace
...
*
collapse (sum) shock_fao production, by(iso3 area_name area_code admin_level year)
...
*
**************************************
* D - ASSOCIATE FAO SHOCK WITH GRIDS *
**************************************
/*
...
collapse (mean) test, by(gid region_fao)
drop test
sort gid
save GID_REGION_FAO, replace
*
/* sample of missing values for production data*/
use FAO_PRIO_GRID, clear
sort iso3 region year
merge iso3 region year using production_fao, nokeep
g missing = (production==.)
bys gid year: egen max_perc   = max(percentage)
egen region_group 			  = group(region iso3)
g temp 						  = region_group if max_perc == percentage
bys gid year: egen region_fao = max(region_group)
collapse (sum) production (mean) missing, by(gid year region_fao)
drop production region_fao
sort gid year
label var missing "missing cell-year in FAO-Agromap"
save FAO_missing, replace
erase production_fao.dta
*
use FAO_PRIO_GRID, clear
sort iso3 region year
...
bys gid year: egen region_fao = max(region_group)
*
collapse (sum) shock_fao, by(gid year region_fao)
*
label var shock_fao "World demand weighted by hs spec."
*
drop region_fao
...
save GRID_PERCENTAGE, replace
cd "$fao"
use FAO_missing, clear
cd "$Output_data"
save FAO_missing, replace
*
cd "$fao"
use  GID_REGION_FAO, clear
cd "$Output_data"
save GID_REGION_FAO, replace
*
cd "$fao"
