...
* This do file compute the version of the shock which excludes large players (more than 1% of world trade) 
...
*
************************************************
* C - MERGE WITH WORLD DEMAND FOR HS COMMODITY *
************************************************
*
use temp, clear
rename hs hs4
...
/* market shares */
sort iso3 hs4 year
merge iso3 hs4 year using "$trade\comtrade_hs4_all", nokeep
tab _merge
replace export_ikt = 0 if _merge == 1
*
g export_share_ikt = export_ikt/trade
g import_share_ikt = import_ikt/trade
*
bys iso3 hs4: egen export_share_ik = mean(export_share_ikt)
bys iso3 hs4: egen import_share_ik = mean(import_share_ikt)
*
log using "$results\stats_market_share.log", replace
/* statistics market share */
sum export_share*, d 
sum import_share*, d 
sum export_share_ik if export_share_ik>0, d 
sum import_share_ik if import_share_ik>0, d 
log close
*
/* new shares, excluding "large" players */
replace production = . if export_share_ik > 0.01 | import_share_ik>0.01
bys fao_region year: egen production_tot  = sum(production)
...
*
collapse (sum) shock_fao_mp = shock_fao, by(gid year region_fao)
*
label var shock_fao "World demand weighted by hs spec."
*
drop region_fao
