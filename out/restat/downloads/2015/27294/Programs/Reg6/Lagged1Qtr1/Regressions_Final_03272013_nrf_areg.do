...
keep origin dest year quarter HHI ;
duplicates drop;
...
use Sample_04192011, clear;
cap drop x;
dmerge origin dest year quarter using `temp', nokeep;
tab _m;
drop _m;
...
dmerge origin dest using "Input_Files/DistNS.dta", nokeep;
drop _m;
...
gen inter_HHI_lag = InterCPSadj * HHI_1qtr_lag;
...
****  Demean w/o instruments ************;

use `temp', clear;

keep if inter_HHI_lag * HHI_1qtr_lag != .;
foreach name in InterCPSadj HHI inter_HHI LCC Fare log_Fare load_factor log_LF
                ...
                arrdelayDum log_schelapsedtime inter_HHI_lag HHI_1qtr_lag {;
                        sort `fe';
...
capture for X in num 1/50:  sort `fe' \
...
drop  opcarrier acft_type quarter  ;

display "`fe', W/o Instrum";
sum HHI_1qtr_lag inter_HHI_lag arrdelay schelapsedtime log_schelapsedtime total_actual_time depdelay log_total_actual_time  arrdelayDum
...
keep if log_instrum2_lf * log_instrum2_fare * inter_HHI_lag * HHI_1qtr_lag != .;
foreach name in InterCPSadj HHI inter_HHI LCC Fare log_Fare load_factor log_LF
...
use `temp'wo_instrum, clear;

qui ivreg `name' InterCPS empl_perc log_incpc log_pop
                late_arrival scheduled_buffer flight_in_day
                (HHI inter_HHI = HHI_1qtr_lag inter_HHI_lag)
                log_LF
...
estout  m4 m5 
        using "Reg6/Lagged1Qtr1/`name'_`fe'_large100_fnl_`name1'_nrf.xls",
