...
gen inter_LCC  = InterCPS * LCC;
gen inter_HHI_LCC = inter_HHI * LCC;
...
keep origin dest distance LCC;
duplicates drop;

bysort LCC: egen p20 = pctile(distance), p(20);
bysort LCC: egen p40 = pctile(distance), p(40);
bysort LCC: egen p60 = pctile(distance), p(60);
bysort LCC: egen p80 = pctile(distance), p(80);
...
keep origin dest quantile LCC;
sort origin dest LCC;
save `temp'dist_q,  replace;

use `temp', clear;
sort origin dest LCC;
merge origin dest LCC using `temp'dist_q, nokeep;
drop _merge;
...
keep LCC quantile opcarrier revenue Fare passengers passengers_t100 seats_t100 year quarter;
duplicates drop;

collapse (sum) revenue_q = revenue pass_q1 = passengers pass_q2 = passengers_t100 seats_q = seats_t100 ,
     by(year quarter LCC opcarrier quantile);
save `temp'1, replace;

use `temp', clear;
sort year quarter LCC opcarrier quantile;
merge year quarter LCC opcarrier quantile using `temp'1, nokeep;
drop _m;
...
qui areg `name' InterCPS empl_perc log_incpc log_pop
                LCC inter_HHI_LCC HHI inter_HHI inter_LCC
...
estimates store m1, title(Model 4);
display "`name', Model 1";
test InterCPS + inter_LCC = 0;
test InterCPS + 0.9 * inter_HHI = 0;
test InterCPS + 0.9 * inter_HHI + inter_LCC + 0.9 * inter_HHI_LCC = 0;
...
qui areg `name' InterCPS empl_perc log_incpc log_pop
                ...
                nr_arrs_day_dest nr_arrs_day_orig
                cym* arcft*, r cluster(segment) absorb(segment) ;
estimates store m2, title(Model 5);
...
ivreg `name' InterCPS (log_Fare  = log_instrum2_fare  log_instrum2_lf )
                LCC inter_HHI_LCC HHI inter_HHI inter_LCC
                ...
                nr_flt_dep_Dest_130_230 nr_flt_arr_Origin_30_130 nr_flt_dep_Dest_230_330 nr_deps_day_orig nr_deps_day_dest 
                nr_arrs_day_dest nr_arrs_day_orig
                cym* arcft*, r cluster(segment) first;
estimates store m3, title(Model 6);
...
estout m1 m2 m3
        ...
        keep (InterCPSadj log_Fare
                empl_perc log_incpc log_pop
                HHI inter_HHI
                LCC inter_LCC inter_HHI_LCC
                nr_deps_hour_orig nr_deps_hour_dest nr_arrs_hour_dest nr_arrs_hour_orig
                HHI_dep_arpt HHI_arr_arpt nr_flt_arr_Origin_130_230 nr_flt_arr_Origin_230_330 nr_flt_dep_Dest_30_130 
                nr_flt_dep_Dest_130_230 nr_flt_arr_Origin_30_130 nr_flt_dep_Dest_230_330 nr_deps_day_orig nr_deps_day_dest 
                nr_arrs_day_dest nr_arrs_day_orig )
        order (InterCPSadj HHI inter_HHI LCC inter_LCC inter_HHI_LCC
               nr_deps_hour_orig nr_deps_hour_dest nr_arrs_hour_dest nr_arrs_hour_orig
               log_Fare);
