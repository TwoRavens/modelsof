...
capture for X in num 1001/2000:  sort `fe' \
                        by `fe': gen x = sum(ocX * z) / sum(z) \
                        by `fe': replace x = x[_N] \
                        replace ocX = ocX - x \
                        drop x;                        
...
keep if log_instrum2_lf * log_instrum2_fare * log_instrum1_lf * log_instrum1_fare *
         instrum1 * instrum2 * instrum4 != .;
foreach name in InterCPSadj HHI inter_HHI LCC Fare log_Fare load_factor log_LF
...
qui reg `name' InterCPS empl_perc log_incpc log_pop
                late_arrival scheduled_buffer flight_in_day
                HHI
                log_LF
...
estout m0 m4 m5 
        using "Reg1/`name'_`fe'_large100_fnl_`name1'_nrf.xls",
