...
foreach i of local industries {
    ...
    egen tmp3 = mean(gth_vareal), by(wbcode isic)     /*taking the mean of each sector*/
    ...
    egen sd2=sd(gth_vareal), by(wbcode isic)         /*Standard deviation of each sector*/
    egen n2=count(gth_vareal), by(wbcode isic)       /*Counting observations of each sector*/
    replace sd2=sd2*sqrt((n2-1)/n2)              /*Unbiased estimator*/
    gen tmp5=gth_vareal-tmp3                         /*demean each sectors' growth*/
    ...
    qui gen corr`i'=(tmp8/tmp9)/(sd1*sd2)        /*Computing correlation*/
    qui gen cov`i' =(tmp8/tmp9)
    pause
...
collapse gth_varea corr* cov* minyear nestab share (count) ngvar=gth_vareal (sd) sdgvar = gth_vareal, by(wbcode isic)

save correlations_8003.dta
