...
*Generates a dta file with the frequency statistics that is later used to generate the paper's tables
...
local share_obs_w_sales=0
local share_prod_w_sales=0
...
* *median and mean observations per good
* use "${dir_temp}\temp_${sales}_${database}.dta", clear
* by id: egen count=count(id) if miss!=1
* collapse count, by(id)
* qui log on 
* *median and mean observations per good
* tabstat count, statistics( mean median ) columns(statistics)
* qui log off
...
**NOW WE CAN COLLAPSE BY ID 
**this is a much easier database to use. From now on, I can use tabstat to compute means, medians, etc. 
*Note that I calculate "mean", but within each id all these variables are the same, so no real effect there, just using collapse command
collapse (mean) category bppcat  freq_full_id, by(id)
label values category bppcat
...
label var index_sup "Scraped Retailer Index"

*An average food index to compare with official ayb stats
gen index_food=index if category<=300
...
label var index_ayb "Scraped Food Index"
label var index_sup "Scraped Retailer Index"
label var index_sup2 "Scraped Retailer Index v2"
...
sort date
tsset date
capture tssmooth ma index_sup_ma=index_sup, window(29 1)
