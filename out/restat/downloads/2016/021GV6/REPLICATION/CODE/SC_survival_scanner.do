...
histogram duration if duration<=36, width(1) percent xlabel(0(1)36) xscale(range(0 36))  title(Duration Spells) note(Notes: 1-week bins. Observations = `obsdur' | Mean = `meandur' | Median = `mediandur' | Stdev=`sddur' )
...
graph export ${dir_graphs}\survival_hazard_a4_WEEKLY_${sales}_${database}.png, replace

sts graph , hazard noboundary kernel(gaussian) legend(off) ci level(99) tmax(25) width(1) 
graph export ${dir_graphs}\survival_hazard_a4_WEEKLY_25_${sales}_${database}.png, replace

sts graph , hazard noboundary kernel(gaussian) legend(off) ci level(99) tmax(52) width(1) 
graph export ${dir_graphs}\survival_hazard_a4_WEEKLY_52_${sales}_${database}.png, replace
...
*<ake each spell like a different id. Note short spells influence the results a lot.
stset date1,  time0(date0) origin(time date0) exit(time .) failure(c_mprice) 
stdes
stvary
sts graph , hazard noboundary kernel(gaussian) legend(off) ci  width(1) 
