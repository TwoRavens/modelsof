...
* *Identify initial and last spells
* tsspell price, spell(spellprice) seq(seqprice) end(endprice)
...
* Now average inflation per category

*select category level to get average
drop category
gen category=bppcat

bysort category date: egen cat_mean_nonmiss=gmean(relative)
...
save "${dir_rawdata}\cpi_bppcat_${sales}_${database}.dta", replace
*******************
