...
log using "`log'\restat_spectest_covbalplacebo.log", replace;
local exp "entry04rd";

/* 
Manasi Deshpande, 12/06/2015
Covariate balance placebo test varying the cutoff date
*/
...
forval poly=1(1)`polyend' {;
	...
	/* Create empty matrix to fill (one matrix per poly order) */
	matrix covbal_poly`poly' = J(365, 3, .);
	matrix colnames covbal_poly`poly'= date year2001 year2002;

	foreach yr in 2001 2002 {;

		forval i=1(1)365 {;

		/* Create running variable */
		gen awddte_run=awddte_stata-(mdy(01,01,`yr')+`i'-1);
		matrix covbal_poly`poly'[`i',1]=(mdy(01,01,`yr')+`i'-1);
		...
		test n_post;
		local pval=r(p);
		matrix covbal_poly`poly'[`i',`=`yr'-1999']=`pval';
...
log using "`log'\restat_spectest_covbalplacebo.log", append;

forval poly=1(1)`polyend' {;
	matrix list covbal_poly`poly';
	clear;
	svmat covbal_poly`poly';
	save "`data'\statsby\restat_spectest_covbalplacebo_poly`poly'.dta", replace;
};
