...
/*THIS FILE PRODUCES THE RESULTS OF DATA APPENDIX, TABLES A8*/ 
/*Careful: must choose which set of columns. See code below*/
...
/**** Retired, young and old***/
gen retired=(q==3 & eta>65);		/*Fo columns 1 to 3*/
gen young=(eta<58);
/******************************/
...
sort nq anno; merge nq anno using BS_house, update replace;	/*To fit housing exp model*/
...
/*MUST CHOOSE*/
/*drop if retired==1;		/*Columns 1, 2 and 3*/ */
/*drop if young==0;		/*Columns 4. 5 and 6*/ */
drop if young==1;		/*Columns 7, 8 and 9*/
...
set seed 171202
	...
	*bsample, cluster(nquest) id(id) strata(panel)		/*HERE... WHAT?*/
	...
	matrix define results=	reg1\reg2\reg9\reg10\reg11\reg12\
	/*K x 1 matrix*/		reg25\reg26;
...
matrix define true=	reg1\reg2\reg9\reg10\reg11\reg12\
	/*K x 1 matrix*/		reg25\reg26;
...
scalar drop	reg1 reg2 reg9 reg10 reg11 reg12 reg25 reg26;
