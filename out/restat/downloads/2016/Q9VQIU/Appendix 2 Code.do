...
graph twoway  
 	        (scatter inj_accident_f_np_r age_months, mcolor(blue)  msymbol(O)  msize(.6) yaxis(1) 
			   yscale(titlegap(2)) 
			   ylabel(0(500)1000, axis(1) nogrid)
			   ) 
            ...
            (line inj_by_oth_f_np_right age_months, lwidth(thin) lcolor(green) yaxis(2)) 			
             , 
			 title("Appendix 2: Emergency Department Visits by Cause - Female", size(4)) 
			 xtitle("Age at Time of ED Visit")  
			 ytitle("Accidental Injuries",axis(1)) 
			 ytitle("Alcohol Intoxication & Deliberate Injuries", axis(2)) 
			 legend(off) 
			 text(1000 20 "Accidental Injury",color(blue) size(3)) 
			 text( 430 20 "Alcohol Intoxication", color(black) size(3))
			 text( 250 20 "Deliberately Self Inflicted Injury", color(red) size(3)) 
			 text( 600 20 "Deliberately Injured by Other Person", color(green) size(3)) 
      ...
      ;
#delimit cr
grexportpdf "`base'\Code for Appendices\Appendix 2.pdf"
