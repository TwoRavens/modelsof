...
use mayhog200m, clear
local quarters "300 400 101 201 301 401 102 202 302 402 103 203 303 403 104 204 304"
foreach quarter of local quarters {
	append using mayhog`quarter'm
}
compress
...
foreach quarter of local quarters {
	svy, subpop(if real(PER) == `quarter' & include == 1): mean rhwage
	mat define B = e(b)
	replace qmeanwage = B[1,1] if real(PER) == `quarter' & include == 1
}
