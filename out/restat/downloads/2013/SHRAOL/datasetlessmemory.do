...
set memory 900m
set more off
log using datasetlessmemoryrev.log, replace
...
use L_NAC rmig rmig1 rmig2 rmig3 m1 m2 m3 m4 ENT ESC SEX E_CIV T_RES MEN_12 PAR EDA P3 P2B1_2 P2C MUN EST P4 PER P7A_1 P7A_2 P6_1 P5 FAC using mayhog`quarter'm, clear

/*Creating labels for the Mexican states*/
generate nent = real(ENT)
generate lnac = real(L_NAC)
...
/*Household size*/
gen hhsize = real(T_RES)

/*Children less than 12*/
gen children = real(MEN_12)

/*Position in household*/
/*Household head*/
gen head = (real(PAR)==1)
/*Head's spouse*/
gen spouse = (real(PAR)==2)
/*Sons, daughters*/
gen descendent = (real(PAR)==3)
/*Others*/
gen otherrel = 0
replace otherrel = 1 if real(PAR)>3 & real(PAR)<10
...
lab def age 1 "12 to 15"
lab def age 2 "16 to 20", add
lab def age 3 "21 to 25", add
lab def age 4 "26 to 30", add
lab def age 5 "31 to 35", add
lab def age 6 "36 to 40", add
lab def age 7 "41 to 45", add
lab def age 8 "46 to 50", add
lab def age 9 "51 to 55", add
lab def age 10 "56 to 60", add
lab def age 11 "61 to 65", add
lab def age 12 "66 or more", add
...
lab def schooling 2 "1 to 5", add
lab def schooling 3 "6 to 8", add
...
/*Distance to the border variable*/
sort municipio
merge municipio using distanceborder, keep(distancia) uniqusing nokeep
drop _merge

/*Network variable (McKenzie and Rapoport, 2007) from the ENADID*/
sort municipio
merge municipio using municipalnetwork, keep(network) uniqusing nokeep
drop _merge

/*Network variable from the ENET*/
sort municipio
merge municipio using municipalnetwork2, keep(network2) uniqusing nokeep
drop _merge

/*Network variable (McKenzie and Rapoport, 2007) from the ENADID, using the work in the US question*/
sort municipio
merge municipio using municipalnetwork3, keep(network3) uniqusing nokeep
drop _merge

/*Rural variable*/
gen rural = (real(EST)>=40)

/*Agriculture*/
/*Work in agriculture*/
gen agriculture = 0 if laborforce == 1
replace agriculture = 1 if (match(substr(P4,1,2),"41") == 1 & laborforce == 1)
/*Manufacturing*/
gen manufacture = 0 if laborforce == 1
replace manufacture = 1 if (match(substr(P4,1,1),"5") == 1 & laborforce == 1)
...
replace rhwage = (hwage*116.301)/(10.7777*112.0787) if real(PER) == 404
...
replace rmwage = (mwage*116.301)/(10.7777*112.0787) if real(PER) == 404
...
/*Generating wages relative to the average quarter wage*/
gen include = 1 if edad > 15 & edad < 66 & rhwage ~= .
gen qmeanwage = .
svyset, clear
svyset [pw=FAC]
svy, subpop(if include == 1): mean rhwage
mat define B = e(b)
replace qmeanwage = B[1,1] if include == 1

/*Standardized wages (real wage divided by quarter average)*/
gen wagest = rhwage/qmeanwage
gen lwage = log(wagest)

drop ENT ESC SEX E_CIV T_RES MEN_12 PAR EDA P3 P2B1_2 P2C MUN P4 PER P7A_1 P7A_2 P6_1 P5 L_NAC
compress
save data`quarter'rev, replace
}
