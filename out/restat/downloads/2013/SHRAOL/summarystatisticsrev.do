/*Summary statistics of the ENET for comparison with the MMP: it only includes household heads*/
clear
capture log close
set matsize 800
set memory 900m
set more off
log using summarystatisticsrev.log, replace

use head EST rmig rmig1 rmig2 rmig3 m1 m2 m3 m4 laborforce agriculture manufacture wageearner paro rhwage ametdum rural schoolyears year mujer FAC edad using datashortrev, clear
compress

gen include = 1 if edad > 15 & edad < 66 & head == 1
...
_pctile edad [pw=FAC] if mujer == 0 & mig == 0 & include == 1 
matrix resultsmen[9,1] = r(r1)
_pctile edad [pw=FAC] if mujer == 0 & mig == 1 & include == 1 
...
_pctile schoolyears [pw=FAC] if mujer == 0 & mig == 0 & include == 1 
matrix resultsmen[13,1] = r(r1)
_pctile schoolyears [pw=FAC] if mujer == 0 & mig == 1 & include == 1 
...
svy, subpop(if include == 1 & mujer == 0): mean wageearner, over(mig) 
matrix B = e(b)
matrix resultsmen[20,1] = B[1,1]
matrix resultsmen[20,2] = B[1,2]
matrix V = e(V)
matrix resultsmen[21,1] = sqrt(V[1,1])
matrix resultsmen[21,2] = sqrt(V[2,2])
...
svy, subpop(if include == 1 & mujer == 0): mean rhwage, over(mig)
matrix B = e(b)
...
_pctile rhwage [pw=FAC] if mujer == 0 & mig == 0 & include == 1 
matrix resultsmen[27,1] = r(r1)
_pctile rhwage [pw=FAC] if mujer == 0 & mig == 1 & include == 1 
matrix resultsmen[27,2] = r(r1)
...
_pctile edad [pw=FAC] if mujer == 1 & mig == 0 & include == 1 
matrix resultswomen[9,1] = r(r1)
_pctile edad [pw=FAC] if mujer == 1 & mig == 1 & include == 1 
...
_pctile schoolyears [pw=FAC] if mujer == 1 & mig == 0 & include == 1 
matrix resultswomen[13,1] = r(r1)
_pctile schoolyears [pw=FAC] if mujer == 1 & mig == 1 & include == 1 
...
svy, subpop(if include == 1 & mujer == 1): mean wageearner, over(mig) 
matrix B = e(b)
matrix resultswomen[20,1] = B[1,1]
matrix resultswomen[20,2] = B[1,2]
matrix V = e(V)
matrix resultswomen[21,1] = sqrt(V[1,1])
matrix resultswomen[21,2] = sqrt(V[2,2])
...
svy, subpop(if include == 1 & mujer == 1): mean rhwage, over(mig)
matrix B = e(b)
matrix resultswomen[25,1] = B[1,1]
matrix resultswomen[25,2] = B[1,2]
matrix V = e(V)
matrix resultswomen[26,1] = sqrt(V[1,1])
matrix resultswomen[26,2] = sqrt(V[2,2])
_pctile rhwage [pw=FAC] if mujer == 1 & mig == 0 & include == 1 
matrix resultswomen[27,1] = r(r1)
_pctile rhwage [pw=FAC] if mujer == 1 & mig == 1 & include == 1 
matrix resultswomen[27,2] = r(r1)
...
gen metrocat = 1 if real(EST)<20
replace metrocat = 2 if real(EST)<30 & real(EST)>19
replace metrocat = 3 if real(EST)<40 & real(EST)>29
replace metrocat = 4 if real(EST)>39

svy, subpop(if include == 1 & mujer == 0): prop metrocat
svy, subpop(if include == 1 & mujer == 0): prop metrocat, over(mig)
svy, subpop(if include == 1 & mujer == 0): mean schoolyears rhwage, over(metrocat mig)
...
*Same averages for the whole population*
gen include2 = 1 if  edad > 15 & edad < 66 
svy, subpop(if include2 == 1 & mujer == 0): prop metrocat
svy, subpop(if include2 == 1 & mujer == 0): prop metrocat, over(mig)
svy, subpop(if include2 == 1 & mujer ==0 ): mean schoolyears rhwage, over(metrocat mig)
