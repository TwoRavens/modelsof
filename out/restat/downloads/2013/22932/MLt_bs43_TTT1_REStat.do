...
use "O:\tekes\newdata\est_y_99_TTT1_REStat.dta"
...
4. estimation of the the application and R&D investment equations simultaneously by heckman ml method.
*/

* generate counter for bs-rounds
gen laskuri = .
gen yhati1 = .
gen yhati2 = .
gen yhati3 = .
...
/* 15/12/03 checked that having the non-applicants in the sample does NOT bias results */
sum laskuri
...
replace yhati1=_b[_cons]+_b[ika]*ika+_b[lnhlo]*lnhlo/*
*/+_b[lvh]*lvh+_b[sme]*sme+_b[emo_y]*emo_y+_b[hak_aik]*hak_aik
replace yhati2 =_b[tj_pj]*tj_pj+_b[hallkoko]*hallkoko+_b[exporter]*exporter/*
*/+_b[food]*food+_b[paper]*paper+_b[chemi]*chemi+_b[rubber]*rubber+_b[metals]*metals+_b[electric]*electric+_b[radiotv]*radiotv
replace yhati3 =_b[otherman]*otherman+_b[datapro]*datapro+_b[r_d]*r_d+_b[laani02]*laani02+_b[laani03]*laani03/*
*/+_b[laani04]*laani04+_b[laani056]*laani056
gen yhat00 = yhati1 + yhati2 + yhati3
...
replace lnlnes = .
replace lnlnes = ln(lnes)

/*4. OWN SAMPLE SELECTION MODEL (R&D AND APPLICATION EQUATIONS)*/
sum laskuri
heckman lnappl $rhst4, select(hakija = $rhst4 sme lnlnes)
matrix b = e(b)
/*retrieve the coefficients of the application equation*/
matrix probb`i' = b[1, e(df_m)+2..e(k)-2]
/*retrieve the coefficients of the investment equation*/
matrix mb1`i' = b[1, 1..e(df_m)+1]
/*retrieve the covariance structure*/
scalar isig_nu = b[1,e(k)-3]
scalar rho = e(rho) 
scalar s_eps = e(sigma)
matrix cov`i' = [isig_nu, s_eps, rho]
scalar drop rho s_eps isig_nu

drop ph* pr* coeffs* lnes es_i
