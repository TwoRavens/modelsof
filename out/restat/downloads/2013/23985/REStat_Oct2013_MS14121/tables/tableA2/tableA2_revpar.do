...
* Generate means by hotel

	xi i.m_tour
	foreach v of varlist nrooms age _Im_tour_* {
...
* Define independent variables		
	...
	local indepvars franchised nrooms age h_rentcar h_resto h_ac ///
		...
		m_pop m_income i.brand_id i.m_tour i.m_hcompdec i.m_resto i.monthdate
		
	local meanvars avnrooms avage av_Im_tour_*
	
	
* Run regressions and save estimates for later

	reg revpar `indepvars' , cluster(hotel_id)
	...
	reg revpar `indepvars' `meanvars' , cluster(hotel_id)
	...
	xtreg revpar `indepvars' `meanvars' , re vce(robust)
