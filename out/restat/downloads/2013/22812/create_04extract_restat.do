/* this file creates the dataset from the 2004 NPSAS */
cd D:\NCESData\cadena-benkeys\CadenaKeysNPSAS\statadata

log using extract04_restat.txt, text replace

clear
set mem 200m
set more off

/* the dataset "merged" has raw data from a NPSAS0304 extract */
...
gen ccbalanceyesno = (crbaldue>0)

/* whether parents provide ANY assistance */
gen parentshelp = (parhelpa==1 | parhelpb==1 | parhelpc==1 | parhelpd==1)
gen nontuithelp = (parhelpa==1 | parhelpb==1 | parhelpc==1)
...
gen smartparent = .
replace smartparent= 0 if pareduc==1 | pareduc==2
replace smartparent = 1 if pareduc>2 & pareduc!=.
...
/*Parental Income*/
gen highinc = 0
replace highinc = 1 if depinc > 50000
...
gen notinterested = ( c04038==2)
replace notinterested = . if c04038 == -9
