...
local result_dir "F:\NETSData2007\Generated_files\SB05\Results5b"
local temp_dir "F:\NETSData2007\Generated_files\SB05\Temp5b"
...
*********************
*NO BIRTHS OPTION 10/10/2008
*********************

local nobirths_option = 1
* local nobirths_option = 0
...
local log_file "`log_dir'\sb05_usa_copy5b.log"
...
if `nobirths_option' == 1 {
	******************
	***Drop Births. 10/10/08 This is to address referee question. Excludes births from data set.
	******************
	sort dunsnumber year
	drop if emp==0 & F.emp>0 & F.emp~=. 
}
...
if `nobirths_option' == 1 {
	...
	***Drop Births. 10/10/08 This is to address referee question. Excludes births from data set.
	******************
	drop if firmemp==0 & F.firmemp>0 & F.firmemp~=. 
}
...
drop if emp==0 & L.emp==0
* this command replaces the "drop if avgemp==0" command in this section in previous iterations of code.
