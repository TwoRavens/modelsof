...
xi: reg pcm coffeeRaw i.brand i.retailer

gen pcm2=pcm+A[1,1]*0.10*coffeeRaw
...
forvalues j=1/$nb_e {
	mat SP2`j'PG=DSPP1*IR`j'${tt}*obj${tt}
			}

forvalues i=2/$nb_p {	
	forvalues j=1/$nb_e {
		mat SP2`j'P`i'=DSPP`i'*IR`j'${tt}*obj${tt}
...
forvalues i=1/$nb_m {
		mat SPI`i'=IF`i'${tt}*P2W*sp_star*IF`i'${tt}
        	mata s=st_matrix("SPI`i'")
        	mata s=qrinv(s)
        	mata st_matrix("SPII`i'", s) 
		mat drop SPI`i'
		mat objM`i'${tt}=-SPII`i'*(IF`i'${tt}*P2W*sj_p_star+ IF`i'${tt}*P2W*sp_star*((I($nb_p)-IF`i'${tt})*obj${tt}))
		mat drop SPII`i'
		} 
...
save simulRPM_WRPM_cost_RC.dta,replace
