...
forvalues i=1/7 {
	mat IF`i'${tt}=J($nb_p,$nb_p,0)
	mat IFU`i'${tt}=J($nb_p,$nb_b,0)
		}
local i=1
while `i'<=$nb_p {
	global a=mod(`i',7)
...
forvalues j=1/$nb_e{
	...
	mat drop SP2`j'PG
	matginv(SP2`j'PGSP), ginv(SP2`j'PGSPI)
	mat drop SP2`j'PGSP
...
save simulRPM_WRPM_unif_premerger_cost10.dta,replace
