...
/***Retailer margins: Bertrand Nash***/

forvalues j=1/$nb_e {
    forvalues t=1/$T {
        mat DSI`j'`t'=IR`j'`t'*DS`t'*IR`j'`t'
        mata s=st_matrix("DSI`j'`t'")
        mata s=qrinv(s)
        mata st_matrix("DSII`j'`t'", s)
        mat drop DSI`j'`t'
        mat gamma2D`j'`t'= -DSII`j'`t'*IR`j'`t'*sj_p_star[1...,`t']
...
foreach E of numlist 1/$nb_e {
    replace gamma2D_MS=gamma2D`E' if retailer==`E'
    drop gamma2D`E' 
}
forvalues t=1/$T {
    gen gamma2D`t'_MS=gamma2D_MS if (_n>=((`t'-1)*$nb_p)+1) & (_n<=$nb_p*`t')
    mkmat gamma2D`t'_MS if gamma2D`t'_MS!=.
    drop gamma2D`t'_MS
        }

gen Pgamma2D_MS=gamma2D_MS/price*100
...
forvalues t=1/$T {
    di "t vaut " `t'
    forvalues j=1/$nb_e {
        mat SP2`j'P1`t'=D`t'SPP1*IR`j'`t'*gamma2D`t'_MS
            }
    ...
    forvalues i=2/$nb_p {   
        forvalues j=1/$nb_e {
            mat SP2`j'P`i'`t'=D`t'SPP`i'*IR`j'`t'*gamma2D`t'_MS
            }
    ...
    forvalues j=1/$nb_e{
        mata s=st_matrix("SP2`j'PGSP`t'")
        mata s=qrinv(s)
        mata st_matrix("SP2`j'PGSPI`t'", s)
...
forvalues i=1/$nb_m {
    ...
    forvalues t=1/$T {
        ...
        mata s=st_matrix("P2WSP`i'`t'")
        mata s=qrinv(s)
        mata st_matrix("P2WSPI`i'`t'", s)

        mat drop P2WSP`i'`t'
        mat gamma2P`i'`t'=-P2WSPI`i'`t'*IF`i'`t'*sj_p_star[1...,`t']
        mat drop P2WSPI`i'`t'
...
gen gamma6P_MS=.
forvalues i=1/$nb_m {
    replace gamma6P_MS=gamma6P`i' if manufacturer==`i' 
            }
gen Pgamma6P_MS=gamma6P_MS/price*100 
...
forvalues s=1/$ns {
    ...
    forvalues k=1/$nb_p {
        mat A`k'=V
        mat A`k'[`k',1]=1              
        mat DSPP`k'=DSPP`k'+(1/$ns)*(($beta+$sigma*rdraws`t'[`k',`s'])^2)*sj_p_star`s'[`k',1]*(hadamard(2*hadamard(sj_p_star`s'*(sj_p_star`s')',C)-hadamard(sj_p_star`s'*(J($nb_p,1,1)-2*sj_p_star`s')',B),A-(A`k'*U+U'*(A`k')'-(A`k')*(A`k')'))-hadamard((hadamard(sj_p_star`s'*(J($nb_p,1,1)-2*sj_p_star`s')',U'*(A`k')')+hadamard((J($nb_p,1,1)-2*sj_p_star`s')*(sj_p_star`s')',(A`k')*U)),A-(A`k'*(A`k')'))+hadamard((J($nb_p,1,1)-sj_p_star`s')*(J($nb_p,1,1)-2*sj_p_star`s')',(A`k')*(A`k')'))
                }
...
forvalues j=1/$nb_e {
    mat tata=IR`j'${tt}*sp_star*IR`j'${tt}

    mata s=st_matrix("tata")
    mata s=qrinv(s)
    mata st_matrix("DSII`j'", s) 

    mat drop tata
    mat objR`j'${tt}= -DSII`j'*IR`j'${tt}*sj_p_star
            }


forvalues j=1/$nb_e {
    mat SP2`j'PG=DSPP1*IR`j'${tt}*objR`j'${tt}
            }

forvalues i=2/$nb_p {   
    forvalues j=1/$nb_e {
        mat SP2`j'P`i'=DSPP`i'*IR`j'${tt}*objR`j'${tt}
        mat SP2`j'PG=SP2`j'PG,SP2`j'P`i'
        mat drop SP2`j'P`i'
                }
        }
forvalues j=1/$nb_e {
    mat SP2`j'PGSP=SP2`j'PG*IR`j'${tt} + sp_star*IR`j'${tt} + IR`j'${tt}*(sp_star)'*IR`j'${tt}
    mat drop SP2`j'PG

    mata s=st_matrix("SP2`j'PGSP")
    mata s=qrinv(s)
    mata st_matrix("SP2`j'PGSPI", s) 

    mat drop SP2`j'PGSP
    mat PW`j'=IR`j'${tt}*sp_star*IR`j'${tt}*SP2`j'PGSPI
    mat drop SP2`j'PGSPI
            }

mat P2W1=PW1[1..$nb_b,1..$nb_p]
mat P2W=P2W1
forvalues j=2/$nb_e {
    mat P2W`j'=PW`j'[(`j'-1)*$nb_b+1..`j'*$nb_b,1..$nb_p]
    mat P2W=P2W\P2W`j'
            }
        
forvalues i=1/$nb_m {
    mat P2WSP`i'=IF`i'${tt}*P2W*sp_star*IF`i'${tt}

    mata s=st_matrix("P2WSP`i'")
    mata s=qrinv(s)
    mata st_matrix("P2WSPI`i'", s) 
...
forvalues i=1/$nb_e {
    mat obj${tt}=obj${tt}+IR`i'${tt}*objR`i'${tt}
        }
...
save simulRPM_LP_MS.dta,replace
