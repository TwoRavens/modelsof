...
forvalues j=1/$nb_e{
	...
	mat drop SP2`j'PGSP
	mat PW`j'=(IRU`j'${tt})'*(sp_star)'*IR`j'${tt}*SP2`j'PGSPI
	mat drop SP2`j'PGSPI
			}


mat P2W1=PW1[1..$nb_b,1..$nb_b]
mat P2UW=P2W1
forvalues j=2/$nb_e {
	mat P2W`j'=PW`j'[1..$nb_b,(`j'-1)*$nb_b+1..`j'*$nb_b]
	mat P2UW=P2UW,P2W`j'
		}

		
forvalues i=1/$nb_m {
	mat SPI`i'=IFU`i'${tt}*P2UW*sp_star*IFU`i'${tt}
        mata s=st_matrix("SPI`i'")
        mata s=qrinv(s)
        mata st_matrix("SPII`i'", s) 
	mat drop SPI`i'
	mat objM`i'${tt}=-SPII`i'*(IFU`i'${tt}*P2UW*sj_p_star+ IFU`i'${tt}*P2UW*sp_star*((I($nb_p)-IF`i'${tt})*obj${tt}))
	mat drop SPII`i'
...
save simulRPM_WRPM_unif_cost10_RC.dta,replace
