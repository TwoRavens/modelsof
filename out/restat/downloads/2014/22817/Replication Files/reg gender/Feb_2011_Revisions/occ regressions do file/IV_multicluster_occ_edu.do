...
clear
cd "C:\Users\kp\Documents\thesis_topic occdist\reg_gender"
log using "Feb_2011_Revisions\log\IV_`sx'_occ_edu_3clust_omb_obs`xobs'_`yr'.log", replace  
...
sort occ1990 pwmetro bpl
merge occ1990 pwmetro bpl using `sx'_census`ivyr'_occdist_us_occ.dta
drop if _merge==2
drop _merge
...
forvalues pop= 1(1)5 { 
	...
	if `pop'==1 & `yr'==00 {
	outreg p_occ_old_country p_nat_occ p_occmet cntry_met mean_occedu `controls' using "Feb_2011_Revisions\reg_output\output_occ_edu\IV_`sx'_occ_edu_obs`xobs'_3clust_omb.xls", bdec(3) rdec(3) sigsymb(**,*,+) 10pct coefastr ctitle(occpop) bracket replace
	}
	else { 
	outreg p_occ_old_country p_nat_occ p_occmet cntry_met mean_occedu `controls' using "Feb_2011_Revisions\reg_output\output_occ_edu\IV_`sx'_occ_edu_obs`xobs'_3clust_omb.xls", bdec(3) rdec(3) sigsymb(**,*,+) 10pct coefastr ctitle(occpop) bracket append
	}
	...
	if `pop'==1 & `yr'==00 {
	outreg p_occ_old_country p_nat_occ p_occmet cntry_met mean_occedu `controls' using "Feb_2011_Revisions\reg_output\output_occ_edu\IV_`sx'_occ_edu_obs`xobs'_3clust_omb_nomex.xls" , bdec(3) rdec(3) sigsymb(**,*,+) 10pct coefastr ctitle(occpop) bracket replace
	}
	else { 
	outreg p_occ_old_country p_nat_occ p_occmet cntry_met mean_occedu `controls' using "Feb_2011_Revisions\reg_output\output_occ_edu\IV_`sx'_occ_edu_obs`xobs'_3clust_omb_nomex.xls" , bdec(3) rdec(3) sigsymb(**,*,+) 10pct coefastr ctitle(occpop) bracket append
	}	
