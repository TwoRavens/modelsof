...
clear
log using "Feb_2011_Revisions\log_lowedu\OLS_`sx'_edu_3clust_omb_obs`xobs'_`yr'.log", replace  
...
/***************** OLS ******************/
if `yr'==80 {
local controls "cntry_met age age2 male english edu2 marst2 marst3 marst4 marst5 marst6"
}
else {
local controls "cntry_met age age2 male english edu2 marst2 marst3 marst4 marst5 marst6"
}
forvalues pop= 1(1)5 { 
	...
	if `pop'==1 & `yr'==00 {
	outreg p_occ_old_country p_nat_occ p_occmet mean_occedu `controls' using "Feb_2011_Revisions\reg_output_lowedu\output_edu\OLS_`sx'_edu_obs`xobs'_3clust_omb.xls", bdec(3) rdec(3) sigsymb(**,*,+) 10pct coefastr ctitle(occpop) bracket replace
	}
	else { 
	outreg p_occ_old_country p_nat_occ p_occmet mean_occedu `controls' using "Feb_2011_Revisions\reg_output_lowedu\output_edu\OLS_`sx'_edu_obs`xobs'_3clust_omb.xls", bdec(3) rdec(3) sigsymb(**,*,+) 10pct coefastr ctitle(occpop) bracket append
	}
	/* no mex */
	...
	if `pop'==1 & `yr'==00 {
	outreg p_occ_old_country p_nat_occ p_occmet mean_occedu `controls' using "Feb_2011_Revisions\reg_output_lowedu\output_edu\OLS_`sx'_edu_obs`xobs'_3clust_omb_nomex.xls", bdec(3) rdec(3) sigsymb(**,*,+) 10pct coefastr ctitle(occpop) bracket replace
	}
	else { 
	outreg p_occ_old_country p_nat_occ p_occmet mean_occedu `controls' using "Feb_2011_Revisions\reg_output_lowedu\output_edu\OLS_`sx'_edu_obs`xobs'_3clust_omb_nomex.xls", bdec(3) rdec(3) sigsymb(**,*,+) 10pct coefastr ctitle(occpop) bracket append
	}
