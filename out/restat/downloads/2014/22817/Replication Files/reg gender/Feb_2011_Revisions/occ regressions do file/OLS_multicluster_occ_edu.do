...
log using "reg_gender\Feb_2011_Revisions\log\OLS_`sx'_occ_edu_3clust_omb_obs`xobs'_`yr'.log", replace  
cd "C:\Users\kp\Documents\thesis_topic occdist\reg_gender"
...
/***************** OLS ******************/
if `yr'==80 {
local controls "cntry_met age age2 male english edu2 edu3 edu4 marst2 marst3 marst4 marst5 marst6"
}
else {
local controls "cntry_met age age2 male english edu2 edu3 edu4 edu5 edu6 marst2 marst3 marst4 marst5 marst6"
}
forvalues pop= 1(1)5 { 
	...
	xi: cgmreg occpop p_occ_old_country p_nat_occ p_occmet mean_occedu `controls' i.bpl i.pwmetro i.region, cluster(pwmetro occ1990 bpl)
	if `pop'==1 & `yr'==00 {
	outreg p_occ_old_country p_nat_occ p_occmet mean_occedu `controls' using "Feb_2011_Revisions\reg_output\output_occ_edu\OLS_`sx'_occ_edu_obs`xobs'_3clust_omb.xls", bdec(3) rdec(3) sigsymb(**,*,+) 10pct coefastr ctitle(occpop) bracket replace
	}
	else { 
	outreg p_occ_old_country p_nat_occ p_occmet mean_occedu `controls' using "Feb_2011_Revisions\reg_output\output_occ_edu\OLS_`sx'_occ_edu_obs`xobs'_3clust_omb.xls", bdec(3) rdec(3) sigsymb(**,*,+) 10pct coefastr ctitle(occpop) bracket append
	}
	/* no mex */
	display "Excluding Mexicans, for popular occuption: `pop'"
	xi: cgmreg occpop p_occ_old_country p_nat_occ p_occmet mean_occedu `controls' i.bpl i.pwmetro i.region if bpl!=200, cluster(pwmetro occ1990 bpl)
	if `pop'==1 & `yr'==00 {
	outreg p_occ_old_country p_nat_occ p_occmet mean_occedu `controls' using "Feb_2011_Revisions\reg_output\output_occ_edu\OLS_`sx'_occ_edu_obs`xobs'_3clust_omb_nomex.xls", bdec(3) rdec(3) sigsymb(**,*,+) 10pct coefastr ctitle(occpop) bracket replace
	}
	else { 
	outreg p_occ_old_country p_nat_occ p_occmet mean_occedu `controls' using "Feb_2011_Revisions\reg_output\output_occ_edu\OLS_`sx'_occ_edu_obs`xobs'_3clust_omb_nomex.xls", bdec(3) rdec(3) sigsymb(**,*,+) 10pct coefastr ctitle(occpop) bracket append
	}
