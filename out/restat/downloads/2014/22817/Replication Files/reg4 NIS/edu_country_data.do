...
generate pop=1;
collapse (sum) perwt pop wt_sum (mean) edu, by (state occ1990 ciscobinsmo);

by state occ1990, sort: egen mean_occ_edu=mean(edu);
by state occ1990, sort: egen sum_statocc1_wt=sum(wt_sum);
by state occ1990, sort: egen sum_statocc2_wt=sum(perwt);
by state occ1990, sort: generate mean_occ_edu_wt=sum_statocc1_wt/sum_statocc2_wt;
generate mean_edu_wt= wt_sum/ perwt;
rename edu mean_edu;
keep  occ1990 state  ciscobinsmo mean_edu mean_edu_wt mean_occ_edu mean_occ_edu_wt;
sort ciscobinsmo occ1990 state;
generate year=2000;
save "reg4 NIS\census00_edu_country.dta", replace;
...
sort ciscobinsmo occ1990 state;
generate year=1990;
save "reg4 NIS\census90_edu_country.dta", replace;
...
sort ciscobinsmo occ1990 state;
generate year=1980;
save "reg4 NIS\census80_edu_country.dta", replace;

append using "reg4 NIS\census90_edu_country.dta";
append using "reg4 NIS\census00_edu_country.dta";
sort ciscobinsmo occ1990 state year;
save "reg4 NIS\census_edu_country.dta", replace;
