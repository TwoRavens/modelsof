...
#delimit;
cap n log close; log using ./logdir/p-prep5-ran-do2a.log, replace;
...
sort scat; list; keep if (scat>=14 & scat<=14);
...
foreach KKA of local scatindexA {;
  use patent ctryn ayear scat uspc
    if ctryn=="US" & (scat!=. & uspc!=.) & (ayear>=1990 & ayear<=1999) & scat==`KKA'
    using ./svdata/p-work-pat_2009_short.dta, clear;
  egen temp1=count(patent);
  levelsof temp1, local(ctindexA);
forvalues LLA=156(1)1000 {;
  use zip if [_n]<`ctindexA' using ./svdata-do/random1/do-patent-list-`LLA', clear;
  gen ct1=1; collapse (sum) ct1, by(zip) fast;
  save ./svdata-do/random1/do-temp1A, replace;
  ren zip zip2; ren ct1 ct2;
  cross using ./svdata-do/random1/do-temp1A.dta;
  erase ./svdata-do/random1/do-temp1A.dta;
  ren zip zip1;
  ...
  compress; sort dist; 
  cap n erase ./svdata-do/random2/do-dist-cross-`KKA'-`LLA'.dta.gz;
  save ./svdata-do/random2/do-dist-cross-`KKA'-`LLA', replace;
  !gzip ./svdata-do/random2/do-dist-cross-`KKA'-`LLA'.dta;
};
};
