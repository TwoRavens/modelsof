...
#delimit;
cap n log close; log using ./logdir/p-prep5-ran-do2f.log, replace;
...
sort scat; list; keep if (scat>=60 & scat<=69);
levelsof scat, local(scatindexF);

*********************************;
** Combine 1000 random draws   **;
*********************************;

foreach KKF of local scatindexF {;
  use patent ctryn ayear scat uspc
    if ctryn=="US" & (scat!=. & uspc!=.) & (ayear>=1990 & ayear<=1999) & scat==`KKF'
    using ./svdata/p-work-pat_2009_short.dta, clear;
  egen temp1=count(patent);
  levelsof temp1, local(ctindexF);
forvalues LLF=1(1)1000 {;
  use zip if [_n]<`ctindexF' using ./svdata-do/random1/do-patent-list-`LLF', clear;
  gen ct1=1; collapse (sum) ct1, by(zip) fast;
  save ./svdata-do/random1/do-temp1F, replace;
  ren zip zip2; ren ct1 ct2;
  cross using ./svdata-do/random1/do-temp1F.dta;
  erase ./svdata-do/random1/do-temp1F.dta;
  ren zip zip1;
  ...
  compress; sort dist; 
  cap n erase ./svdata-do/random2/do-dist-cross-`KKF'-`LLF'.dta.gz;
  save ./svdata-do/random2/do-dist-cross-`KKF'-`LLF', replace;
  !gzip ./svdata-do/random2/do-dist-cross-`KKF'-`LLF'.dta;
};
};
