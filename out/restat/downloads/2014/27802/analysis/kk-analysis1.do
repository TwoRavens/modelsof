...
cap n log close; log using kk-analysis1.log, replace;
...
*** Open KD materials;
use  ./data/do-scat-kd, clear;
sort scat; merge scat using  ./data/scat-ct; drop _m;
...
*** Cross-sectional data for graphs;
gen denscap=dens; replace denscap=0.5 if dens>0.5;
gen misc=(scat==19 | scat==39 | scat==49 | scat==59 | scat==69);

*** Reshape to long format and merge LCI;
reshape long kd_ct kdt_ct, i(scat) j(dist);
sort scat dist; merge scat dist using ./data/do-ran-density-lci.dta; drop _m;

*** Generate deviations;
gen Loc5=kd_ct-p95_kd_ct; 
gen Loc1=kd_ct-p99_kd_ct; 
gen Dis5=p05_kd_ct-kd_ct; 
gen Dis1=p01_kd_ct-kd_ct;
for var Loc* Dis*: replace X=0 if X<0;
for any 1 5: replace DisX=0 if DisX>0 & LocX>0;
gen Com5=Loc5-Dis5;
gen Com1=Loc1-Dis1;
...
drop if dist>1000;
gen cat=int(scat/10);
gen cat2=1 if cat==1 | cat==3;
replace cat2=2 if cat==2 | cat==4;
replace cat2=3 if cat==5 | cat==6;

*** Figures 5a and 5b;
gen kd_ctx=kd_ct*1000;
gen QLoc5=(Loc5>0); gen QLoc1=(Loc1>0);
table dist cat2, c(mean kd_ctx) f(%5.4f);
table dist cat2, c(mean QLoc1) f(%5.4f);
drop kd_ctx;
drop if dist>250;

*** Merge in controls;
sort scat; merge scat using ./data/control1-mainmsa, keep(wageav) nok; drop _m;
sort scat; merge scat using ./data/control1-patextra2, keep(sh*) nok; drop _m;
for var wageav sh*: egen temp1=median(X) \ gen UX=(X>temp1) \ drop temp1;

*** Transform to (0,1);
log off;
for var Loc* Dis* Com* kd* rad* sh*: egen temp1=std(X) \ replace X=temp1 \ drop temp1;
log on;
...
*** Prepare distance bands;
log off;
gen ZD1=(dist==1);
forvalues CC=5(5)250 {;
gen ZD`CC'=(dist==`CC');
}; 
log on;

*** Figures 6 and 7;
for var ZD*: replace X=X*radus1;
areg kd_ct ZD*, a(dist) cl(scat);
areg Com5 ZD*, a(dist) cl(scat);
for var ZD*: replace X=raduk if X!=0;
areg kd_ct ZD*, a(dist) cl(scat);
areg Com5 ZD*, a(dist) cl(scat);
drop ZD*;
...
*** Prepare smaller distance bands;
for any 25 50 75 100 125 150 175 200 225: gen ZDX=(dist>X-25 & dist<=X);
for any 25 50 75 100 125 150 175 200 225: gen ZODX=ZDX*shinzip \ gen ZJDX=ZDX*shjacobs \ gen ZYDX=ZDX*shyoung;
xi i.dist;

*** Graphs;
for var ZD*: replace X=X*radus1;
areg kd_ct ZD*, a(dist) cl(scat);
areg kd_ct ZD* [aw=wt], a(dist) cl(scat); 
areg kd_ct ZD* if misc==0, a(dist) cl(scat);
reg kd_ct ZD* _I*, vce(bootstrap, seed(1) reps(100));
bysort Ushinzip: areg kd_ct ZD*, a(dist) cl(scat);
bysort Ushjacob: areg kd_ct ZD*, a(dist) cl(scat);
bysort Ushyoung: areg kd_ct ZD*, a(dist) cl(scat);
bysort Uwageav: areg kd_ct ZD*, a(dist) cl(scat);

areg Com5 ZD*, a(dist) cl(scat);
areg Com5 ZD* if misc==0, a(dist) cl(scat); 
areg Com5 ZD* [aw=wt], a(dist) cl(scat);
reg Com5 ZD* _I*, vce(bootstrap, seed(1) reps(100));
bysort Ushinzip: areg Com5 ZD*, a(dist) cl(scat);
bysort Ushjacob: areg Com5 ZD*, a(dist) cl(scat);
bysort Ushyoung: areg Com5 ZD*, a(dist) cl(scat);
bysort Uwageav: areg Com5 ZD*, a(dist) cl(scat);

**************;
** US RAD2 ***;
**************;

for var ZD*: replace X=-radus2 if X!=0;
areg kd_ct ZD*, a(dist) cl(scat);
areg Com5 ZD*, a(dist) cl(scat);
...
for var ZD*: replace X=radus3 if X!=0;
areg kd_ct ZD*, a(dist) cl(scat);
areg Com5 ZD*, a(dist) cl(scat);
...
for var ZD*: replace X=-raduk2 if X!=0;
areg kd_ct ZD*, a(dist) cl(scat); 
areg Com5 ZD*, a(dist) cl(scat); 
