...
***USES ALTERNATIVE MATCHING OF TEACHERS TO STUDENTS WHERE FOR STANFORD TESTS - THE TEST IN YEAR t APPLIES TO TEACHERS IN SPRING OF t - 1 and FALL OF t
clear all
set mem 2g
set more off
cap log close
...
***ADJUST TEACHER-STUDENT LINKS SO THAT THE STUDENT IS LINKED TO THE TEACHER FROM THE SPRING AND FALL SEMESTERS PRIOR TO STANFORD ADMINISTRATION IN JANUARY
use "C:\D\Research\Charter\Houston\HISDdata\DataFiles\SecondGrades\secondary_classes.dta", clear

	**REPLACE YEAR t-1 WITH t FOR SPRING SEMESTER
	replace year = year + 1 if term == "S"
	sort id year

	save secondary_classes_alt.dta, replace
...
*LIMIT TO GRADES 9 - 11
cap rm teachers_share_alt.txt
cap rm teachers_share_alt.xml
...
*MERGE IN STUDENT-TEACHER LINKS
	sort id campus year
	merge 1:m id campus year using "C:\teacher free riding\secondary_classes_alt.dta", keepusing(tch_number course_type course crs_title)
	drop if _merge == 2
...
drop _merge
merge campus year grade course_type_a tch_number using "C:\teacher free riding\grade_percentages_alt.dta"
drop if _merge == 2
...
outreg2 post_share_students share_students using teachers_share_alt, excel dec(3) nocons ctitle("English")
...
outreg2 post_share_students share_students using teachers_share_alt, excel dec(3) nocons ctitle("Science")
...
outreg2 post_share_students share_students using teachers_share_alt, excel dec(3) nocons ctitle("Social Studies")
...
outreg2 post_share_students share_students using teachers_share_alt, excel dec(3) nocons ctitle("Math - Stanf")
