...
local aggprices			"aggprices_lc.csv"
...
*****************************************************************************
*** Insheeting xr data, creating date variable. Merged in below
...
*****************************************************************************
*** Insheeting aggregate price data for 11 countries + US, formatting date variable
insheet using "$datapath1/`aggprices'", comma names clear
drop if code == ""									// Dropping blank rows
gen date2 = date(code, "MDY")								// Generating date variable
gen mo = month(date2)
gen yr = year(date2)
...
merge date using "`xr'"
tab _merge
keep if _merge==3						// XR data on dates for which we don't have prices
drop _merge
...
reshape long p_ s_, i(date) j(country) string
sort country date
*****************************************************************************

drop if country == "ie" | country=="us"	// Dropping Ireland (no data) and US (can't have pair with itself)
drop if country == "fl"				// Dropping Finland (so as to be comparable with disaggregate data)
...
sort pairid
drop country weight
reshape wide p_ s_, i(date) j(pairid)
sort date


save "$outpath1/imbsdata_agg_lc_wide.dta", replace
