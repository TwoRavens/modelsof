...
local programpath "P:\BerginGlickWu Replication\Table 6\programs\6a"
local outpath1 "P:\BerginGlickWu Replication\Table 6\results\6a"
...
local filename "ecm_newPT3_demeanq_semiannual_PW_f1_indust_averages.csv"
local irfcoeffs "irfcoeffs_newPT3_demeanq_semiannual_PW_f1_indust.dta"

matrix avg_stat1_ = J(101,47,.)
matrix avg_stat2_ = J(101,47,.)

	use "`datapath'/$datasetPWT", clear
	ccep_ecm_newPT3_demeanq, group(`numpairs') goods(`numgoods') nirp(`nirp') tsobs(`tsobs')
	...
	#delimit ;
	collapse pairs 
	b_coef_d0s_Q1 t_stat_d0s_Q1 
	b_coef_d0s_qk1 t_stat_d0s_qk1
	b_coef_d0P_Q1 t_stat_d0P_Q1
	b_coef_d0P_qk1 t_stat_d0P_qk1
	b_coef_d0pk_Q1 t_stat_d0pk_Q1
	b_coef_d0pk_qk1 t_stat_d0pk_qk1
	;
	#delimit cr
	...
	preserve
	collapse (sd) b_coef_d0s_Q1 b_coef_d0s_qk1 b_coef_d0pk_Q1 b_coef_d0pk_qk1 b_coef_d0P_Q1 b_coef_d0P_qk1 
	...
	merge 1:1 m using `averages'
	order avg_pairs avg_*
	
	**Observations, time period =33 * pairs
	gen avg_observations = avg_pairs*33
	drop m _merge
	order avg_observations
