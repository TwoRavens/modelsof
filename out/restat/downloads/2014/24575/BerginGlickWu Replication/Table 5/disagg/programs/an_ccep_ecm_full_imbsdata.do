...
*/
*** Note: this file uses the Imbs et al sectoral data: 11 countries(end up dropping Finland, so 10), 19 sectors
...
local programpath "P:\BerginGlickWu Replication\Table 5\disagg\programs"
local outpath1 "P:\BerginGlickWu Replication\Table 5\results"
local datapath1 "P:\BerginGlickWu Replication\data_creation\datasets"

global ImbsDataset "imbsdata_sectoral_lc_wide.dta"
...
local numsectors = 19
local numcountries = 11
local type "CCEP ECM"

**************************************************
*						 *
****************** Estimation on Imbs sectoral panel
*						 *
**************************************************


local filename "ecm_full_imbsdata.csv"

capture program drop _all
matrix avg_stat1_ = J(`numsectors',11,.)
matrix avg_stat2_ = J(`numsectors',11,.)

forvalues i = 1/`numsectors' {
	global j = `i'
	use sectorid date p_* s_* if sectorid == `i' using "`datapath1'/$ImbsDataset", clear
