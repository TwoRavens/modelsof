...
*Programs for Table 2 where labor is assumed to be homogeneous; lags of labor as instruments


use basefile.dta
run makepolynomialv5_.ado
	//calls program to construct polynomial of a list of variables
run acf_train_bootstrapped_lag.do
...
*Preparing Data	
	...
	*****create year dummies
	...
	gen trainlshare_lag2 = l2.trainlshare
...
*Executing Program
	*All sectors pooled
		set seed 123456789
		bootstrap  "acf_trainingbootstrap, exog(k k_lag l_lag l_lag2 trainlshare)" r(btrain_acfGMMfix) r(bk_acfGMMfix) r(bl_acfGMMfix) /*
					*/   r(btrainl_olsm) /*
					*/ r(bl_olsm) r(bk_olsm) r(btr_wagebase) r(btr_wageaug) r(bcl_wageaug) r(btfp_wageaug) /*
					*/ , reps(500) cluster(mark) dots saving(bootstrapiterations_all)
	...
	*Manufacturing sectors		
		...
		bootstrap  "acf_trainingbootstrap, exog(k k_lag l_lag l_lag2 trainlshare)" r(btrain_acfGMMfix) r(bk_acfGMMfix) r(bl_acfGMMfix) /*
					*/    r(btrainl_olsm) /*
					*/ r(bl_olsm) r(bk_olsm) r(btr_wagebase) r(btr_wageaug) r(bcl_wageaug) r(btfp_wageaug) /*
					*/ , reps(500) cluster(mark) dots saving(bootstrapiterations_manuf)

				/*
		This results in the following estimates used in Table 2 ("Manufacturing" columns):
		Production function:
			bl_olsm is the labor coefficient for OLS2
			bk_olsm is the capital coefficient for OLS2
			btrainl_olsm is the training coefficient for OLS2, used to compute beta_T
	...
	*Non-manufacturing sectors	
		...
		/*
		This results in the following estimates used in Table 2 ("Non-Manufacturing" columns):
		Production function:
			bl_olsm is the labor coefficient for OLS2
			bk_olsm is the capital coefficient for OLS2
			btrainl_olsm is the training coefficient for OLS2, used to compute beta_T
