*difference with acf_train_bootstrapped is that on line 20, the excluded observations include 
*as well the observations for which the lags of the variables are missing
...
program define acf_trainingbootstrap, rclass
	...
	***********making data for second step
	...
	gen `tag'  = missing(`phihat_lag', k , k_lag,  l, l_lag2, av, m, trainlshare , trainlshare_lag ) 
	

	****GMM

	gen `sample' = missing(`phi_hat', `phihat_lag', k, l, av, l_lag, l_lag2, k_lag,  trainlshare, trainlshare_lag)
	replace `sample' = 1 - `sample'
	quietly: reg av l k trainlshare ydum* ndum* if `sample' , cluster(mark)
	esttab , keep(l k trainlshare) r2 se
	...
	mata: acftrain1(("`exog'"), "`phi_hat'" , "`phihat_lag'", "k_lag" , "l_lag", "trainlshare_lag", "`sample'")
	...
	****wage regression*******
	...
	tempvar cl w tfp
	gen `tfp' = av - bl_acfGMMfix*l - bk_acfGMMfix*k - btrain_acfGMMfix*trainlshare
	gen `w' = ln(wage)
	gen `cl' = ln((tangibleassetstheur/capitaldefl)/avemplfte)
	label var `cl' "caplab"
	label var `w' "wage"
	label var `tfp' "tfp"
	quietly reg `w' trainlshare ydum* ndum* if `sample', cluster(mark)
	esttab  , drop(ndum* ydum*) se r2 scalars(N_clust) label
	return scalar btr_wagebase = _b[trainlshare]
	quietly: reg `w' trainlshare `cl' `tfp' ydum* ndum* if `sample', cluster(mark)
	esttab  , drop(ndum* ydum*) se r2 scalars(N_clust) label
...
mata
mata clear
	void acftrain1(string vector exog, string scalar phi_hat, string scalar phihat_lag, string scalar k_lag, string scalar l_lag, string scalar trainlshare_lag, string scalar sample)
		{
		external m_phihat, m_phihatlag, m_k, m_l, m_klag, m_llag, cons, m_trainlshare, m_trainlsharelag, instr, min
	...
	void acf_gmmtrain1(todo,beta,critxi,g,H) 
		{
		external m_phihat, m_phihatlag, m_k, m_l, m_klag, m_llag, m_trainlshare, m_trainlsharelag, cons , instr, min
		inputs = (m_l,m_k, m_trainlshare)
		inputslag = (m_llag, m_klag, m_trainlsharelag)
