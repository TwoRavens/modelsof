...
capture program drop acf_trainingbootstrapbediensch
program define acf_trainingbootstrapbediensch, rclass
	syntax , exog(varlist)
	...
	****GMM
	...
	gen `sample' = missing(`phi_hat', `phihat_lag', k, l, av, l_lag, k_lag, bedsh, bedsh_lag, schooling, trainlshare,trainlshare_lag )
	replace `sample' = 1 - `sample'
	quietly: reg av l k trainlshare bedsh schooling ydum* ndum* if `sample' , cluster(mark)
	...
	mata: acftrain1(("`exog'"), "`phi_hat'" , "`phihat_lag'", "k_lag" , "l_lag", "trainlshare", "trainlshare_lag", "bedsh", "bedsh_lag", "schooling", "`sample'")
	...
	return scalar bschool_acfGMMfix = paramfix[1,5]
	...
	scalar bschool_acfGMMfix = paramfix[1,5]	

	****wage regression*******

	tempvar cl w tfp
	gen `tfp' = av - bl_acfGMMfix*l - bk_acfGMMfix*k - btr_acfGMMfix*trainlshare - bbed_acfGMMfix*bedsh - bschool_acfGMMfix*schooling
	gen `w' = ln(wage)
	gen `cl' = ln((tangibleassetstheur/capitaldefl)/avemplfte)
	label var `cl' "caplab"
	label var `w' "wage"
	label var `tfp' "tfp"
	quietly reg `w' trainlshare bedsh schooling ydum* ndum* if `sample', cluster(mark)
	...
	quietly: reg `w' trainlshare bedsh schooling `cl' `tfp' ydum* ndum* if `sample', cluster(mark)
	esttab  , drop(ndum* ydum*) se r2 scalars(N_clust) label
...
mata
mata clear
	void acftrain1(string vector exog, string scalar phi_hat, string scalar phihat_lag, string scalar k_lag, string scalar l_lag, string scalar trainlshare, string scalar trainlshare_lag, string scalar bedsh, string scalar bedsh_lag, string scalar schooling, string scalar sample)
		{
		external m_phihat, m_phihatlag, m_k, m_l, m_klag, m_llag, cons, m_trainlshare, m_trainlsharelag, m_bedsh, m_bedshlag, m_schooling, instr, min
		...
		init=(st_numscalar("initl"), st_numscalar("initk"), st_numscalar("inittrainl"), st_numscalar("initbed"), st_numscalar("initschool"))	
		...
		"NACE `i'"
		"Coefficients"
		names = ("l" , "k", "trainlshare", "bedsh", "schooling")
	...
	void acf_gmmtrain1(todo,beta,critxi,g,H) 
		{
		external m_phihat, m_phihatlag, m_k, m_l, m_klag, m_llag, cons, m_trainlshare, m_trainlsharelag, m_bedsh, m_bedshlag, m_schooling, instr, min
		inputs = (m_l,m_k, m_trainlshare, m_bedsh, m_schooling)
		inputslag = (m_llag, m_klag, m_trainlsharelag, m_bedshlag, m_schooling)
