...
preserve
	...
	if  "`Dtail'" == "P99" & "`measure'"=="Killed" {
		/* drop coutries in P99 except NICA(145) */
		drop   gdppccteus_56_PL0_100 gdppccteus_85_PL0_100 gdppccteus_93_PL0_100
		egen   gdppccteus_AVG_100 = rowmean(gdppccteus_145_PL0_100)
	}

	sort  YearsFromLD
	save	 gdppccteus_AVG_`measure'_`Dtail'_NICA.dta, replace
...
preserve
	...
	if  "`Dtail'" == "P99" & "`measure'"=="Killed" {
		/* drop coutries in P99 except NICA(145)*/
		drop gdppccteus_56_PL0synth_100 gdppccteus_85_PL0synth_100 gdppccteus_93_PL0synth_100
		egen   gdppccteus_AVG_100_Synthetic = rowmean(gdppccteus_145_PL0synth_100)
	}
	
	sort  YearsFromLD
	save	 gdppccteus_AVG_synthetic_`measure'_`Dtail'_NICA.dta, replace
restore


use gdppccteus_AVG_`measure'_`Dtail'_NICA.dta
merge  YearsFromLD using gdppccteus_AVG_synthetic_`measure'_`Dtail'_NICA.dta
gen diff_AVG_100 =gdppccteus_AVG_100- gdppccteus_AVG_100_Synthetic
save fullgraph_`measure'_`Dtail'_NICA.dta, replace
	...
	#delimit ;
		...
		/*subtitle("The 1972 Earthquake in Nicaragua")*/
		title("The 1972 Earthquake in Nicaragua")
		...
		saving(AvgRealGDPpc_`measure'_`Dtail'_NICA, replace);
		graph export AvgRealGDPpc_`measure'_`Dtail'_upLead_`MaxLEAD'_NICA.eps, replace;
...
matwrite using diffs_ActualAVG_`Dtail'_NICA, replace
