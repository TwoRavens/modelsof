...
local drink_meas  = "propdaysdank_p propdaysbinged_p"
...
gen df = results_propdaysdank_p2 - 5
...
gen band = results_propdaysdank_p1/12
...
graph twoway  (scatter results_propdaysdank_p3 band, mcolor(black)  msymbol(T)  msize(small)  yscale(range(0,15)) ylabel(#3, nogrid))  ///
              (line results_propdaysdank_p3 band, lwidth(thin) lcolor(black) ) ///
			  (line propdaysdank_p_upper_CI band, lwidth(thin) lcolor(black) clpattern(dot)) ///
			  (line propdaysdank_p_lower_CI  band, lwidth(thin) lcolor(black) clpattern(dot)) ///
              (scatter results_propdaysbinged_p3 band, mcolor(red)  msymbol(S)  msize(small)) ///
              (line results_propdaysbinged_p3 band, lwidth(thin) lcolor(red)) ///	
			  (line propdaysbinged_p_upper_CI band, lwidth(thin) lcolor(red) clpattern(dot)) 	///
			  (line propdaysbinged_p_lower_CI  band, lwidth(thin) lcolor(red) clpattern(dot)) 	///
              , title("Appendix P: Percent of Days Drinking and Binge Drinking")  subtitle("Robustness to Bandwidth Choice") xtitle("Bandwidth") ///
			   ytitle("Estimate of Precent Jump at 21")  ///
			 legend(off) text(8 1.7 "Drinking", color(black)) text(3 1.7 "Binge Drinking", color(red)) ///
