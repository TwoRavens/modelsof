...
*THIS ONE HAS NO ADJUSTMENT FOR MEDIAN ACS WAGES
...
capture log close
log using buildx07-nadj.log,replace
...
**Subtract off the median in each year
egen mlinc = median(lincwgb), by(year)
egen m06linc = median(lincwgb) if year==2006
egen M06linc = max(m06linc)
replace lincwgb = lincwgb-mlinc+M06linc
drop mlinc m06linc M06linc
egen mlinc = median(lincwgb_w), by(year)
egen m06linc = median(lincwgb_w) if year==2006
egen M06linc = max(m06linc)
...
compress
saveold census07-nadj.dta, replace
