...
log using kb2016sppq_mc02.log, text replace

/* *************************************************************      */
/*  File Name:  kb2016sppq_mc02.do                                    */
...
/*              monte carlo with correlated random effects, reported  */
/*              in Figures 5 and 6 and Table 3.                       */
/*  Input:      kb2016sppq_mc_indvars.dta                             */
/*  Output:     kb2016sppq_mc02.log,                                  */
/*              kb2016sppq_mc02.dta,                                  */
/*              kb2016sppq_mc02-XXX.gph                               */
...
program define pehasim, rclass
	...
	* Generate random effects.
	...
	matrix C = `scale'*(1 , 0.15 , 0.15 \ 0.15 , 0.3 , 0.10 \ 0.15 , 0.10 , 0.3)
	
	drawnorm u_b0 u_b1 u_b2, means(0 0 0) cov(C) n(30) clear
	...
	* M2: PEHA with fixed effects to model heterogeneity in effects
	...
	* Use capture in case it doesn't converge. 
	...
	capture noisily melogit adopt berry_norm medincome_norm nbrs_lag time, iterate($maxiters) || policy_num: berry_norm medincome_norm, cov(unstructured)
	...
	* Capture values if converges, missing otherwise. 
	 ...
	 if _rc==0 {
	  scalar b_0_mru 	= 	_b[_cons]
	  scalar b_x1_mru 	= 	_b[berry_norm]
	  scalar b_x2_mru 	= 	_b[medincome_norm]
	  scalar b_nb_mru 	= 	_b[nbrs_lag]
	  scalar b_ti_mru 	= 	_b[time]
	  scalar se_b0_mru 	= 	_se[_cons]
	  scalar se_x1_mru 	= 	_se[berry_norm]
	  scalar se_x2_mru 	= 	_se[medincome_norm]
	  scalar se_nb_mru 	= 	_se[nbrs_lag]
	  scalar se_ti_mru 	= 	_se[time]
	  
	  matrix B = e(b)
	  
	  scalar tau2_0_mru	=  	B[1,8]	  
	  scalar tau2_1_mru	=  	B[1,6]	  
	  scalar tau2_2_mru	=  	B[1,7]	  
	  scalar tau_01_mru	=  	B[1,10]	  
	  scalar tau_02_mru	=  	B[1,11]	  
	  scalar tau_12_mru	=  	B[1,9]	  
	  }
...
save kb2016sppq_mc02, replace emptyok
...
forvalues scale = 0.5(0.5)2 {
  ...
  simulate ///
	...
	b_0_mru=b_0_mru 	b_x1_mru=b_x1_mru 	b_x2_mru=b_x2_mru  	b_nb_mru=b_nb_mru 	b_ti_mru=b_ti_mru  /// 	
	se_b0_mru=se_b0_mru  se_x1_mru=se_x1_mru  se_x2_mru=se_x2_mru se_nb_mru=se_nb_mru  se_ti_mru=se_ti_mru ///
	tau2_0_mlm=tau2_0_mlm 	tau2_0_mrc=tau2_0_mrc 	tau2_1_mrc=tau2_1_mrc 	tau2_2_mrc=tau2_2_mrc ///
	tau2_0_mru=tau2_0_mru tau2_1_mru=tau2_1_mru tau2_2_mru=tau2_2_mru ///
	tau_01_mru=tau_01_mru tau_02_mru=tau_02_mru tau_12_mru=tau_12_mru ///
	iter_fe=iter_fe iter_mlm=iter_mlm iter_mrc=iter_mrc iter_mru=iter_mru, ///
	reps(250) seed(5563): pehasim, indvars(kb2016sppq_mc_indvars) scale(`scale')

  append using kb2016sppq_mc02
	
  save kb2016sppq_mc02, replace
...
foreach var of varlist *_mru {

  replace `var' = . if iter_mru == $maxiters
...
generat tau_0_mru = sqrt(tau2_0_mru)
generat tau_1_mru = sqrt(tau2_1_mru)
generat tau_2_mru = sqrt(tau2_2_mru)
  
	* label variables
  summarize

  label variable b_0_log	"Regression"
  label variable b_0_fes	"Fixed Effects"
  label variable b_0_clu	"Clustered"
  label variable b_0_mlm	"MLM"
  label variable b_0_mrc	"MLM w/ Rand Coef (Ind.)"
  label variable b_0_mru	"MLM w/ Rand Coef"
  ...
  label variable b_x1_mrc	"MLM w/ Rand Coef (Ind.)"
  label variable b_x1_mru	"MLM w/ Rand Coef"
  ...
  label variable b_x2_mrc	"MLM w/ Rand Coef (Ind.)"
  label variable b_x2_mru	"MLM w/ Rand Coef"
  ...
  label variable b_nb_mrc	"MLM w/ Rand Coef (Ind.)"
  label variable b_nb_mru	"MLM w/ Rand Coef"
  ...
  label variable b_ti_mrc	"MLM w/ Rand Coef (Ind.)"
  label variable b_ti_mru	"MLM w/ Rand Coef"
  ...
  label variable se_x1_mrc	"MLM w/ Rand Coef (Ind.)"
  label variable se_x1_mru	"MLM w/ Rand Coef"
  ...
  label variable se_x2_mrc	"MLM w/ Rand Coef (Ind.)"
  label variable se_x2_mru	"MLM w/ Rand Coef"
  ...
  label variable se_nb_mrc	"MLM w/ Rand Coef (Ind.)"
  label variable se_nb_mru	"MLM w/ Rand Coef"
  ...
  label variable se_ti_mrc	"MLM w/ Rand Coef (Ind.)"
  label variable se_ti_mru	"MLM w/ Rand Coef"
...
twoway kdensity b_x2_log, scheme(s1mono) by(scale_b2, legend(off) note("") title(Income)) subtitle(, fcolor(white)) lcolor(gs0) lpattern(dot) 	///
	|| kdensity b_x2_fes, lcolor(gs0) lpattern(shortdash)  ///
	|| kdensity b_x2_mlm, lcolor(gs0) lpattern(dash)   ///
	|| kdensity b_x2_mrc, lcolor(gs0) lpattern(longdash)     ///
	|| kdensity b_x2_mru, lcolor(gs0) lpattern(solid) xline(1) ///
	ylabel(#7, grid) ytitle("") xtitle("") ///
	xsize(3.25) ysize(3) ///
	name(b2_bw, replace)
	
twoway kdensity b_nb_log, scheme(s1mono) by(scale_bn, note("") title(Neighbors)) subtitle(, fcolor(white)) lcolor(gs0) lpattern(dot) 	///
	|| kdensity b_nb_fes, lcolor(gs0) lpattern(shortdash)  ///
	|| kdensity b_nb_mlm, lcolor(gs0) lpattern(dash)   ///
	|| kdensity b_nb_mrc, lcolor(gs0) lpattern(longdash)     ///
	|| kdensity b_nb_mru, lcolor(gs0) lpattern(solid) xline(0.25) ///
	ylabel(#7, grid) ytitle("") xtitle("") ///
	xsize(3.25) ysize(3) ///
	legend(label(1 Logit) label(2 Fixed Effects) label(3 Random Effects)   label(4 Random Coeffs. (Ind.))  label(5 Random Coeffs.) rows(1) size(small) symxsize(large)) ///
	name(bn_bw, replace)
...
grc1leg b2_bw bn_bw, scheme(s1mono) ///
	...
	imargin(tiny) ///
	xsize(6.5) ysize(8) ///	
	saving(kb2016sppq_mc02-fig05.gph, replace)
...
collapse (mean) b_* se_* ///
   (sd) sd_b0_log=b_0_log sd_x1_log=b_x1_log sd_x2_log=b_x2_log sd_nb_log=b_nb_log sd_ti_log=b_ti_log ///
	...
	sd_b0_mrc=b_0_mrc sd_x1_mrc=b_x1_mrc sd_x2_mrc=b_x2_mrc sd_nb_mrc=b_nb_mrc sd_ti_mrc=b_ti_mrc ///
	sd_b0_mru=b_0_mru sd_x1_mru=b_x1_mru sd_x2_mru=b_x2_mru sd_nb_mru=b_nb_mru sd_ti_mru=b_ti_mru, by(scale)
...
foreach var of varlist se_* {
	...
	save kb2016sppq_mc02-collapsed, replace
...
graph bar se_x1_ sd_x1_, over(scale) over(model, label(labsize(medsmall))) scheme(s1mono) ///
	...
	saving(kb2016sppq_mc02-sesd_b1-bw, replace)
...
graph bar se_nb_ sd_nb_, over(scale) over(model, label(labsize(medsmall))) scheme(s1mono) ///
	...
	saving(kb2016sppq_mc02-sesd_bn-bw, replace)

		/* Use these two as good examples. */
	
grc1leg sesd_b1 sesd_bn, scheme(s1mono) ///
	rows(2) imargin(small) ///
	xsize(6.5) ysize(6) ///
	saving(kb2016sppq_mc02-fig06, replace)
...
use kb2016sppq_mc02-collapsed, clear
...
reshape long b_log_ b_fes_ b_clu_ b_mlm_ b_mrc_ b_mru_ se_log_ se_fes_ se_clu_ se_mlm_ se_mrc_ se_mru_ sd_log_ sd_fes_ sd_clu_ sd_mlm_ sd_mrc_ sd_mru_, i(scale) j(variable) string
  ...
  outsheet c variable *log* *fes* *clu* *mlm* *mrc* *mru* using kb2016sppq_mc02-table03.csv, replace comma
