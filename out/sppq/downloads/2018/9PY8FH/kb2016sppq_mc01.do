...
log using kb2016sppq_mc01.log, text replace

/* *************************************************************      */
/*  File Name:  kb2016sppq_mc01.do                                    */
...
/*              monte carlo reported in Figures 1 and 2 and in the    */
/*              tables in the supplemental appendix.                  */
/*  Input:      kb2016sppq_mc_indvars.dta                             */
/*  Output:     kb2016sppq_mc01.log,                                  */
/*              kb2016sppq_mc01.dta,                                  */
/*              kb2016sppq_mc01-XXX.gph                               */
...
program define pehasim, rclass
	...
	* Generate random effects.
	...
	matrix C = `scale'*(1 , 0 , 0 \ 0 , 0.3 , 0 \ 0 , 0 , 0.3)
	...
	* M2: PEHA with fixed effects to model heterogenerat in effects
	...
	capture logit adopt berry_norm medincome_norm nbrs_lag time i.policy, iterate(50)
	...
	* M4: PEHA with random effects, group is policy
	...
	capture melogit adopt berry_norm medincome_norm nbrs_lag time || policy_num:
	...
	* M5: mlm with random parameters, group is policy and random effect on ideology and income
	...
	capture melogit adopt berry_norm medincome_norm nbrs_lag time || policy_num: berry_norm medincome_norm, cov(independent)
...
save kb2016sppq_mc01, replace emptyok
...
forvalues scale = 0.5(0.5)2 {
  ...
  append using kb2016sppq_mc01
	
  save kb2016sppq_mc01, replace
...
grc1leg b2_bw bn_bw, scheme(s1mono) ///
	...
	imargin(tiny) ///
	xsize(6.5) ysize(8) ///
	saving(kb2016sppq_mc01-fig01, replace)
...
foreach var of varlist se_* {
	...
	save kb2016sppq_mc01-collapsed, replace
...
grc1leg sesd_b1 sesd_bn, scheme(s1mono) ///
	...
	xsize(6.5) ysize(6) ///
	saving(kb2016sppq_mc01-fig02, replace)
...
use kb2016sppq_mc01-collapsed, clear
...
reshape long b_log_ b_fes_ b_clu_ b_mlm_ b_mrc_ se_log_ se_fes_ se_clu_ se_mlm_ se_mrc_ sd_log_ sd_fes_ sd_clu_ sd_mlm_ sd_mrc_, i(scale) j(variable) string
  ...
  outsheet c variable *log* *fes* *clu* *mlm* *mrc* using kb2016sppq_mc01-table02.csv, replace comma
