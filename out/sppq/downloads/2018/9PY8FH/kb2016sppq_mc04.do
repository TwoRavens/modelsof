...
log using kb2016sppq_mc04.log, text replace

/* *************************************************************      */
/*  File Name:  kb2016sppq_mc04.do                                    */
...
/*              monte carlo that correlates random effects with the   */
/*              exogeneous variables, reported in Figures 9 and 10    */
/*              and Table 5.                                          */
/*  Input:      kb2016sppq_mc_indvars.dta                             */
/*  Output:     kb2016sppq_mc04.log,                                  */
/*              kb2016sppq_mc04.dta,                                  */
/*              kb2016sppq_mc04-XXX.gph                               */
...
global maxiters 50
		...
		/* Create a variable that varies across policies. */
...
generat policy = _n
		...
		/* This will have mean 0 and sd 1. */

generat salience = 3.46*(uniform() - 0.5)

save kb2016sppq_mc04_indvars, replace	

use kb2016sppq_mc_indvars, clear

	expand 30
	
	  sort stateno year
	  
	  egen policy = seq(), from(1) to(30) 
	
  merge m:1 policy using kb2016sppq_mc04_indvars, assert(match)
...
program define pehasim, rclass
	...
	* Generate random effects.
	...
	collapse (mean) salience, by(policy)
	
	rename salience u1
		
		/* Doing it this way should correlate the random effects with x but still hold x fixed. */
		
	matrix C = `scale'*(1 , 0.5 , 0.15 , 0.15 \ 0.5 , 1 , 0 , 0 \ 0.15 , 0 , 0.3 , 0 \ 0.15 , 0 , 0 , 0.3)
	
		matrix H = cholesky(C)	

		drawnorm u2 u3 u4, means(0, 0, 0) sds(1, 1, 1)
		
		mkmat u1 u2 u3 u4, matrix(X)
		...
		correlate u1 u_b0 u_b1 u_b2, cov

		scalar corr_x_u0 = el(r(C),2,1)
		scalar corr_x_u1 = el(r(C),3,1)
		scalar corr_x_u2 = el(r(C),4,1)
	...
	* Now merge with independent variables
	...
	merge 1:1 stateno year policy using `indvars', keep(match)
	...
	* Loop over years to accumulate neighbors' adoptions.
	...
	forvalues t = 1/40 {
	  ...
	  if `t' == 1 {
		...
		replace ystar = (-4 + u_b0) + (0.5 + u_b1)*berry_norm + (1 + u_b2)*medincome_norm + 0.5*salience + logit(uniform()) if time == `t'
	  ...
	  if `t' > 1 {
		...
		replace ystar = (-4 + u_b0) + (0.5 + u_b1)*berry_norm + (1 + u_b2)*medincome_norm + 0.5*salience + 0.25*nbrs_lag + logit(uniform()) if time == `t'
	...
	* M1: an uncorrected PEHA, that doesn't account for group correlation or heterog effects
	...
	logit adopt berry_norm medincome_norm salience nbrs_lag time 
	
	  scalar b_0_log 		= 	_b[_cons]
	  scalar b_x1_log 		= 	_b[berry_norm]
	  scalar b_x2_log 		= 	_b[medincome_norm]
	  scalar b_sa_log 		= 	_b[salience]
	...
	* M3: PEHA with clustering, group is policy
	...
	logit adopt berry_norm medincome_norm salience nbrs_lag time, cluster(policy_num)
	
	  scalar b_0_clu 		= 	_b[_cons]
	  scalar b_x1_clu 		= 	_b[berry_norm]
	  scalar b_x2_clu 		= 	_b[medincome_norm]
	  scalar b_sa_clu 		= 	_b[salience]
	...
	* Use capture in case it doesn't converge. 
	...
	capture noisily melogit adopt berry_norm medincome_norm salience nbrs_lag time, iterate($maxiters) || policy_num:
	...
	* Use capture in case it doesn't converge. 
	...
	capture noisily melogit adopt berry_norm medincome_norm salience nbrs_lag time, iterate($maxiters) || policy_num: berry_norm medincome_norm, cov(independent)
...
save kb2016sppq_mc04, replace emptyok
...
forvalues scale = 0.5(0.5)2 {
  ...
  simulate ///
	scale=(`scale') ///
	b_0_log=b_0_log 	b_x1_log=b_x1_log 	b_x2_log=b_x2_log 	b_sa_log=b_sa_log 	b_nb_log=b_nb_log 	b_ti_log=b_ti_log  ///	
	se_b0_log=se_b0_log 	se_x1_log=se_x1_log 	se_x2_log=se_x2_log	se_sa_log=se_sa_log 	se_nb_log=se_nb_log 	se_ti_log=se_ti_log  ///
	...
	b_0_clu=b_0_clu 	b_x1_clu=b_x1_clu 	b_x2_clu=b_x2_clu  	b_sa_clu=b_sa_clu 	b_nb_clu=b_nb_clu 	b_ti_clu=b_ti_clu  ///	
	se_b0_clu=se_b0_clu 	se_x1_clu=se_x1_clu 	se_x2_clu=se_x2_clu 	se_sa_clu=se_sa_clu 	se_nb_clu=se_nb_clu 	se_ti_clu=se_ti_clu  ///
	b_0_mlm=b_0_mlm 	b_x1_mlm=b_x1_mlm 	b_x2_mlm=b_x2_mlm  	b_sa_mlm=b_sa_mlm 	b_nb_mlm=b_nb_mlm 	b_ti_mlm=b_ti_mlm  ///
	se_b0_mlm=se_b0_mlm 	se_x1_mlm=se_x1_mlm 	se_x2_mlm=se_x2_mlm	se_sa_mlm=se_sa_mlm 	se_nb_mlm=se_nb_mlm 	se_ti_mlm=se_ti_mlm  ///
	b_0_mrc=b_0_mrc 	b_x1_mrc=b_x1_mrc 	b_x2_mrc=b_x2_mrc  	b_sa_mrc=b_sa_mrc 	b_nb_mrc=b_nb_mrc 	b_ti_mrc=b_ti_mrc  /// 	
	se_b0_mrc=se_b0_mrc 	se_x1_mrc=se_x1_mrc 	se_x2_mrc=se_x2_mrc	se_sa_mrc=se_sa_mrc 	se_nb_mrc=se_nb_mrc 	se_ti_mrc=se_ti_mrc ///
	...
	corr_x_u0=corr_x_u0 corr_x_u1=corr_x_u1 corr_x_u2=corr_x_u2, ///
	reps(500) seed(5563): pehasim, indvars(kb2016sppq_mc04_indvars) scale(`scale')

  append using kb2016sppq_mc04
	
  save kb2016sppq_mc04, replace
	...
	/****************************/
		...
		/* Calculate average correlations between ideology and errors. */
	
table scale, c(mean corr_x_u0 mean corr_x_u1 mean corr_x_u2)
...
generat tau_2_mrc = sqrt(tau2_2_mrc)
  ...
  label variable se_sa_log	"Regression"
  label variable se_sa_clu	"Clustered"
  label variable se_sa_mlm	"MLM"
  label variable se_sa_mrc	"MLM w/ Rand Coef"
...
twoway kdensity b_sa_log, scheme(s1mono) by(scale_bs, legend(off) note("") title(Salience)) subtitle(, fcolor(white)) lcolor(gs0) lpattern(shortdash) 	///
	|| kdensity b_sa_mlm, lcolor(gs0) lpattern(dash) ///
	|| kdensity b_sa_mrc, lcolor(gs0) lpattern(solid) xline(0.5) ///
	ylabel(#7, grid) ytitle("") xtitle("") ///
	xsize(3.25) ysize(3) ///
	name(bs_bw, replace) 
...
grc1leg b2_bw bs_bw bn_bw, scheme(s1mono) ///
	rows(3) ///
	imargin(tiny) ///
	xsize(6.5) ysize(7) ///
	saving(kb2016sppq_mc04-fig09, replace)
...
collapse (mean) b_* se_* ///
   (sd) sd_b0_log=b_0_log sd_x1_log=b_x1_log sd_x2_log=b_x2_log sd_sa_log=b_sa_log sd_nb_log=b_nb_log sd_ti_log=b_ti_log ///
	sd_b0_fes=b_0_fes sd_x1_fes=b_x1_fes sd_x2_fes=b_x2_fes sd_nb_fes=b_nb_fes sd_ti_fes=b_ti_fes ///
	sd_b0_clu=b_0_clu sd_x1_clu=b_x1_clu sd_x2_clu=b_x2_clu sd_sa_clu=b_sa_clu sd_nb_clu=b_nb_clu sd_ti_clu=b_ti_clu ///
	sd_b0_mlm=b_0_mlm sd_x1_mlm=b_x1_mlm sd_x2_mlm=b_x2_mlm sd_sa_mlm=b_sa_mlm sd_nb_mlm=b_nb_mlm sd_ti_mlm=b_ti_mlm ///
	sd_b0_mrc=b_0_mrc sd_x1_mrc=b_x1_mrc sd_x2_mrc=b_x2_mrc sd_sa_mrc=b_sa_mrc sd_nb_mrc=b_nb_mrc sd_ti_mrc=b_ti_mrc, by(scale)
...
foreach var of varlist se_* {
	...
	save kb2016sppq_mc04-collapsed, replace
...
reshape long se_b0_ se_x1_ se_x2_ se_sa_ se_nb_ se_ti_ sd_b0_ sd_x1_ sd_x2_ sd_sa_ sd_nb_ sd_ti_ , i(scale) j(model) string
  ...
  replace model = "Random Coeffs." if model == "mrc"
...
graph bar se_sa_ sd_sa_, over(scale) over(model, label(labsize(medsmall))) scheme(s1mono) ///
	bar(1, color(gs2)) /// 
	bar(2, color(gs8)) ///
	title(Coefficient on Salience) ///
	legend(label(1 Standard Error) label(2 Standard Deviation) size(small)) ///
	xsize(6.5) ysize(3) ///
	name(sesd_bs, replace) 
...
graph bar se_nb_ sd_nb_, over(scale) over(model, label(labsize(medsmall))) scheme(s1mono) ///
	...
	name(sesd_bn, replace) 
		...
		/* Use these three as good examples. */
	
grc1leg sesd_b1 sesd_bn sesd_bs, scheme(s1mono) ///
	rows(3) imargin(small) ///
	xsize(6.5) ysize(8) ///
	saving(kb2016sppq_mc04-fig10, replace)
...
use kb2016sppq_mc04-collapsed, clear
...
reshape long b_log_ b_fes_ b_clu_ b_mlm_ b_mrc_ se_log_ se_fes_ se_clu_ se_mlm_ se_mrc_ sd_log_ sd_fes_ sd_clu_ sd_mlm_ sd_mrc_, i(scale) j(variable) string
  ...
  outsheet c variable *log* *fes* *clu* *mlm* *mrc* using kb2016sppq_mc04-table05.csv, replace comma
