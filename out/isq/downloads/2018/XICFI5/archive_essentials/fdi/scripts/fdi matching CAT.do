...
log using "fdi/results/fdi_CAT.smcl", replace
...
gen treat = 0
replace treat = 1 if cat[_n]==1 & cat[_n-1]==0 & (countryname[_n]==countryname[_n-1]) & ((year[_n-1]+1)==year[_n])
...
** xi: xtreg  fdi2 i.year l.cat l.physint l.empinx l.ln_rgdpc l.ln_rgdpc_sq  l.ln_trade  l.polity2  l.durable  l.gdpgrowth  l.exratemdev_80_00  l.gdpdeflator  if OECD!=1, fe
...
gen dropme = 0
replace dropme = 1 if cat==1
foreach i of numlist 1/6 {
  replace dropme = 1 if cat[_n+`i']==1 & (countryname[_n]==countryname[_n+`i']) & (year[_n]+`i'==year[_n+`i'])
}
