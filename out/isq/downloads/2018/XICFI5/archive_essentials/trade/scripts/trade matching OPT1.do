...
gen treat = 0 
replace treat = 1 if opt1name_2[_n]==1 & opt1name_2[_n-1]==0 & (name_1[_n]==name_1[_n-1]) & (name_2[_n]==name_2[_n-1]) & ((year[_n-1]+1)==year[_n])
...
** xi: areg imports i.year opt1name_2 physint1 empinx1 gattFF gattFO ptarecip ptanonrecip gsp currencyunion colorbit gdp distance share_language share_border landlocked island land_area if OECD==1 & OECDname2==0, absorb(directed_dyad_id) cluster(directed_dyad_id) robust
...
local controls imports opt1name_2 physint1 empinx1 gattFF gattFO ptarecip ptanonrecip gsp currencyunion colorbit gdp distance share_language share_border landlocked island land_area 
...
local keepme name_1 name_2 directed_dyad_id year f1* f2* f3* f4* f5* l1* l2* l3* l4* l5*  ///
             imports opt1name_2 physint1 empinx1 gattFF gattFO ptarecip ptanonrecip gsp currencyunion colorbit gdp distance share_language share_border landlocked island land_area ///
             elig opt1name_1 eimports
...
gen dropme = 0
replace dropme = 1 if opt1name_2==1
foreach i of numlist 1/6 {
  replace dropme = 1 if opt1name_2[_n+`i']==1 & (name_1[_n]==name_1[_n+`i']) & (name_2[_n]==name_2[_n+`i']) & (year[_n]+`i'==year[_n+`i'])
}
...
save "trade/madedata/alltradeOPT1.dta", replace
...
library(foreign)
dat <- read.dta("trade/madedata/alltradeOPT1.dta", convert.underscore=T)
...
for(i in 1:ncol(dat)){
  if(length(grep("l1",names(dat)[i]))>0){
    ...
    if(names(dat)[i]=="l1opt1name.2"){next}
...
#setwd("C:\\Documents and Settings\\Rich\\Desktop\\Rewards for Ratification\\Matching analysis")
library(foreign)
#write.dta(mout$match.dat,"trade/madedata/mahmatches_OPT1trade.dta")

write.dta(cdat,"trade/madedata/mahmatchescaliper_OPT1trade.dta")
...
use "trade/madedata/mahmatchescaliper_OPT1trade.dta", clear

xtset directed_dyad_id


** This is trade between OECD treaty ratifiers and non-OECD countries
xtreg imports012345 treated l1* l2* l3* l4* l5* if OECD==1 & OECDname2==0 & opt1name_1==1, cluster(directed_dyad_id) robust re
table treated if e(sample)==1

xtreg imports012345 treated l1* l2* l3* if OECD==1 & OECDname2==0 & opt1name_1==1, cluster(directed_dyad_id) robust re
table treated if e(sample)==1

** This is all trade with all countries
xtreg imports012345 treated l1* l2* l3* if opt1name_1==1, cluster(directed_dyad_id) robust re
table treated if e(sample)==1

** with OECDname2==0 and treaty members
xtreg imports012345 treated l1* l2* l3* if OECDname2==0 & opt1name_1==1, cluster(directed_dyad_id) robust re
table treated if e(sample)==1


** 21 May 2012 -- Additional robustness checks
log using "trade/results/trade_OPT1.smcl", replace
...
use "trade/madedata/mahmatchescaliper_OPT1trade.dta", clear
xtset directed_dyad_id
** 28 may 2012 -- figured out that these DON'T get dropped from one of the main regressions
**  but it doesn't change the result at all.  Change these models (and the other trade do files)
**  to exclude these if I do a future re-running of these models.
** Probably also a good idea to manually drop lags of the time-invariant vars: island, landlocked, shared border, shared language
*drop  l1opt1name_2 l2opt1name_2 l3opt1name_2 l4opt1name_2 l5opt1name_2

** Original Models
xtreg imports012345 treated l1* l2* l3* l4* l5* if OECD==1 & OECDname2==0 & opt1name_1==1, cluster(directed_dyad_id) robust re
outreg2 using "trade/results/trade table.doc", word append
xtreg imports012 treated l1* l2* l3* l4* l5* if OECD==1 & OECDname2==0 & opt1name_1==1, cluster(directed_dyad_id) robust re
xtreg imports123 treated l1* l2* l3* l4* l5* if OECD==1 & OECDname2==0 & opt1name_1==1, cluster(directed_dyad_id) robust re
...
** No controls
reg imports012345 treated if OECD==1 & OECDname2==0 & opt1name_1==1, cluster(directed_dyad_id) robust
reg imports012 treated if OECD==1 & OECDname2==0 & opt1name_1==1, cluster(directed_dyad_id) robust
reg imports123 treated if OECD==1 & OECDname2==0 & opt1name_1==1, cluster(directed_dyad_id) robust
...
** Fixed effects
areg imports012345 treated l1* l2* l3* l4* l5* if OECD==1 & OECDname2==0 & opt1name_1==1, a(directed_dyad_id)
areg imports012 treated l1* l2* l3* l4* l5* if OECD==1 & OECDname2==0 & opt1name_1==1, a(directed_dyad_id)
areg imports123 treated l1* l2* l3* l4* l5* if OECD==1 & OECDname2==0 & opt1name_1==1, a(directed_dyad_id)
...
xtreg diffimports01234 treated l1* l2* l3* l4* l5* if OECD==1 & OECDname2==0 & opt1name_1==1, cluster(directed_dyad_id) robust re
xtreg diffimports012 treated l1* l2* l3* l4* l5* if OECD==1 & OECDname2==0 & opt1name_1==1, cluster(directed_dyad_id) robust re
xtreg diffimports123 treated l1* l2* l3* l4* l5* if OECD==1 & OECDname2==0 & opt1name_1==1, cluster(directed_dyad_id) robust re
...
xtreg imports treated l1* if OECD==1 & OECDname2==0 & opt1name_1==1, cluster(directed_dyad_id) robust re
...
xtreg f1imports treated imports physint1 empinx1 gattFF gattFO ptarecip ptanonrecip gsp currencyunion gdp distance  share_language  share_border  landlocked island land_area if OECD==1 & OECDname2==0 & opt1name_1==1, cluster(directed_dyad_id) robust re
...
xtreg imports21012 treated l1* l2* l3* l4* l5* if OECD==1 & OECDname2==0 & opt1name_1==1, cluster(directed_dyad_id) robust re
xtreg imports210 treated l1* l2* l3* l4* l5* if OECD==1 & OECDname2==0 & opt1name_1==1, cluster(directed_dyad_id) robust re
...
use "trade/madedata/alltradeOPT1.dta", clear
xtset directed_dyad_id
