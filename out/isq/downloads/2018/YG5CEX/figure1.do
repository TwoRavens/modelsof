...
use "Data Files\CPGold.dta", clear;
...
outreg using billresults, replace;
...
generate x_betahat1 = SN_b1*(h_fixed + 1)
                + SN_b2*h_lowcbi  
                + SN_b3*(h_fixed+1)*h_lowcbi 
                + SN_b4*(h_fixed+1)*`a' 
                + SN_b5*h_lowcbi*`a'  
                + SN_b6*(h_fixed+1)*h_lowcbi*`a'  
                + SN_b7*h_endogenous 
                + SN_b8*h_lnDistricts 
                + SN_b9*h_singleparty_govt;
...
graph twoway line risk_hat MV if risk_hat<1000& (MV>=3) & (MV<=15) , clwidth(medium) clcolor(blue) clcolor(black)
        ...
        ||   line risk_hi  MV if risk_hi<500 & (MV>=3) & (MV<=15), clpattern(dash) clwidth(thin) clcolor(black)
        ||  ,   
