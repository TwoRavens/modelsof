...
oprobit LoTMilitaryRecode ITERATELoclag p_polity2lagrecode ITERATEpolitylag gdppclag poplag civilwarlag ucdp_type2lag catlag RstrctAccess dissentlag, robust cluster (ccodecow) nolog;;
        ...
        ||   line diff_hi5  MV, clpattern(dash) clwidth(thin) clcolor(black)
      	||  ,
			...
			legend(col(1) order(1 2)  label(1 "Marginal Effect of Increasing Attacks from 0 to 20") 
                                      ...
                                      label(3 " "))
            yline(0)
            title("Marginal Effect of Increase in Transnational Attacks (0 to 20 Attacks)" "on the Probability of Systemic Military Torture", size(4))
            subtitle(" " "At Varying Levels of Democracy" " ", size(3))
            xtitle("Level of Democracy", size(3.5)  )
            ytitle("Marginal Effect of Transnational Attacks", size(3.5))
            xsca(titlegap(2))
            ysca(titlegap(2))
