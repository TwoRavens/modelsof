clear
etime, start
log using "\\file\UsersW$\wrr15\Home\My Documents\My Files\XINDONG XUE\ECONOMICS E-JOURNAL (Revision)\DATA AND PROGRAMS\TABLE3.log", replace
set more off
set scheme s2mono
set type double
...
// I then estimate the best model from TABLE 2.
...
// I then reestimate the best model, replacing the dependent variable with the
// new binary variable
oglm SRHH stdtrust stdsocial stdsp age female han partymember loginc  ///
midses highses unmarried divorced widowed  primaryeduc junioreduc univeduc ///
gini lpincom if urban==1, hetero(loginc midses unmarried gini) link(probit) cluster(county)
...
// As a check I also estimate a homoskedastic probit and check the margin values
...
// As a further check, I estimate a LPM

regress SRHH stdtrust stdsocial stdsp age female han partymember loginc  ///
midses highses unmarried divorced widowed  primaryeduc junioreduc univeduc ///
gini lpincom if urban==1, cluster(county)


//We then follow the same approach for RURAL 
...
oglm SRHH stdurbsn age female han partymember loginc midses highses ///
unmarried divorced widowed primaryeduc junioreduc univeduc gini lpincom ///
if urbann==1, hetero(primaryeduc gini) link(probit) cluster(county)
...
probit SRHH stdurbsn age female han partymember loginc midses highses ///
unmarried divorced widowed primaryeduc junioreduc univeduc gini lpincom ///
...
regress SRHH stdurbsn age female han partymember loginc midses highses ///
unmarried divorced widowed primaryeduc junioreduc univeduc gini lpincom ///
if urbann==1, cluster(county)
...
oglm SRHH stdrursn age female han partymember loginc midses highses ///
unmarried divorced widowed primaryeduc junioreduc univeduc gini lpincom ///
if urbann==0, hetero(primaryeduc junioreduc) link(probit) cluster(county)
...
probit SRHH stdrursn age female han partymember loginc midses highses ///
unmarried divorced widowed primaryeduc junioreduc univeduc gini lpincom ///
...
regress SRHH stdrursn age female han partymember loginc midses highses ///
unmarried divorced widowed primaryeduc junioreduc univeduc gini lpincom ///
if urbann==0, cluster(county)
