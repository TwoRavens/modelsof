...
foreach c of numlist 93/110 {
	preserve
	simulate vetoP=r(v), reps(10000) saving(vetosimDataSenate`c', replace): votesim if congress == `c'
	restore
}

foreach c of numlist 93/110 {
	
	quietly use vetosimDataSenate`c', clear
	quietly gen veto = (vetoP > 0)
	quietly summ
	display as text "Senate `c':      " 1 - r(mean)
}
