...
**********
capture log close
log using "Data analysis\Data management\msim-data07-allysimvalued.log", replace


* ***********************************************************************************************************
* Generates component variables for calculation of similarity measures based on valued alliance relationships
* ***********************************************************************************************************

* Programme:	msim-data07-allysimvalued.do
...
* Description
*************
* This do-file generates the component variables for the calculation of various similarity measures based on valued alliance relationships.
* In the process, the yearly socio-matrices of valued alliance relationships are transformed into undirected dyadic datasets.
...
foreach year of numlist 1816/2000 {
	...
	* Load directed alliance dataset of the respective year
	use "Datasets\Derived\Individual years\Alliances\Valued\msim-data04a-allyvalued-`year'.dta", clear
	...
	* Define variables and generate dataset
	postfile `sim' /*
		...
		*/ using "Datasets\Derived\Individual years\Alliances\Valued\Similarity\msim-data07-allysimvalued-`year'.dta", replace
	...
	* Loop through all possible combinations of country pairs
	foreach country1 of local cabblist {
		foreach country2 of local cabblist {
			...
			* Generate weighted sum of maximum absolute difference: sum[weight*3]
			generate sadwm = matcap*3 if nonmiss == 1
			qui sum sadwm
			local sadwm = r(sum)
			...
			* Generate sum of weighted maximum squared difference: sum[(weight*3)^2]
			generate ssdwm = matcap*9 if nonmiss == 1
			qui sum ssdwm
			local ssdwm = r(sum)
	...
	* Load dataset
	use "Datasets\Derived\Individual years\Alliances\Valued\Similarity\msim-data07-allysimvalued-`year'.dta", clear
	...
	* Save dataset with labeled variables
	compress
	save "Datasets\Derived\Individual years\Alliances\Valued\Similarity\msim-data07-allysimvalued-`year'.dta", replace
